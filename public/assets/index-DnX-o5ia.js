...
Šalių parašų vietos ir data: [įrašykite]`}];function Ne(){return Object.fromEntries(Me.map(e=>[e.id,e.value]))}function Pe(){let e=Ne();try{let t=localStorage.getItem(Oe);if(!t)return e;let n=JSON.parse(t);Me.forEach(t=>{typeof n[t.id]==`string`&&(e[t.id]=n[t.id])})}catch{return e}return e}function Fe({root:e,renderHeader:t,renderFooter:n}){I({title:`Baldų pirkimo sutarties šablonas | Pirkėjo įrankiai`,description:`Naršyklėje redaguojamas ir spausdinamas B2C baldų pirkimo sutarties struktūros šablonas su aiškiu teisinės konsultacijos ribojimu.`,path:Te,type:`article`,structuredData:[F([{name:`Gamintojų katalogas`,path:`/`},{name:`Pirkėjo gidas`,path:`/gidas`},{name:`Baldų pirkimo sutarties šablonas`,path:Te}])]});let r=Pe();e.innerHTML=`
  ...
  `}function Mt(e){let t=new Set(q(e.category_codes));return t.has(`W`)?{slug:`spintos-ir-drabuzines-kaina`,label:`Spintų ir drabužinių kainos bei apimties klausimai`}:t.has(`K`)?{slug:`kvarcas-ar-akmuo-stalvirsiui`,label:`Stalviršio medžiagos ir apimties klausimai`}:t.has(`OC`)||t.has(`HR`)?{slug:`matavimas-ir-montavimas-kontrole`,label:`Matavimo ir montavimo kontrolinis sąrašas`}:t.has(`SW`)?{slug:`mdf-faneruote-masyvas-fasadai`,label:`Medžiagų ir apdailos klausimai`}:{slug:`matavimas-ir-montavimas-kontrole`,label:`Matavimo ir montavimo kontrolinis sąrašas`}}function Nt(e){let t=[],n=e.city?.trim(),r=n?ye(U).find(e=>e.city===n):void 0;r&&t.push({kind:`Miestas`,slug:r.slug,label:`Baldų gamintojų kandidatai: ${r.city}`});let i=new Set(q(e.category_codes));return he.forEach(e=>{i.has(e.code)&&t.push({kind:`Kategorija`,slug:e.slug,label:e.title})}),t}function Pt(e){let t=Nt(e);if(!t.length)return null;let n=document.createElement(`section`);return n.className=`profile-landings`,n.setAttribute(`aria-labelledby`,`profile-landings-title`),n.innerHTML=`
    <div class="section-heading">
  ...
  `,n}function Ft(){let e=ye(U);return`
  ...
  `}function Z(e,t,n,r,i){let a=document.createElement(`div`);a.className=`filter-field`;let o=document.createElement(`label`);o.htmlFor=e,o.textContent=t;let s=document.createElement(`div`);s.className=`select-wrap`;let c=document.createElement(`select`);c.id=e,c.name=e;let l=document.createElement(`option`);return l.value=``,l.textContent=n,c.append(l),r.forEach(e=>{let t=document.createElement(`option`);t.value=e.value,t.textContent=e.label,t.selected=e.value===i,c.append(t)}),s.append(c),a.append(o,s),a}function It(e){let t=document.createElement(`div`);t.className=`filter-toggle`;let n=document.createElement(`input`);n.id=e,n.name=e,n.type=`checkbox`,n.checked=W.registryCheckedOnly;let r=document.createElement(`label`);r.htmlFor=e;let i=document.createElement(`strong`);i.textContent=`Tik patikrinti registro duomenys`;let a=document.createElement(`span`);return a.textContent=`Tai duomenų būsenos žyma, ne kokybės ar prieinamumo garantija.`,r.append(i,a),t.append(n,r),t}function Lt(e){let t=document.createElement(`div`);t.className=`filter-grid filter-grid--advanced`;let n=Z(`${e}-size-filter`,`Įmonės dydis`,`Visi darbuotojų skaičiai`,et,W.employeeBand),r=Z(`${e}-founded-filter`,`Įkūrimo laikotarpis`,`Visi įkūrimo metai`,tt,W.foundedPeriod),i=It(`${e}-registry-filter`);return t.append(n,r,i),{fields:t,employeeBandSelect:n.querySelector(`select`),foundedPeriodSelect:r.querySelector(`select`),registryToggle:i.querySelector(`input`)}}function Rt(e){if(!Et(e))return null;let t=document.createElement(`p`);t.className=`registry-card-status`;let n=document.createElement(`span`);n.className=`registry-check-mark`,n.setAttribute(`aria-hidden`,`true`);let r=document.createElement(`strong`);r.textContent=`Registro duomenys patikrinti`,t.append(n,r);let i=pt(e.verified_at);if(i){let e=document.createTextNode(` · `),n=document.createElement(`time`);n.dateTime=i.iso,n.textContent=i.label,t.append(e,n)}return t}function zt(e){let t=document.createElement(`article`);t.className=`manufacturer-card`;let n=document.createElement(`div`);n.className=`card-heading`;let r=document.createElement(`h3`);r.textContent=K(e.trading_name,`Pavadinimas nenurodytas`);let i=jt(e);if(i){let e=document.createElement(`p`);e.className=`legal-name`,e.textContent=i,n.append(r,e)}else n.append(r);let a=document.createElement(`p`);a.className=`location-line`;let o=document.createElement(`strong`);o.textContent=K(e.city,`Miestas nenurodytas`);let s=document.createElement(`span`);s.setAttribute(`aria-hidden`,`true`),s.textContent=` · `;let c=document.createElement(`span`);c.textContent=`Regiono grupė: ${K(e.region_label,`nenurodyta`)}`,a.append(o,s,c);let l=document.createElement(`p`);l.className=e.description_lt?.trim()?`description`:`description description--fallback`,l.textContent=K(e.description_lt,`Trumpas aprašymas šaltiniuose nepateiktas.`);let u=document.createElement(`ul`);u.className=`category-list`,u.setAttribute(`aria-label`,`Gaminamų baldų kategorijos`);let d=q(e.category_labels);(d.length?d:[`Kategorijos šaltiniuose nenurodytos`]).forEach(e=>{let t=document.createElement(`li`);t.textContent=e,u.append(t)});let f=document.createElement(`a`);f.className=`profile-link`,f.href=`/gamintojas/${encodeURIComponent(e.slug)}${window.location.search}`,f.dataset.internalLink=`true`,f.textContent=`Peržiūrėti katalogo įrašą`;let p=document.createElement(`span`);p.setAttribute(`aria-hidden`,`true`),p.textContent=` →`,f.append(p),t.append(n);let m=Rt(e);return m&&t.append(m),t.append(a,l,u,f),t}function Bt(){V&&(V.innerHTML=`
    ...
    ${X()}
  `)}function Vt(){let e=document.querySelector(`#browse-content`);e&&(e.innerHTML=`
  ...
  `)}function Ht(){V&&(I({title:`Katalogas nepasiekiamas | Baldai pagal užsakymą Lietuvoje`,description:`Gamintojų katalogo duomenų šiuo metu nepavyko gauti.`,path:window.location.pathname,robots:`noindex, follow`}),V.innerHTML=`
    ${Y(`directory`)}
    ...
    ${X()}
  `,document.querySelector(`#retry-button`)?.addEventListener(`click`,()=>void _n()))}function Ut(){let e=document.querySelector(`#browse-content`);if(!e)return;let t=St(U),n=Ct(U,`city`,`city`),r=Ct(U,`region`,`region_label`);wt(t,n,r),e.replaceChildren();let i=document.createElement(`div`);i.className=`browse-controls`;let a=document.createElement(`div`);a.className=`controls-heading`,a.innerHTML=`
    <div>
  ...
  `;let o=document.createElement(`form`);o.className=`filter-form`,o.setAttribute(`role`,`search`),o.addEventListener(`submit`,e=>e.preventDefault());let s=document.createElement(`div`);s.className=`filter-field filter-field--search`;let c=document.createElement(`label`);c.htmlFor=`directory-search`,c.textContent=`Ieškoti kataloge`;let l=document.createElement(`input`);l.id=`directory-search`,l.name=`paieska`,l.type=`search`,l.autocomplete=`off`,l.placeholder=`Pavadinimas, aprašymas ar kategorija`,l.value=W.query,s.append(c,l);let u=document.createElement(`div`);u.className=`filter-grid filter-grid--primary`,u.append(s,Z(`category-filter`,`Baldų kategorija`,`Visos kategorijos`,t,W.category),Z(`city-filter`,`Miestas`,`Visi miestai`,n,W.city),Z(`region-filter`,`Šaltinio regiono grupė`,`Visos regiono grupės`,r,W.region));let d=Lt(`directory`),f=document.createElement(`div`);f.className=`filter-actions`;let p=document.createElement(`p`);p.textContent=`Paieška atnaujinama iškart vedant tekstą.`;let m=document.createElement(`button`);m.className=`text-button`,m.id=`clear-filters`,m.type=`button`,m.textContent=`Išvalyti paiešką ir filtrus`,f.append(p,m),o.append(u,d.fields,f),i.append(a,o);let h=document.createElement(`div`);h.className=`results-area`,h.id=`results-area`,e.append(i,h),l.addEventListener(`input`,()=>{W.query=l.value.trimStart(),J(`replace`),Q()}),document.querySelector(`#category-filter`)?.addEventListener(`change`,e=>{W.category=e.currentTarget.value,J(`push`),Q()}),document.querySelector(`#city-filter`)?.addEventListener(`change`,e=>{W.city=e.currentTarget.value,J(`push`),Q()}),document.querySelector(`#region-filter`)?.addEventListener(`change`,e=>{W.region=e.currentTarget.value,J(`push`),Q()}),d.employeeBandSelect.addEventListener(`change`,()=>{W.employeeBand=d.employeeBandSelect.value,J(`push`),Q()}),d.foundedPeriodSelect.addEventListener(`change`,()=>{W.foundedPeriod=d.foundedPeriodSelect.value,J(`push`),Q()}),d.registryToggle.addEventListener(`change`,()=>{W.registryCheckedOnly=d.registryToggle.checked,J(`push`),Q()}),m.addEventListener(`click`,()=>{kt(),J(`push`),Ut(),document.querySelector(`#directory-search`)?.focus()}),Q()}function Q(){let e=document.querySelector(`#results-area`);if(!e)return;window.location.pathname===`/`&&hn();let t=Dt(U),n=!!(W.query||W.category||W.city||W.region||Ot());e.replaceChildren();let r=document.createElement(`div`);r.className=`result-header`;let i=document.createElement(`p`);if(i.className=`result-count`,i.setAttribute(`role`,`status`),i.setAttribute(`aria-live`,`polite`),i.textContent=n?`Rodoma įrašų: ${t.length}. Iš viso kataloge: ${U.length}.`:`Kataloge – ${At(U.length)}.`,r.append(i),n){let e=document.createElement(`p`);e.className=`active-filters`,e.textContent=`Aktyvi atranka pagal pasirinktus kriterijus`,r.append(e)}if(e.append(r),t.length===0){let t=document.createElement(`div`);t.className=`message-state`;let n=document.createElement(`p`);n.className=`state-label`,n.textContent=`Rezultatų nėra`;let r=document.createElement(`h3`);r.textContent=`Pagal šiuos kriterijus įrašų nerasta`;let i=document.createElement(`p`);i.textContent=`Pakeiskite paieškos žodį, pasirinkite platesnę vietovę arba išvalykite filtrus.`;let a=document.createElement(`button`);a.className=`primary-button`,a.type=`button`,a.textContent=`Išvalyti visus kriterijus`,a.addEventListener(`click`,()=>{kt(),J(`push`),Ut()}),t.append(n,r,i,a),e.append(t);return}let a=document.createElement(`div`);a.className=`manufacturer-list`,t.forEach(e=>a.append(zt(e))),e.append(a)}function $(e,t){let n=document.createElement(`div`),r=document.createElement(`dt`);r.textContent=e;let i=document.createElement(`dd`);return i.textContent=t,n.append(r,i),n}function Wt(e,t){let n=document.createElement(`div`),r=document.createElement(`dt`);r.textContent=`Įmonės dydžio signalas`;let i=document.createElement(`dd`),a=document.createElement(`strong`);a.textContent=ut(t);let o=document.createElement(`p`);o.className=`fact-explanation`,o.textContent=`Tai viešame įmonės įraše nurodyta darbuotojų skaičiaus grupė. Ji neparodo darbų kokybės, dabartinio užimtumo ar galimybės priimti jūsų projektą.`,i.append(a,o);let s=q(e.public_details_source_urls).filter(e=>e.includes(`rekvizitai.vz.lt`));if(s.length){let e=document.createElement(`ul`);e.className=`fact-source-list`,s.forEach((t,n)=>{let r=document.createElement(`li`),i=document.createElement(`a`);i.href=t,i.target=`_blank`,i.rel=`noopener noreferrer`,i.textContent=s.length===1?`Atverti viešą darbuotojų skaičiaus šaltinį`:`Atverti viešą šaltinį ${n+1}`,r.append(i),e.append(r)}),i.append(e)}return n.append(r,i),n}function Gt(e,t){let n=document.createElement(`div`),r=document.createElement(`dt`);r.textContent=e;let i=document.createElement(`dd`),a=t?.trim();if(a){let e=document.createElement(`a`);e.href=a,e.target=`_blank`,e.rel=`noopener noreferrer`,e.textContent=a,i.append(e)}else i.textContent=`Viešuose šaltiniuose nenurodyta.`,i.className=`unknown-value`;return n.append(r,i),n}function Kt(e,t){let n=document.createElement(`div`),r=document.createElement(`dt`);r.textContent=e;let i=document.createElement(`dd`),a=document.createElement(`a`);return a.href=`tel:${t.replace(/[^+\d]/g,``).replace(/(?!^)\+/g,``)}`,a.textContent=t,i.append(a),n.append(r,i),n}function qt(e){let t=[],n=e.company_code?.trim(),r=e.public_phone?.trim(),i=e.street_address?.trim(),a=e.postcode?.trim(),o=e.employee_count_band?.trim();if(n&&t.push($(`Įmonės kodas`,n)),i&&t.push($(`Registracijos adresas`,i)),a&&t.push($(`Pašto kodas`,a)),Number.isInteger(e.founded_year)&&t.push($(`Įkurta`,String(e.founded_year))),o&&t.push(Wt(e,o)),r&&t.push(Kt(`Viešas telefono numeris`,r)),!t.length)return null;let s=document.createElement(`section`);s.className=`profile-details profile-public-details`,s.setAttribute(`aria-labelledby`,`profile-public-details-title`),s.innerHTML=`
    <div class="section-heading">
  ...
  `}function tn(e,t){let n=document.querySelector(`#landing-filter-controls`),r=document.querySelector(`#landing-manufacturer-list`);if(!n||!r)return;wt([],[],[],!1),n.replaceChildren();let i=document.createElement(`form`);i.className=`filter-form landing-filter-form`,i.setAttribute(`role`,`search`),i.addEventListener(`submit`,e=>e.preventDefault());let a=document.createElement(`div`);a.className=`filter-field filter-field--search`;let o=document.createElement(`label`);o.htmlFor=`landing-search`,o.textContent=`Ieškoti šiame sąraše`;let s=document.createElement(`input`);s.id=`landing-search`,s.name=`paieska`,s.type=`search`,s.autocomplete=`off`,s.placeholder=`Pavadinimas, aprašymas ar kategorija`,s.value=W.query,a.append(o,s);let c=Lt(`landing`),l=document.createElement(`div`);l.className=`filter-grid filter-grid--landing`,l.append(a,...Array.from(c.fields.children));let u=document.createElement(`div`);u.className=`filter-actions`;let d=document.createElement(`p`);d.className=`landing-filter-count`,d.setAttribute(`role`,`status`),d.setAttribute(`aria-live`,`polite`);let f=document.createElement(`button`);f.className=`text-button`,f.type=`button`,f.textContent=`Išvalyti šio sąrašo filtrus`,u.append(d,f),i.append(l,u),n.append(i);let p=()=>{kt(),J(`push`),s.value=``,c.employeeBandSelect.value=``,c.foundedPeriodSelect.value=``,c.registryToggle.checked=!1,m(),s.focus()},m=()=>{t();let n=Dt(e,!1),i=!!(W.query||Ot());if(d.textContent=i?`Rodoma įrašų: ${n.length}. Iš viso šiame sąraše: ${e.length}.`:`Šiame sąraše – ${At(e.length)}.`,r.replaceChildren(),!n.length){let e=document.createElement(`div`);e.className=`message-state landing-empty-state`;let t=document.createElement(`h3`);t.textContent=`Pagal šiuos kriterijus įrašų nerasta`;let n=document.createElement(`p`);n.textContent=`Pasirinkite platesnį įmonės dydį ar įkūrimo laikotarpį, pakeiskite paiešką arba išvalykite filtrus.`;let i=document.createElement(`button`);i.className=`primary-button`,i.type=`button`,i.textContent=`Išvalyti šio sąrašo filtrus`,i.addEventListener(`click`,p),e.append(t,n,i),r.append(e);return}n.forEach(e=>r.append(zt(e)))};s.addEventListener(`input`,()=>{W.query=s.value.trimStart(),J(`replace`),m()}),c.employeeBandSelect.addEventListener(`change`,()=>{W.employeeBand=c.employeeBandSelect.value,J(`push`),m()}),c.foundedPeriodSelect.addEventListener(`change`,()=>{W.foundedPeriod=c.foundedPeriodSelect.value,J(`push`),m()}),c.registryToggle.addEventListener(`change`,()=>{W.registryCheckedOnly=c.registryToggle.checked,J(`push`),m()}),f.addEventListener(`click`,p),m()}function nn(e){if(!V)return;let t=he.find(t=>t.slug===e),n=ye(U),r=n.find(t=>t.slug===e);if(!t&&!r){on(`Paieškos puslapis nerastas`,`Tokio kategorijos ar miesto puslapio nėra. Grįžkite į katalogą ir naudokite paiešką arba filtrus.`);return}let i=t?U.filter(e=>q(e.category_codes).includes(t.code)):U.filter(e=>e.city===r?.city),a=t?t.title:`Baldų gamintojų kandidatai: ${r?.city}`,o=t?t.intro:`Čia pateikiami ${i.length} nepatvirtinti baldų gamintojų kandidatai, kurių viešo šaltinio įraše kaip bazės miestas ar vietovė nurodytas ${r?.city}.`,s=t?t.buyer_note:`Šis sąrašas nepatvirtina, kad kandidatai aptarnauja visą miestą ar aplinkinį regioną. Matavimo, pristatymo ir montavimo vietas patikrinkite tiesiogiai.`,c=t?Qt(t):$t(r?.city??`šiame mieste`),l=`/baldai-pagal-uzsakyma/${e}`,u=t?`${t.title}: ${i.length} viešais šaltiniais paremti nepatvirtinti Lietuvos gamintojų kandidatai, miestai ir atrankos gairės.`:`${r?.city}: ${i.length} viešuose šaltiniuose šiame mieste registruoti baldų gamintojų kandidatai. Sąrašas nėra paslaugų teritorijos ar kokybės garantija.`,d=()=>{I({title:`${a} | Gamintojų katalogas`,description:u,path:l,robots:window.location.search?`noindex, follow`:`index, follow`,structuredData:[F([{name:`Gamintojų katalogas`,path:`/`},{name:a,path:l}]),Ce(c),we(i)]})};d();let f=t?n.map(e=>({...e,count:i.filter(t=>t.city===e.city).length})).filter(e=>e.count>0).sort((e,t)=>t.count-e.count||at.compare(e.city,t.city)):[],p=r?he.map(e=>({...e,count:i.filter(t=>q(t.category_codes).includes(e.code)).length})).filter(e=>e.count>0):[];V.innerHTML=`
    ${Y(`directory`)}
    ...
    ${X()}
  `,tn(i,d)}function rn(){V&&Qe({root:V,renderHeader:Y,renderFooter:X,manufacturers:U})}function an(e){V&&(I({title:e.title,description:e.description,path:e.path,structuredData:[F([{name:`Gamintojų katalogas`,path:`/`},{name:e.heading,path:e.path}])]}),V.innerHTML=`
    ${Y(`policy`)}
    ...
    ${X()}
  `)}function on(e,t){V&&(I({title:`${e} | Baldai pagal užsakymą Lietuvoje`,description:t,path:window.location.pathname,robots:`noindex, follow`}),V.innerHTML=`
    ${Y(`directory`)}
    <main class="profile-main">
    ...
    ${X()}
  `)}function sn(e){if(!V)return;V.innerHTML=`
    ${Y(`directory`)}
  ...
  `;V.innerHTML=`
    ...
    ${X()}
  `}function ln(e,t){if(!V)return;let n=`/gidas/${e.slug}`;I({title:`${e.title} | Pirkėjo gidas`,description:e.summary,path:n,type:`article`,structuredData:[F([{name:`Gamintojų katalogas`,path:`/`},{name:`Pirkėjo gidas`,path:`/gidas`},{name:e.title,path:n}])]}),V.innerHTML=`
    ${Y(`guide`)}
  ...
  `)}function pn(){V&&(I({title:`Gido straipsnis nerastas | Pirkėjo gidas`,description:`Prašomas pirkėjo gido straipsnis nerastas.`,path:window.location.pathname,robots:`noindex, follow`}),V.innerHTML=`
    ${Y(`guide`)}
    <main class="article-main">
      <a class="back-link" href="/gidas" data-internal-link="true">← Grįžti į pirkėjo gidą</a>
    ...
    ${X()}
  `)}function mn(){let e=window.location.pathname.replace(/\/+$/,``)||`/`;if(e===`/gidas`){cn();return}if(e===`/gidas/baldu-pirkimo-sutarties-sablonas`){if(!V)return;Fe({root:V,renderHeader:Y,renderFooter:X});return}let t=decodeURIComponent(e.split(`/`).filter(Boolean)[1]??``);if(H.find(e=>e.slug===t)?.featured){if(V?.querySelector(`.guide-article`))return;window.location.replace(`${e}/`);return}if(t===`trumpasis-sarasas`){un();return}if(t===`uzklausa-ir-pasiulymas`){dn();return}if(t===`terminai`){fn();return}pn()}function hn(){I({title:`Baldai pagal užsakymą Lietuvoje | Gamintojų katalogas`,description:`Viešais šaltiniais paremtas nepatvirtintų Lietuvos nestandartinių baldų gamintojų kandidatų katalogas su paieška pagal kategoriją ir vietą.`,path:`/`,robots:window.location.search?`noindex, follow`:`index, follow`})}function gn(){let e=yt();if(e){an(e);return}if(ht()){mn();return}if(vt()){if(!V)return;Ge({root:V,renderHeader:Y,renderFooter:X});return}if(_t()){rn();return}if(window.location.pathname.startsWith(`/gamintojas/`)){let e=decodeURIComponent(window.location.pathname.split(`/`).filter(Boolean)[1]??``);sn(U.find(t=>t.slug===e));return}if(gt()){nn(decodeURIComponent(window.location.pathname.split(`/`).filter(Boolean)[1]??``));return}if(window.location.pathname!==`/`){on(`Tokio puslapio nėra`,`Patikrinkite adresą arba grįžkite į gamintojų katalogą.`);return}hn(),Bt(),Ut()}async function _n(){if(!st){st=!0,window.location.pathname===`/`?V?.hasChildNodes()||(Bt(),Vt()):window.location.pathname.startsWith(`/gamintojas/`)?V&&!V.hasChildNodes()&&(V.innerHTML=`
        ${Y(`directory`)}
        <main class="profile-main">
          <div class="loading-state" role="status" aria-live="polite">
            <span class="loading-mark" aria-hidden="true"><span></span><span></span><span></span></span>
        ...
        ${X()}
      `):_t()&&V&&!V.hasChildNodes()&&(V.innerHTML=`
        ${Y(`request`)}
        ...
        ${X()}
      `);try{U=await xt(),ct=!0,W=mt(),gn()}catch(e){console.error(`Nepavyko gauti gamintojų katalogo.`,e),Ht()}finally{st=!1}}}function vn(){if(W=mt(),ht()||bt()||vt()){gn();return}if(ct){gn();return}_n()}document.addEventListener(`click`,e=>{let t=e.target;if(!(t instanceof Element))return;let n=t.closest(`a[data-internal-link="true"]`);if(!n||e.defaultPrevented||e.button!==0||e.metaKey||e.ctrlKey||e.shiftKey||e.altKey)return;let r=new URL(n.href,window.location.origin);r.origin===window.location.origin&&(e.preventDefault(),window.history.pushState({},``,`${r.pathname}${r.search}${r.hash}`),vn(),window.scrollTo({top:0,behavior:`auto`}))}),window.addEventListener(`popstate`,vn),ht()||bt()||vt()?gn():_n();