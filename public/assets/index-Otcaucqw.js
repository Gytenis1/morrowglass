...
Šalių parašų vietos ir data: [įrašykite]`}];function Pe(){return Object.fromEntries(Ne.map(e=>[e.id,e.value]))}function Fe(){let e=Pe();try{let t=localStorage.getItem(ke);if(!t)return e;let n=JSON.parse(t);Ne.forEach(t=>{typeof n[t.id]==`string`&&(e[t.id]=n[t.id])})}catch{return e}return e}function Ie({root:e,renderHeader:t,renderFooter:n}){F({title:`Baldų pirkimo sutarties šablonas | Pirkėjo įrankiai`,description:`Naršyklėje redaguojamas ir spausdinamas B2C baldų pirkimo sutarties struktūros šablonas su aiškiu teisinės konsultacijos ribojimu.`,path:Ee,type:`article`,structuredData:[P([{name:`Gamintojų katalogas`,path:`/`},{name:`Pirkėjo gidas`,path:`/gidas`},{name:`Baldų pirkimo sutarties šablonas`,path:Ee}])]});let r=Fe();e.innerHTML=`
    ${t(`guide`)}
    ...
    <fieldset class="offer-editor" data-offer-id="${I(e.id)}">
      ...
      <div class="offer-primary-fields">
        <div class="form-field">
          ...
          <input id="provider-${e.id}" data-field="provider" type="text" maxlength="160" value="${I(e.provider)}" placeholder="Pasiūlyme nurodytas pavadinimas">
        </div>
        <div class="form-field">
          <label for="total-${e.id}">Bendra suma su PVM, €</label>
          <input id="total-${e.id}" data-field="totalInclVat" type="number" min="0" step="0.01" inputmode="decimal" value="${I(e.totalInclVat)}" placeholder="0,00">
        </div>
      </div>
      <div class="offer-criteria">
        ${L.map(t=>`
          <section class="offer-criterion" data-criterion="${t.id}">
            <div class="form-field">
              <label for="evidence-${e.id}-${t.id}">${I(t.evidenceLabel)}</label>
              <textarea id="evidence-${e.id}-${t.id}" data-evidence="${t.id}" rows="3" maxlength="3000" placeholder="${I(t.placeholder)}">${I(e.evidence[t.id])}</textarea>
            </div>
      ...
      <div class="form-field">
        ...
        <textarea id="notes-${e.id}" data-field="notes" rows="3" maxlength="3000" placeholder="Klausimai, versijos data, ką dar reikia patikslinti">${I(e.notes)}</textarea>
      </div>
    </fieldset>
  `}function Ue(e){return{weights:Object.fromEntries(L.map(t=>{let n=e.querySelector(`[data-weight="${t.id}"]`);return[t.id,Math.min(5,Math.max(0,Number(n?.value)||0))]})),offers:Array.from(e.querySelectorAll(`[data-offer-id]`)).map((e,t)=>{let n=Re(t);return n.id=e.dataset.offerId??n.id,n.provider=e.querySelector(`[data-field="provider"]`)?.value.trim()??``,n.totalInclVat=e.querySelector(`[data-field="totalInclVat"]`)?.value??``,n.notes=e.querySelector(`[data-field="notes"]`)?.value.trim()??``,L.forEach(t=>{n.evidence[t.id]=e.querySelector(`[data-evidence="${t.id}"]`)?.value.trim()??``,n.ratings[t.id]=Number(e.querySelector(`[data-rating="${t.id}"]`)?.value)||0}),n})}}function We(e,t){try{localStorage.setItem(Ae,JSON.stringify(e)),t&&(t.textContent=`Palyginimo duomenys išsaugoti tik šioje naršyklėje.`)}catch{t&&(t.textContent=`Naršyklė neleido išsaugoti. Duomenys liks tik iki puslapio uždarymo.`)}}function Ge(e,t){let n=e.querySelector(`#comparison-results`);if(!n)return;let r=L.reduce((e,n)=>e+t.weights[n.id],0);if(r===0){n.innerHTML=`<div class="form-status form-status--error" role="alert">Bent vienam kriterijui nustatykite didesnį nei 0 svorį.</div>`,n.focus();return}let i=t.offers.map((e,n)=>{let i=L.reduce((n,r)=>n+e.ratings[r.id]*t.weights[r.id],0),a=r*3,o=a?i/a*100:0,s=[];return e.provider||s.push({type:`missing`,label:`trūksta tiekėjo pavadinimo`}),(!e.totalInclVat||Number(e.totalInclVat)<0)&&s.push({type:`missing`,label:`trūksta bendros sumos su PVM`}),L.forEach(t=>{e.evidence[t.id]||s.push({type:`missing`,label:`trūksta: ${t.evidenceLabel.toLocaleLowerCase(`lt-LT`)}`}),e.ratings[t.id]===1&&s.push({type:`nonComparable`,label:`nepalyginama: ${t.label.toLocaleLowerCase(`lt-LT`)}`})}),{offer:e,index:n,numerator:i,denominator:a,score:o,issues:s}}).sort((e,t)=>t.score-e.score||e.index-t.index),a=new Map;i.forEach(e=>{let t=e.score.toFixed(6);a.set(t,(a.get(t)??0)+1)});let o=new Intl.NumberFormat(`lt-LT`,{style:`currency`,currency:`EUR`});n.innerHTML=`
    ...
    <div class="comparison-result-list">
      ${i.map(e=>{let n=e.offer.provider||`Pasiūlymas ${e.index+1}`,r=(a.get(e.score.toFixed(6))??0)>1,i=e.offer.totalInclVat&&Number.isFinite(Number(e.offer.totalInclVat))?o.format(Number(e.offer.totalInclVat)):`Suma nepateikta`,s=e.issues.filter(e=>e.type===`missing`),c=e.issues.filter(e=>e.type===`nonComparable`);return`
          <article class="comparison-result" aria-labelledby="result-${e.offer.id}">
            <header>
              <div>
                <h3 id="result-${e.offer.id}">${I(n)}</h3>
                <p>${I(i)} · skaičiavimas ${e.numerator} / ${e.denominator}</p>
              </div>
            ...
            ${s.length||c.length?`
              <div class="comparison-issues">
                ${s.length?`<p><strong>Trūksta:</strong> ${I(s.map(e=>e.label.replace(/^trūksta:?\s*/i,``)).join(`; `))}.</p>`:``}
                ${c.length?`<p><strong>Nepalyginama:</strong> ${I(c.map(e=>e.label.replace(/^nepalyginama:?\s*/i,``)).join(`; `))}.</p>`:``}
              </div>
            ...
            <dl class="comparison-evidence-list">
              ${L.map(n=>`
                <div class="${e.offer.evidence[n.id]?e.offer.ratings[n.id]===1?`is-non-comparable`:``:`is-missing`}">
                  <dt>${I(n.label)} <span>Svoris ${t.weights[n.id]} · vertinimas ${e.offer.ratings[n.id]}</span></dt>
                  <dd>${I(e.offer.evidence[n.id]||`Duomenų nepateikta.`)}</dd>
                </div>
              `).join(``)}
              <div><dt>Pastabos</dt><dd>${I(e.offer.notes||`Pastabų nepateikta.`)}</dd></div>
            </dl>
          </article>
        `}).join(``)}
    </div>
  `,n.focus()}function Ke({root:e,renderHeader:t,renderFooter:n}){F({title:`Palyginti baldų pasiūlymus | Pirkėjo įrankiai`,description:`Deterministinis 2–5 baldų pasiūlymų palyginimas pagal pirkėjo svorius, aiškią formulę, įrodymus, trūkstamus ir nepalyginamus laukus.`,path:De,structuredData:[P([{name:`Gamintojų katalogas`,path:`/`},{name:`Pirkėjo įrankiai`,path:`/gidas`},{name:`Pasiūlymų palyginimas`,path:De}])]});let r=Be(),i=()=>{e.innerHTML=`
      ${t(`guide`)}
      <main class="buyer-tool-main comparison-tool-main">
        ...
        <section class="comparison-offers" aria-labelledby="offers-title">
          ...
          <div class="offer-editor-list">
            ${r.offers.map((e,t)=>He(e,t,r.offers.length>2)).join(``)}
          </div>
      ...
      ${n()}
    `;let a=e.querySelector(`#comparison-save-status`),o=0,s=()=>{r=Ue(e),We(r,a)};e.querySelectorAll(`input, textarea, select`).forEach(e=>{e.addEventListener(`input`,()=>{a&&(a.textContent=`Saugoma…`),window.clearTimeout(o),o=window.setTimeout(s,220)}),e.addEventListener(`change`,s)}),e.querySelector(`#add-offer`)?.addEventListener(`click`,()=>{r=Ue(e),!(r.offers.length>=5)&&(r.offers.push(Re(r.offers.length)),We(r),i())}),e.querySelectorAll(`.remove-offer`).forEach(t=>{t.addEventListener(`click`,()=>{if(r=Ue(e),r.offers.length<=2)return;let n=t.closest(`[data-offer-id]`);r.offers=r.offers.filter(e=>e.id!==n?.dataset.offerId),We(r),i()})}),e.querySelector(`#calculate-comparison`)?.addEventListener(`click`,()=>{r=Ue(e),We(r,a),Ge(e,r)}),e.querySelector(`#reset-comparison`)?.addEventListener(`click`,()=>{if(window.confirm(`Išvalyti visus šioje naršyklėje išsaugotus pasiūlymus ir svorius?`)){try{localStorage.removeItem(Ae)}catch{}r=ze(),i()}})};i()}var qe=[`Virtuvės baldai`,`Spintos ar įmontuojami baldai`,`Miegamojo ar vonios baldai`,`Biuro ar komerciniai baldai`,`HoReCa ar prekybos baldai`,`Minkšti baldai`,`Medžio masyvo ar kiti nestandartiniai baldai`,`Kitas baldų projektas`],Je=[`Idėja ir poreikių formavimas`,`Yra preliminarūs matmenys`,`Yra patalpos planas ar projektas`,`Parinktos pagrindinės medžiagos`,`Objektas parengtas galutiniam matavimui`,`Reikia pakeisti ar papildyti esamą projektą`];function Ye(e,t){return`<option value="">${I(t)}</option>${e.map(e=>`<option value="${I(e)}">${I(e)}</option>`).join(``)}`}function R(e){return`<p class="field-error" id="error-${e}" aria-live="polite"></p>`}function z(e,t,n=``){let r=e.querySelector(`[name="${t}"]`),i=e.querySelector(`#error-${t.replaceAll(`_`,`-`)}`);r&&(r.setAttribute(`aria-invalid`,n?`true`:`false`),r.setCustomValidity(n)),i&&(i.textContent=n)}function Xe(e){e.querySelectorAll(`.field-error`).forEach(e=>{e.textContent=``}),e.querySelectorAll(`[aria-invalid]`).forEach(e=>{e.setAttribute(`aria-invalid`,`false`),e.setCustomValidity(``)});let t=e.querySelector(`#rfq-error-summary`);t&&(t.hidden=!0,t.innerHTML=``)}function Ze(e,t){Xe(e);let n=[],r=(e,r,i)=>{let a=t.elements.namedItem(e);(!a||a.value.trim().length<r)&&n.push({field:e,message:`${i}: įrašykite bent ${r} ženkl${r===1?`ą`:`ų`}.`})};r(`full_name`,2,`Vardas ir pavardė`),r(`category`,2,`Projekto kategorija`),r(`municipality`,2,`Savivaldybė`),r(`service_region`,2,`Paslaugos vieta`),r(`project_stage`,2,`Projekto etapas`),r(`project_scope`,10,`Projekto apimtis`),r(`dimensions_room_count`,1,`Matmenys ir patalpų skaičius`),r(`materials_requirements`,1,`Medžiagų reikalavimai`),r(`installation_access_constraints`,1,`Montavimo ir patekimo sąlygos`),t.elements.namedItem(`email`)?.validity.valid||n.push({field:`email`,message:`Nurodykite galiojantį el. pašto adresą.`});let i=t.elements.namedItem(`phone`);i?.value&&!/^[0-9+().\-\s]+$/.test(i.value.trim())&&n.push({field:`phone`,message:`Telefono numeryje naudokite tik skaitmenis ir įprastus skyrybos ženklus.`});let a=t.elements.namedItem(`budget_min`),o=t.elements.namedItem(`budget_max`),s=Number(a?.value),c=Number(o?.value);if((!a?.value||!Number.isFinite(s)||s<0)&&n.push({field:`budget_min`,message:`Nurodykite neneigiamą mažiausią biudžetą.`}),(!o?.value||!Number.isFinite(c)||c<0)&&n.push({field:`budget_max`,message:`Nurodykite neneigiamą didžiausią biudžetą.`}),a?.value&&o?.value&&Number.isFinite(s)&&Number.isFinite(c)&&c<s&&n.push({field:`budget_max`,message:`Didžiausias biudžetas negali būti mažesnis už mažiausią.`}),t.elements.namedItem(`desired_completion_date`)?.value||n.push({field:`desired_completion_date`,message:`Nurodykite pageidaujamą užbaigimo datą.`}),e.querySelectorAll(`[name="preferred_shortlist"]:checked`).length>8&&n.push({field:`preferred_shortlist`,message:`Pasirinkite ne daugiau kaip 8 gamintojų kandidatus.`}),n.forEach(t=>z(e,t.field,t.message)),!n.length&&t.checkValidity())return!0;let l=e.querySelector(`#rfq-error-summary`);return l&&(l.hidden=!1,l.innerHTML=`<strong>Patikrinkite formą:</strong><ul>${n.map(e=>`<li>${I(e.message)}</li>`).join(``)}</ul>`,l.focus()),n.length||t.reportValidity(),!1}function Qe(e,t){if(!t||typeof t!=`object`)return!1;let n=t.data;if(!n||typeof n!=`object`)return!1;let r=!1;return Object.entries(n).forEach(([t,n])=>{if(!n||typeof n!=`object`)return;let i=n.message;typeof i==`string`&&(z(e,t,i),r=!0)}),r}function $e({root:e,renderHeader:t,renderFooter:n,manufacturers:r=[]}){F({title:`Pateikite saugią baldų projekto užklausą | Baldai pagal užsakymą Lietuvoje`,description:`Struktūruota baldų projekto pasiūlymo užklausa operatoriaus peržiūrai, be automatinio siuntimo gamintojams ir su atskiru patvirtinimu prieš išsiuntimą.`,path:Oe,robots:window.location.search?`noindex, follow`:`index, follow`,structuredData:[P([{name:`Gamintojų katalogas`,path:`/`},{name:`Projekto užklausa`,path:Oe}])]});let i=new URLSearchParams(window.location.search).get(`gamintojas`)?.trim()??``,a=i?r.find(e=>e.slug===i):void 0,o=!!(i&&!a),s=(a?[a,...r.filter(e=>e.slug!==a.slug)]:r).map(e=>{let t=[e.city?.trim(),(e.category_labels??[]).slice(0,2).join(`, `)].filter(Boolean).join(` · `);return`
      <label class="manufacturer-choice">
        <input type="checkbox" name="preferred_shortlist" value="${I(e.slug)}"${e.slug===a?.slug?` checked`:``}>
        <span><strong>${I(e.trading_name)}</strong><small>${I(t||`Vieta ir kategorijos viešame įraše nenurodytos`)}</small></span>
      </label>
    `}).join(``);e.innerHTML=`
    ${t(`request`)}
        ...
        <section class="request-form-section" aria-labelledby="request-form-title">
          ...
          <form class="buyer-request-form rfq-form" id="rfq-form" novalidate>
            ...
            <div class="form-field">
              ...
              <div class="select-wrap"><select id="category" name="category" required aria-describedby="error-category">${Ye(qe,`Pasirinkite kategoriją`)}</select></div>
              ${R(`category`)}
            </div>
            <div class="form-field">
            ...
            <div class="form-field form-field--wide">
              ...
              <div class="select-wrap"><select id="project-stage" name="project_stage" required aria-describedby="error-project-stage">${Ye(Je,`Pasirinkite dabartinį etapą`)}</select></div>
              ${R(`project-stage`)}
            </div>
            <div class="form-field form-field--wide">
            ...
            <div class="form-field">
              ...
              ${R(`installation-access-constraints`)}
            </div>
  ...
  `;let c=e.querySelector(`#rfq-form`),l=e.querySelector(`#manufacturer-search`),u=e.querySelector(`#manufacturer-choice-list`),d=e.querySelector(`#manufacturer-empty`),f=e.querySelector(`#manufacturer-selection-count`),p=e.querySelector(`#rfq-status`),m=c?.querySelector(`button[type="submit"]`);if(!c||!l||!u||!d||!f||!p||!m)return;let h=Array.from(u.querySelectorAll(`[name="preferred_shortlist"]`)),g=t=>{let n=h.filter(e=>e.checked);n.length>8&&t?(t.checked=!1,n=h.filter(e=>e.checked),z(e,`preferred_shortlist`,`Galima pasirinkti ne daugiau kaip 8 gamintojų kandidatus.`)):z(e,`preferred_shortlist`),f.textContent=n.length===0?`Kandidatų nepasirinkta`:n.length===1?`Pasirinktas 1 kandidatas iš 8`:`Pasirinkta kandidatų: ${n.length} iš 8`};h.forEach(e=>e.addEventListener(`change`,()=>g(e))),l.addEventListener(`input`,()=>{let e=l.value.trim().toLocaleLowerCase(`lt-LT`),t=0;u.querySelectorAll(`.manufacturer-choice`).forEach(n=>{let r=!e||(n.textContent??``).toLocaleLowerCase(`lt-LT`).includes(e);n.hidden=!r,r&&(t+=1)}),d.hidden=t>0}),c.querySelectorAll(`input, textarea, select`).forEach(t=>{t.name!==`preferred_shortlist`&&(t.setAttribute(`aria-invalid`,`false`),t.addEventListener(`input`,()=>z(e,t.name)),t.addEventListener(`blur`,()=>{t.validity.valid||z(e,t.name,t.validationMessage||`Patikrinkite šį lauką.`)}))}),c.addEventListener(`submit`,async t=>{if(t.preventDefault(),!Ze(e,c))return;let n=new FormData(c),r={full_name:String(n.get(`full_name`)??``).trim(),email:String(n.get(`email`)??``).trim(),phone:String(n.get(`phone`)??``).trim(),category:String(n.get(`category`)??``).trim(),municipality:String(n.get(`municipality`)??``).trim(),service_region:String(n.get(`service_region`)??``).trim(),project_stage:String(n.get(`project_stage`)??``).trim(),project_scope:String(n.get(`project_scope`)??``).trim(),dimensions_room_count:String(n.get(`dimensions_room_count`)??``).trim(),materials_requirements:String(n.get(`materials_requirements`)??``).trim(),budget_min:Number(n.get(`budget_min`)),budget_max:Number(n.get(`budget_max`)),desired_completion_date:String(n.get(`desired_completion_date`)??``),installation_access_constraints:String(n.get(`installation_access_constraints`)??``).trim(),preferred_shortlist:h.filter(e=>e.checked).map(e=>e.value)};m.disabled=!0,m.setAttribute(`aria-busy`,`true`),m.textContent=`Pateikiama…`,p.className=`form-status`,p.textContent=`Pasiūlymo užklausa kuriama operatoriaus peržiūrai. Gamintojai nekontaktuojami.`;try{let t=await fetch(`${de.replace(/\/+$/,``)}/api/public/rfqs`,{method:`POST`,credentials:`omit`,headers:{"Content-Type":`application/json`},body:JSON.stringify(r)}),n=await t.json().catch(()=>null);if(!t.ok){if(Qe(e,n)){let t=e.querySelector(`#rfq-error-summary`);if(t){let n=Array.from(e.querySelectorAll(`.field-error`)).map(e=>e.textContent).filter(Boolean);t.hidden=!1,t.innerHTML=`<strong>Serveris paprašė pataisyti:</strong><ul>${n.map(e=>`<li>${I(e??``)}</li>`).join(``)}</ul>`,t.focus()}}throw Error(n?.message||`Pasiūlymo užklausos pateikti nepavyko.`)}if(!n?.reference)throw Error(`Serveris negrąžino pasiūlymo užklausos numerio.`);let i=e.querySelector(`#rfq-success`),a=e.querySelector(`#rfq-reference`);a&&(a.textContent=n.reference),i&&(i.hidden=!1,i.focus()),p.className=`form-status form-status--success`,p.textContent=`Pasiūlymo užklausa ${n.reference} gauta operatoriaus peržiūrai. Gamintojai nebuvo kontaktuoti automatiškai.`,m.textContent=`Pasiūlymo užklausa pateikta`,m.removeAttribute(`aria-busy`),c.querySelectorAll(`input, textarea, select`).forEach(e=>{e.disabled=!0});return}catch(e){console.error(`Nepavyko pateikti pasiūlymo užklausos.`,e),p.className=`form-status form-status--error`,p.setAttribute(`role`,`alert`),p.textContent=`Pasiūlymo užklausos pateikti nepavyko. Įvesti duomenys liko formoje. Patikrinkite pažymėtus laukus ir interneto ryšį, tada bandykite dar kartą.`,p.focus()}finally{(!m.disabled||m.textContent!==`Pasiūlymo užklausa pateikta`)&&(m.disabled=!1,m.removeAttribute(`aria-busy`),m.textContent=`Pateikti pasiūlymo užklausą operatoriaus peržiūrai`)}})}var et=50,tt=[{value:`0`,label:`0 darbuotojų`},{value:`1-9`,label:`1–9 darbuotojai`},{value:`10-49`,label:`10–49 darbuotojai`},{value:`50-249`,label:`50–249 darbuotojai`},{value:`250+`,label:`250 ir daugiau darbuotojų`}],nt=[{value:`iki-1999`,label:`Iki 1999 m.`},{value:`2000-2009`,label:`2000–2009 m.`},{value:`2010-2019`,label:`2010–2019 m.`},{value:`nuo-2020`,label:`2020 m. ir vėliau`}],rt=40,it=2e3,at=[`Virtuvės baldai`,`Spintos ar įmontuojami baldai`,`Miegamojo ar vonios baldai`,`Biuro ar komerciniai baldai`,`Minkšti baldai`,`Medžio masyvo ar kiti nestandartiniai baldai`,`Kitas projektas`],ot=new Intl.Collator(`lt`,{sensitivity:`base`}),B=document.querySelector(`#app`),V=[{slug:`trumpasis-sarasas`,title:`Kaip sudaryti pagrįstą trumpąjį sąrašą`,summary:`Atrankos seka, patikrinami kriterijai ir klausimai prieš priimant pasiūlymą.`,readingLabel:`Atranka ir patikra`},{slug:`uzklausa-ir-pasiulymas`,title:`Kaip parengti užklausą ir palyginti pasiūlymus`,summary:`Ką aprašyti, kad gamintojai vertintų tą pačią apimtį, ir kas dažniausiai keičia kainą.`,readingLabel:`Užklausa ir apimtis`},{slug:`terminai`,title:`Kaip prašyti realistiško darbų grafiko`,summary:`Terminą lemiantys kintamieji, etapai ir klausimai, padedantys valdyti neapibrėžtumą.`,readingLabel:`Terminai ir eiga`},{slug:`kaip-pasirinkti-baldu-gamintoja`,title:`Kaip pasirinkti ir palyginti baldų gamintoją`,summary:`Ką paklausti, kokius įspėjamuosius ženklus pastebėti ir kaip atsargiai skaityti nepatvirtintus katalogo įrašus.`,readingLabel:`Atranka ir patikra`,featured:!0},{slug:`virtuves-baldu-kainos`,title:`Virtuvės baldų kainos: ribos ir kainą keičiantys sprendimai`,summary:`Dvi aiškiai atskirtos viešų šaltinių nuorodos, jų datos ir praktinis sąrašas, kas keičia individualaus projekto kainą.`,readingLabel:`Kaina ir apimtis`,featured:!0},{slug:`virtuves-ir-imontuojamu-baldu-projekto-eiga`,title:`Virtuvės ir įmontuojamų baldų projekto eiga`,summary:`Tipinė etapų seka nuo matavimo iki montavimo ir kontrolinis sąrašas sprendimams, kurie veikia grafiką.`,readingLabel:`Projekto eiga`,featured:!0},{slug:`medziagos-sutartis-avansas-garantija`,title:`Medžiagos, sutartis, avansas ir garantija: ką aptarti`,summary:`LMDP ir MDF, faneruotės, masyvo, stalviršių, furnitūros, briaunų, sutarties ir garantinio aptarnavimo klausimai.`,readingLabel:`Dokumentai ir atsakomybės`,featured:!0},{slug:`spintos-ir-drabuzines-kaina`,title:`Spintos ir drabužinės kaina: ką apibrėžti prieš lyginant pasiūlymus`,summary:`Apimtis, vidaus įranga ir montavimo sąlygos, kurios padeda palyginti pasiūlymus.`,readingLabel:`Spintos ir drabužinės`,featured:!0,buyerIntent:!0},{slug:`mdf-faneruote-masyvas-fasadai`,title:`MDF, faneruotė ar masyvas fasadams: klausimai prieš pasirenkant`,summary:`Fasadų specifikacijos, pavyzdžiai, priežiūra ir kompromisai.`,readingLabel:`Fasadai ir medžiagos`,featured:!0,buyerIntent:!0},{slug:`kvarcas-ar-akmuo-stalvirsiui`,title:`Kvarcas ar natūralus akmuo stalviršiui: apimtis, priežiūra ir klausimai`,summary:`Šablonavimo, išpjovų, sujungimų ir montavimo kontrolinis sąrašas.`,readingLabel:`Stalviršiai`,featured:!0,buyerIntent:!0},{slug:`matavimas-ir-montavimas-kontrole`,title:`Galutinis matavimas ir montavimo diena: kontrolinis sąrašas`,summary:`Objekto parengtis, dokumentai ir priėmimo patikra.`,readingLabel:`Matavimas ir montavimas`,featured:!0,buyerIntent:!0},{slug:`baldu-defektai-ir-garantinis-aptarnavimas`,title:`Baldų defektai ir garantinis aptarnavimas: kaip fiksuoti ir sekti`,summary:`Dokumentavimas, pranešimas ir sutartos korekcijos sekimas.`,readingLabel:`Defektai ir aptarnavimas`,featured:!0,buyerIntent:!0},{slug:`mazo-buto-irengimas-pagal-uzsakyma`,title:`Mažo buto įrengimas pagal užsakymą: prioritetai ir užklausos sąrašas`,summary:`Saugojimo, judėjimo, matavimo ir montavimo prioritetai.`,readingLabel:`Mažas butas`,featured:!0,buyerIntent:!0}],st=[{path:`/privatumas`,title:`Privatumo pranešimas | Baldai pagal užsakymą Lietuvoje`,heading:`Privatumo pranešimas`,description:`Kaip GG Ventures UAB tvarko Baldininkai.org katalogo užklausų, atsiliepimų ir įrašų pataisymo ar atstovavimo formų asmens duomenis.`,summary:`Šiame pranešime paaiškiname, kokius asmens duomenis gauname per katalogo formas, kam juos naudojame, kiek laiko saugome ir kokias teises turite.`,content:`
      ...
      <section><h2>Duomenys ir kontaktas</h2><p>Prašymo informaciją naudojame tik katalogo peržiūrai, ryšiui ir galimiems teisiniams poreikiams pagal <a href="/privatumas" data-internal-link="true">privatumo pranešimą</a>. Tvarką administruoja <strong>GG Ventures UAB</strong>, įmonės kodas <strong>305442420</strong>; kontaktinis el. paštas <a href="mailto:info@baldininkai.org">info@baldininkai.org</a>.</p></section>`}],H=[],U=ht(),ct=!1,lt=!1;function ut(e){return e.trim().toLocaleLowerCase(`lt-LT`)}function W(e){return e.replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`).replace(/"/g,`&quot;`).replace(/'/g,`&#039;`)}function G(e,t=`Viešuose šaltiniuose nenurodyta.`){return e?.trim()||t}function dt(e){return{0:`Viešame darbuotojų skaičiaus įraše – 0 darbuotojų`,"1-9":`Labai maža komanda – 1–9 darbuotojai`,"10-49":`Nedidelė įmonė – 10–49 darbuotojai`,"50-249":`Didesnė įmonė – 50–249 darbuotojai`,"250+":`Didelė įmonė – 250 ar daugiau darbuotojų`}[e]??`${e} darbuotojų (viešo šaltinio grupė)`}function ft(e){let t=e%100,n=e%10;return t>=11&&t<=19||n===0?`${e} patvirtintų atsiliepimų`:n===1?`${e} patvirtintas atsiliepimas`:`${e} patvirtinti atsiliepimai`}function pt(e){let t=e?new Date(e):null;return!t||Number.isNaN(t.getTime())?null:{iso:t.toISOString(),label:new Intl.DateTimeFormat(`lt-LT`,{year:`numeric`,month:`long`,day:`numeric`}).format(t)}}function mt(e){let t=e?.trim()??``,n=/^(\d{4})-(\d{2})-(\d{2})$/.exec(t);if(!n)return null;let r=Number(n[1]),i=Number(n[2]),a=Number(n[3]),o=new Date(Date.UTC(r,i-1,a));return o.getUTCFullYear()!==r||o.getUTCMonth()!==i-1||o.getUTCDate()!==a?null:{iso:t,label:new Intl.DateTimeFormat(`lt-LT`,{year:`numeric`,month:`long`,day:`numeric`,timeZone:`UTC`}).format(o)}}function K(e){return Array.isArray(e)?e.filter(e=>typeof e==`string`&&e.trim().length>0):[]}function ht(){let e=new URLSearchParams(window.location.search);return{query:e.get(`q`)?.trim()??``,category:e.get(`kategorija`)??``,city:e.get(`miestas`)??``,region:e.get(`regionas`)??``,employeeBand:e.get(`dydis`)??``,foundedPeriod:e.get(`ikurta`)??``,registryCheckedOnly:e.get(`registras`)===`patikrinta`}}function q(e){let t=new URLSearchParams;U.query&&t.set(`q`,U.query),U.category&&t.set(`kategorija`,U.category),U.city&&t.set(`miestas`,U.city),U.region&&t.set(`regionas`,U.region),U.employeeBand&&t.set(`dydis`,U.employeeBand),U.foundedPeriod&&t.set(`ikurta`,U.foundedPeriod),U.registryCheckedOnly&&t.set(`registras`,`patikrinta`);let n=t.toString(),r=`${window.location.pathname}${n?`?${n}`:``}`;r!==`${window.location.pathname}${window.location.search}`&&window.history[e===`push`?`pushState`:`replaceState`]({},``,r)}function J(e=window.location.pathname){return e.replace(/\/index\.html$/,``).replace(/\/+$/,``)||`/`}function gt(e=window.location.pathname){return J(e)===`/gidas/baldu-pirkimo-sutarties-sablonas`}function _t(e=window.location.pathname){let t=J(e);return t===`/gidas`||t.startsWith(`/gidas/`)}function vt(e=window.location.pathname){return J(e).startsWith(`/baldai-pagal-uzsakyma/`)}function yt(e=window.location.pathname){return J(e)===`/gauti-pasiulymus`}function bt(e=window.location.pathname){return J(e)===`/palyginti-pasiulymus`}function xt(e=window.location.pathname){let t=e.replace(/\/+$/,``)||`/`;return st.find(e=>e.path===t)}function St(e=window.location.pathname){return!!xt(e)}async function Ct(){let e=[],t=1,n=1;do{let r=await ue.collection(`manufacturers`).getList(t,et,{sort:`trading_name`,requestKey:`manufacturers-page-${t}`});e.push(...r.items),n=r.totalPages,t+=1}while(t<=n);return e.sort((e,t)=>ot.compare(e.trading_name,t.trading_name))}function wt(e){let t=new Map;return e.forEach(e=>{K(e.category_codes).forEach((n,r)=>{let i=K(e.category_labels)[r];n&&i&&t.set(n,i)})}),Array.from(t,([e,t])=>({value:e,label:t})).sort((e,t)=>ot.compare(e.label,t.label))}function Tt(e,t,n){let r=new Map;return e.forEach(e=>{let i=e[t]?.trim(),a=e[n]?.trim();i&&a&&r.set(i,a)}),Array.from(r,([e,t])=>({value:e,label:t})).sort((e,t)=>ot.compare(e.label,t.label))}function Et(e=[],t=[],n=[],r=!0){let i=(e,t)=>e.some(e=>e.value===t),a=!1;r&&U.category&&!i(e,U.category)&&(U.category=``,a=!0),r&&U.city&&!i(t,U.city)&&(U.city=``,a=!0),r&&U.region&&!i(n,U.region)&&(U.region=``,a=!0),U.employeeBand&&!i(tt,U.employeeBand)&&(U.employeeBand=``,a=!0),U.foundedPeriod&&!i(nt,U.foundedPeriod)&&(U.foundedPeriod=``,a=!0),a&&q(`replace`)}function Dt(e,t){return t?Number.isInteger(e)?t===`iki-1999`?Number(e)<=1999:t===`2000-2009`?Number(e)>=2e3&&Number(e)<=2009:t===`2010-2019`?Number(e)>=2010&&Number(e)<=2019:t===`nuo-2020`?Number(e)>=2020:!1:!1:!0}function Ot(e){return ut(e.financial_verification_status??``)===`patikrinta`}function kt(e,t=!0){let n=ut(U.query);return e.filter(e=>{let r=!n||[e.trading_name,e.legal_name??``,e.source_identity,e.description_lt,...K(e.category_labels),...K(e.category_codes)].some(e=>ut(e??``).includes(n)),i=!t||!U.category||K(e.category_codes).includes(U.category),a=!t||!U.city||e.city===U.city,o=!t||!U.region||e.region===U.region,s=!U.employeeBand||e.employee_count_band?.trim()===U.employeeBand,c=Dt(e.founded_year,U.foundedPeriod),l=!U.registryCheckedOnly||Ot(e);return r&&i&&a&&o&&s&&c&&l})}function At(){return!!(U.employeeBand||U.foundedPeriod||U.registryCheckedOnly)}function jt(){U={query:``,category:``,city:``,region:``,employeeBand:``,foundedPeriod:``,registryCheckedOnly:!1}}function Mt(e){let t=e%100,n=e%10;return t>=11&&t<=19?`${e} gamintojų kandidatų`:n===1?`${e} gamintojas kandidatas`:n>=2&&n<=9?`${e} gamintojai kandidatai`:`${e} gamintojų kandidatų`}function Nt(){let e=(e,t)=>e.find(e=>e.value===t)?.label??t,t=[];return U.query&&t.push(`Paieška: „${U.query}“`),U.category&&t.push(`Kategorija: ${e(wt(H),U.category)}`),U.city&&t.push(`Miestas: ${e(Tt(H,`city`,`city`),U.city)}`),U.region&&t.push(`Regiono grupė: ${e(Tt(H,`region`,`region_label`),U.region)}`),U.employeeBand&&t.push(`Įmonės dydis: ${e(tt,U.employeeBand)}`),U.foundedPeriod&&t.push(`Įkurta: ${e(nt,U.foundedPeriod)}`),U.registryCheckedOnly&&t.push(`Registro duomenys patikrinti`),t}function Pt(e){let t=e.legal_name?.trim()??``;if(!t)return``;let n=e=>ut(e).replace(/\b(uab|ab|mb|všį|iį|įi|kib|tūb)\b/g,``).replace(/[^\p{L}\p{N}]+/gu,` `).trim(),r=n(t),i=n(e.trading_name);return!r||r===i||r.includes(i)||i.includes(r)?``:t}function Y(t){let n={directory:`Katalogas`,request:`Projekto užklausa`,guide:`Pirkėjo gidas`,policy:`Informacija`};return`
  ...
  `}function Ft(e){let t=new Set(K(e.category_codes));return t.has(`W`)?{slug:`spintos-ir-drabuzines-kaina`,label:`Spintų ir drabužinių kainos bei apimties klausimai`}:t.has(`K`)?{slug:`kvarcas-ar-akmuo-stalvirsiui`,label:`Stalviršio medžiagos ir apimties klausimai`}:t.has(`OC`)||t.has(`HR`)?{slug:`matavimas-ir-montavimas-kontrole`,label:`Matavimo ir montavimo kontrolinis sąrašas`}:t.has(`SW`)?{slug:`mdf-faneruote-masyvas-fasadai`,label:`Medžiagų ir apdailos klausimai`}:{slug:`matavimas-ir-montavimas-kontrole`,label:`Matavimo ir montavimo kontrolinis sąrašas`}}function It(e){let t=[],n=e.city?.trim(),r=n?ye(H).find(e=>e.city===n):void 0;r&&t.push({kind:`Miestas`,slug:r.slug,label:`Baldų gamintojų kandidatai: ${r.city}`});let i=new Set(K(e.category_codes));return he.forEach(e=>{i.has(e.code)&&t.push({kind:`Kategorija`,slug:e.slug,label:e.title})}),t}function Lt(e){let t=It(e);if(!t.length)return null;let n=document.createElement(`section`);return n.className=`profile-landings`,n.setAttribute(`aria-labelledby`,`profile-landings-title`),n.innerHTML=`
    <div class="section-heading">
      ...
      <ul class="profile-landing-links">
        ${t.map(e=>`
          <li>
            ...
            <a href="/baldai-pagal-uzsakyma/${e.slug}" data-internal-link="true">${W(e.label)} <span aria-hidden="true">→</span></a>
          </li>
        `).join(``)}
      </ul>
    </nav>
  `,n}function Rt(){let e=ye(H);return`
    <section class="landing-directory" aria-labelledby="landing-directory-title">
      ...
      <div class="landing-link-groups">
        <div>
          ...
          <ul>
            ${he.map(e=>`<li><a href="/baldai-pagal-uzsakyma/${e.slug}" data-internal-link="true">${W(e.title)}</a></li>`).join(``)}
          </ul>
        </div>
        ${e.length?`
          <div>
            ...
            <ul>
              ${e.map(e=>`<li><a href="/baldai-pagal-uzsakyma/${e.slug}" data-internal-link="true">Baldų gamintojų kandidatai: ${W(e.city)} (${e.count})</a></li>`).join(``)}
            </ul>
          </div>
        `:``}
      </div>
    </section>
  `}function Z(e,t,n,r,i){let a=document.createElement(`div`);a.className=`filter-field`;let o=document.createElement(`label`);o.htmlFor=e,o.textContent=t;let s=document.createElement(`div`);s.className=`select-wrap`;let c=document.createElement(`select`);c.id=e,c.name=e;let l=document.createElement(`option`);return l.value=``,l.textContent=n,c.append(l),r.forEach(e=>{let t=document.createElement(`option`);t.value=e.value,t.textContent=e.label,t.selected=e.value===i,c.append(t)}),s.append(c),a.append(o,s),a}function zt(e){let t=document.createElement(`div`);t.className=`filter-toggle`;let n=document.createElement(`input`);n.id=e,n.name=e,n.type=`checkbox`,n.checked=U.registryCheckedOnly;let r=document.createElement(`label`);r.htmlFor=e;let i=document.createElement(`strong`);i.textContent=`Tik patikrinti registro duomenys`;let a=document.createElement(`span`);return a.textContent=`Tai duomenų būsenos žyma, ne kokybės ar prieinamumo garantija.`,r.append(i,a),t.append(n,r),t}function Bt(e){let t=document.createElement(`div`);t.className=`filter-grid filter-grid--advanced`;let n=Z(`${e}-size-filter`,`Įmonės dydis`,`Visi darbuotojų skaičiai`,tt,U.employeeBand),r=Z(`${e}-founded-filter`,`Įkūrimo laikotarpis`,`Visi įkūrimo metai`,nt,U.foundedPeriod),i=zt(`${e}-registry-filter`);return t.append(n,r,i),{fields:t,employeeBandSelect:n.querySelector(`select`),foundedPeriodSelect:r.querySelector(`select`),registryToggle:i.querySelector(`input`)}}function Vt(e){if(!Ot(e))return null;let t=document.createElement(`p`);t.className=`registry-card-status`;let n=document.createElement(`span`);n.className=`registry-check-mark`,n.setAttribute(`aria-hidden`,`true`);let r=document.createElement(`strong`);r.textContent=`Registro duomenys patikrinti`,t.append(n,r);let i=mt(e.verified_at);if(i){let e=document.createTextNode(` · `),n=document.createElement(`time`);n.dateTime=i.iso,n.textContent=i.label,t.append(e,n)}return t}function Ht(e){let t=document.createElement(`article`);t.className=`manufacturer-card`;let n=document.createElement(`div`);n.className=`card-heading`;let r=document.createElement(`h3`);r.textContent=G(e.trading_name,`Pavadinimas nenurodytas`);let i=Pt(e);if(i){let e=document.createElement(`p`);e.className=`legal-name`,e.textContent=i,n.append(r,e)}else n.append(r);let a=document.createElement(`p`);a.className=`location-line`;let o=document.createElement(`strong`);o.textContent=G(e.city,`Miestas nenurodytas`);let s=document.createElement(`span`);s.setAttribute(`aria-hidden`,`true`),s.textContent=` · `;let c=document.createElement(`span`);c.textContent=`Regiono grupė: ${G(e.region_label,`nenurodyta`)}`,a.append(o,s,c);let l=document.createElement(`p`);l.className=e.description_lt?.trim()?`description`:`description description--fallback`,l.textContent=G(e.description_lt,`Trumpas aprašymas šaltiniuose nepateiktas.`);let u=document.createElement(`ul`);u.className=`category-list`,u.setAttribute(`aria-label`,`Gaminamų baldų kategorijos`);let d=K(e.category_labels);(d.length?d:[`Kategorijos šaltiniuose nenurodytos`]).forEach(e=>{let t=document.createElement(`li`);t.textContent=e,u.append(t)});let f=document.createElement(`a`);f.className=`profile-link`,f.href=`/gamintojas/${encodeURIComponent(e.slug)}${window.location.search}`,f.dataset.internalLink=`true`,f.textContent=`Peržiūrėti katalogo įrašą`;let p=document.createElement(`span`);p.setAttribute(`aria-hidden`,`true`),p.textContent=` →`,f.append(p),t.append(n);let m=Vt(e);return m&&t.append(m),t.append(a,l,u,f),t}function Ut(){B&&(B.innerHTML=`
    ${Y(`directory`)}
    <main>
    ...
    ${X()}
  `)}function Wt(){let e=document.querySelector(`#browse-content`);e&&(e.innerHTML=`
  ...
  `)}function Gt(){B&&(F({title:`Katalogas nepasiekiamas | Baldai pagal užsakymą Lietuvoje`,description:`Gamintojų katalogo duomenų šiuo metu nepavyko gauti.`,path:window.location.pathname,robots:`noindex, follow`}),B.innerHTML=`
    ${Y(`directory`)}
    ...
    ${X()}
  `,document.querySelector(`#retry-button`)?.addEventListener(`click`,()=>void bn()))}function Kt(){let e=document.querySelector(`#home-filter-controls`),t=document.querySelector(`#browse-content`);if(!e||!t)return;let n=wt(H),r=Tt(H,`city`,`city`),i=Tt(H,`region`,`region_label`);Et(n,r,i),e.replaceChildren(),t.replaceChildren();let a=document.createElement(`div`);a.className=`browse-controls`;let o=document.createElement(`form`);o.className=`filter-form`,o.setAttribute(`role`,`search`),o.setAttribute(`aria-label`,`Gamintojų katalogo paieška`);let s=document.createElement(`div`);s.className=`filter-field filter-field--search`;let c=document.createElement(`label`);c.htmlFor=`directory-search`,c.textContent=`Poreikis arba pavadinimas`;let l=document.createElement(`input`);l.id=`directory-search`,l.name=`paieska`,l.type=`search`,l.autocomplete=`off`,l.placeholder=`Pvz., virtuvės baldai arba įmonės pavadinimas`,l.value=U.query,s.append(c,l);let u=document.createElement(`div`);u.className=`filter-grid filter-grid--primary`,u.append(s,Z(`category-filter`,`Baldų kategorija`,`Visos kategorijos`,n,U.category),Z(`city-filter`,`Miestas`,`Visi miestai`,r,U.city));let d=Bt(`directory`),f=document.createElement(`details`);f.className=`advanced-filters`,f.open=!!(U.region||At());let p=document.createElement(`summary`),m=document.createElement(`span`);m.textContent=`Daugiau filtrų`;let h=document.createElement(`span`);h.className=`advanced-filter-status`,p.append(m,h);let g=document.createElement(`div`);g.className=`filter-grid filter-grid--advanced`,g.append(Z(`region-filter`,`Šaltinio regiono grupė`,`Visos regiono grupės`,i,U.region),...Array.from(d.fields.children)),f.append(p,g);let _=document.createElement(`div`);_.className=`filter-actions`;let v=document.createElement(`button`);v.className=`primary-button home-search-submit`,v.type=`submit`,v.textContent=`Rodyti gamintojus`;let y=document.createElement(`button`);y.className=`text-button`,y.id=`clear-filters`,y.type=`button`,y.textContent=`Išvalyti filtrus`,_.append(v,y),o.append(u,f,_),a.append(o);let b=document.createElement(`div`);b.className=`results-area`,b.id=`results-area`,e.append(a),t.append(b);let x=()=>{let e=[U.region,U.employeeBand,U.foundedPeriod].filter(Boolean).length+ +!!U.registryCheckedOnly;h.textContent=e?`${e} pasirinkta`:`Nebūtina`};x(),o.addEventListener(`submit`,e=>{e.preventDefault();let t=document.querySelector(`.result-count`);document.querySelector(`#gamintojai`)?.scrollIntoView({behavior:window.matchMedia(`(prefers-reduced-motion: reduce)`).matches?`auto`:`smooth`,block:`start`}),t?.focus({preventScroll:!0})}),l.addEventListener(`input`,()=>{U.query=l.value.trimStart(),q(`replace`),Q()}),document.querySelector(`#category-filter`)?.addEventListener(`change`,e=>{U.category=e.currentTarget.value,q(`push`),Q()}),document.querySelector(`#city-filter`)?.addEventListener(`change`,e=>{U.city=e.currentTarget.value,q(`push`),Q()}),document.querySelector(`#region-filter`)?.addEventListener(`change`,e=>{U.region=e.currentTarget.value,q(`push`),x(),Q()}),d.employeeBandSelect.addEventListener(`change`,()=>{U.employeeBand=d.employeeBandSelect.value,q(`push`),x(),Q()}),d.foundedPeriodSelect.addEventListener(`change`,()=>{U.foundedPeriod=d.foundedPeriodSelect.value,q(`push`),x(),Q()}),d.registryToggle.addEventListener(`change`,()=>{U.registryCheckedOnly=d.registryToggle.checked,q(`push`),x(),Q()}),y.addEventListener(`click`,()=>{jt(),q(`push`),Kt(),document.querySelector(`#directory-search`)?.focus()}),Q()}function Q(){let e=document.querySelector(`#results-area`);if(!e)return;window.location.pathname===`/`&&vn();let t=kt(H),n=!!(U.query||U.category||U.city||U.region||At());e.replaceChildren();let r=document.createElement(`div`);r.className=`result-header`;let i=document.createElement(`p`);if(i.className=`result-count`,i.tabIndex=-1,i.setAttribute(`role`,`status`),i.setAttribute(`aria-live`,`polite`),i.textContent=n?`Rodoma įrašų: ${t.length}. Iš viso kataloge: ${H.length}.`:`Kataloge – ${Mt(H.length)}.`,r.append(i),n){let e=document.createElement(`ul`);e.className=`active-filters`,e.setAttribute(`aria-label`,`Aktyvūs paieškos kriterijai`),Nt().forEach(t=>{let n=document.createElement(`li`);n.textContent=t,e.append(n)}),r.append(e)}if(e.append(r),t.length===0){let t=document.createElement(`div`);t.className=`message-state`;let n=document.createElement(`p`);n.className=`state-label`,n.textContent=`Rezultatų nėra`;let r=document.createElement(`h3`);r.textContent=`Pagal šiuos kriterijus įrašų nerasta`;let i=document.createElement(`p`);i.textContent=`Pakeiskite paieškos žodį, pasirinkite platesnę vietovę arba išvalykite filtrus.`;let a=document.createElement(`button`);a.className=`primary-button`,a.type=`button`,a.textContent=`Išvalyti visus kriterijus`,a.addEventListener(`click`,()=>{jt(),q(`push`),Kt()}),t.append(n,r,i,a),e.append(t);return}let a=document.createElement(`div`);a.className=`manufacturer-list`,t.forEach(e=>a.append(Ht(e))),e.append(a)}function $(e,t){let n=document.createElement(`div`),r=document.createElement(`dt`);r.textContent=e;let i=document.createElement(`dd`);return i.textContent=t,n.append(r,i),n}function qt(e,t){let n=document.createElement(`div`),r=document.createElement(`dt`);r.textContent=`Įmonės dydžio signalas`;let i=document.createElement(`dd`),a=document.createElement(`strong`);a.textContent=dt(t);let o=document.createElement(`p`);o.className=`fact-explanation`,o.textContent=`Tai viešame įmonės įraše nurodyta darbuotojų skaičiaus grupė. Ji neparodo darbų kokybės, dabartinio užimtumo ar galimybės priimti jūsų projektą.`,i.append(a,o);let s=K(e.public_details_source_urls).filter(e=>e.includes(`rekvizitai.vz.lt`));if(s.length){let e=document.createElement(`ul`);e.className=`fact-source-list`,s.forEach((t,n)=>{let r=document.createElement(`li`),i=document.createElement(`a`);i.href=t,i.target=`_blank`,i.rel=`noopener noreferrer`,i.textContent=s.length===1?`Atverti viešą darbuotojų skaičiaus šaltinį`:`Atverti viešą šaltinį ${n+1}`,r.append(i),e.append(r)}),i.append(e)}return n.append(r,i),n}function Jt(e,t){let n=document.createElement(`div`),r=document.createElement(`dt`);r.textContent=e;let i=document.createElement(`dd`),a=t?.trim();if(a){let e=document.createElement(`a`);e.href=a,e.target=`_blank`,e.rel=`noopener noreferrer`,e.textContent=a,i.append(e)}else i.textContent=`Viešuose šaltiniuose nenurodyta.`,i.className=`unknown-value`;return n.append(r,i),n}function Yt(e,t){let n=document.createElement(`div`),r=document.createElement(`dt`);r.textContent=e;let i=document.createElement(`dd`),a=document.createElement(`a`);return a.href=`tel:${t.replace(/[^+\d]/g,``).replace(/(?!^)\+/g,``)}`,a.textContent=t,i.append(a),n.append(r,i),n}function Xt(e){let t=[],n=e.company_code?.trim(),r=e.public_phone?.trim(),i=e.street_address?.trim(),a=e.postcode?.trim(),o=e.employee_count_band?.trim();if(n&&t.push($(`Įmonės kodas`,n)),i&&t.push($(`Registracijos adresas`,i)),a&&t.push($(`Pašto kodas`,a)),Number.isInteger(e.founded_year)&&t.push($(`Įkurta`,String(e.founded_year))),o&&t.push(qt(e,o)),r&&t.push(Yt(`Viešas telefono numeris`,r)),!t.length)return null;let s=document.createElement(`section`);s.className=`profile-details profile-public-details`,s.setAttribute(`aria-labelledby`,`profile-public-details-title`),s.innerHTML=`
    <div class="section-heading">
  ...
  `}function an(e,t){let n=document.querySelector(`#landing-filter-controls`),r=document.querySelector(`#landing-manufacturer-list`);if(!n||!r)return;Et([],[],[],!1),n.replaceChildren();let i=document.createElement(`form`);i.className=`filter-form landing-filter-form`,i.setAttribute(`role`,`search`),i.addEventListener(`submit`,e=>e.preventDefault());let a=document.createElement(`div`);a.className=`filter-field filter-field--search`;let o=document.createElement(`label`);o.htmlFor=`landing-search`,o.textContent=`Ieškoti šiame sąraše`;let s=document.createElement(`input`);s.id=`landing-search`,s.name=`paieska`,s.type=`search`,s.autocomplete=`off`,s.placeholder=`Pavadinimas, aprašymas ar kategorija`,s.value=U.query,a.append(o,s);let c=Bt(`landing`),l=document.createElement(`div`);l.className=`filter-grid filter-grid--landing`,l.append(a,...Array.from(c.fields.children));let u=document.createElement(`div`);u.className=`filter-actions`;let d=document.createElement(`p`);d.className=`landing-filter-count`,d.setAttribute(`role`,`status`),d.setAttribute(`aria-live`,`polite`);let f=document.createElement(`button`);f.className=`text-button`,f.type=`button`,f.textContent=`Išvalyti šio sąrašo filtrus`,u.append(d,f),i.append(l,u),n.append(i);let p=()=>{jt(),q(`push`),s.value=``,c.employeeBandSelect.value=``,c.foundedPeriodSelect.value=``,c.registryToggle.checked=!1,m(),s.focus()},m=()=>{t();let n=kt(e,!1),i=!!(U.query||At());if(d.textContent=i?`Rodoma įrašų: ${n.length}. Iš viso šiame sąraše: ${e.length}.`:`Šiame sąraše – ${Mt(e.length)}.`,r.replaceChildren(),!n.length){let e=document.createElement(`div`);e.className=`message-state landing-empty-state`;let t=document.createElement(`h3`);t.textContent=`Pagal šiuos kriterijus įrašų nerasta`;let n=document.createElement(`p`);n.textContent=`Pasirinkite platesnį įmonės dydį ar įkūrimo laikotarpį, pakeiskite paiešką arba išvalykite filtrus.`;let i=document.createElement(`button`);i.className=`primary-button`,i.type=`button`,i.textContent=`Išvalyti šio sąrašo filtrus`,i.addEventListener(`click`,p),e.append(t,n,i),r.append(e);return}n.forEach(e=>r.append(Ht(e)))};s.addEventListener(`input`,()=>{U.query=s.value.trimStart(),q(`replace`),m()}),c.employeeBandSelect.addEventListener(`change`,()=>{U.employeeBand=c.employeeBandSelect.value,q(`push`),m()}),c.foundedPeriodSelect.addEventListener(`change`,()=>{U.foundedPeriod=c.foundedPeriodSelect.value,q(`push`),m()}),c.registryToggle.addEventListener(`change`,()=>{U.registryCheckedOnly=c.registryToggle.checked,q(`push`),m()}),f.addEventListener(`click`,p),m()}function on(e){if(!B)return;let t=he.find(t=>t.slug===e),n=ye(H),r=n.find(t=>t.slug===e);if(!t&&!r){ln(`Paieškos puslapis nerastas`,`Tokio kategorijos ar miesto puslapio nėra. Grįžkite į katalogą ir naudokite paiešką arba filtrus.`);return}let i=t?H.filter(e=>K(e.category_codes).includes(t.code)):H.filter(e=>e.city===r?.city),a=t?t.title:`Baldų gamintojų kandidatai: ${r?.city}`,o=t?t.intro:`Čia pateikiami ${i.length} nepatvirtinti baldų gamintojų kandidatai, kurių viešo šaltinio įraše kaip bazės miestas ar vietovė nurodytas ${r?.city}.`,s=t?t.buyer_note:`Šis sąrašas nepatvirtina, kad kandidatai aptarnauja visą miestą ar aplinkinį regioną. Matavimo, pristatymo ir montavimo vietas patikrinkite tiesiogiai.`,c=t?tn(t):nn(r?.city??`šiame mieste`),l=`/baldai-pagal-uzsakyma/${e}`,u=t?`${t.title}: ${i.length} viešais šaltiniais paremti nepatvirtinti Lietuvos gamintojų kandidatai, miestai ir atrankos gairės.`:`${r?.city}: ${i.length} viešuose šaltiniuose šiame mieste registruoti baldų gamintojų kandidatai. Sąrašas nėra paslaugų teritorijos ar kokybės garantija.`,d=()=>{F({title:`${a} | Gamintojų katalogas`,description:u,path:l,robots:window.location.search?`noindex, follow`:`index, follow`,structuredData:[P([{name:`Gamintojų katalogas`,path:`/`},{name:a,path:l}]),we(c),Te(i)]})};d();let f=t?n.map(e=>({...e,count:i.filter(t=>t.city===e.city).length})).filter(e=>e.count>0).sort((e,t)=>t.count-e.count||ot.compare(e.city,t.city)):[],p=r?he.map(e=>({...e,count:i.filter(t=>K(t.category_codes).includes(e.code)).length})).filter(e=>e.count>0):[];B.innerHTML=`
    ...
    <main class="landing-main">
      ...
      <section class="landing-related" aria-labelledby="related-title">
        ...
        <ul class="landing-related-links">
          ${(t?f:p).map(e=>`<li><a href="/baldai-pagal-uzsakyma/${e.slug}" data-internal-link="true">${W(`city`in e?e.city:e.title)} <span>(${e.count})</span></a></li>`).join(``)}
        </ul>
      </section>
    ...
    ${X()}
  `,an(i,d)}function sn(){B&&$e({root:B,renderHeader:Y,renderFooter:X,manufacturers:H})}function cn(e){B&&(F({title:e.title,description:e.description,path:e.path,structuredData:[P([{name:`Gamintojų katalogas`,path:`/`},{name:e.heading,path:e.path}])]}),B.innerHTML=`
    ${Y(`policy`)}
    ...
    ${X()}
  `)}function ln(e,t){B&&(F({title:`${e} | Baldai pagal užsakymą Lietuvoje`,description:t,path:window.location.pathname,robots:`noindex, follow`}),B.innerHTML=`
    ${Y(`directory`)}
    <main class="profile-main">
  ...
  `;let t=document.querySelector(`#profile-main`);if(!t)return;let n=document.createElement(`a`);if(n.className=`back-link`,n.href=`/${window.location.search}`,n.dataset.internalLink=`true`,n.textContent=`← Grįžti į gamintojų katalogą`,!e){F({title:`Gamintojas nerastas | Baldai pagal užsakymą Lietuvoje`,description:`Gamintojo įrašas šiame viešų šaltinių kataloge nerastas.`,path:window.location.pathname,robots:`noindex, follow`});let e=document.createElement(`section`);e.className=`message-state profile-state not-found-state`,e.innerHTML=`
    ...
    `,t.append(n,e);return}let r=G(e.trading_name,`Gamintojo pavadinimas nenurodytas`),i=`/gamintojas/${e.slug}`;F({title:`${r} | Baldų gamintojo įrašas`,description:`${r}: viešais šaltiniais paremtas, nepatvirtintas gamintojo kandidato įrašas su vieta, kategorijomis ir šaltinių nuorodomis.`,path:i,robots:window.location.search?`noindex, follow`:`index, follow`,type:`profile`,structuredData:[P([{name:`Gamintojų katalogas`,path:`/`},{name:r,path:i}]),Ce(e)]});let a=document.createElement(`article`);a.className=`profile-sheet`;let o=document.createElement(`header`);o.className=`profile-hero`;let s=document.createElement(`div`);s.className=`profile-heading-group`;let c=document.createElement(`p`);c.className=`record-status`,c.textContent=`Nepatvirtintas viešų šaltinių įrašas`;let l=document.createElement(`h1`);l.textContent=r;let u=document.createElement(`p`);u.className=`profile-identity`,u.textContent=G(e.source_identity,`Šaltinyje pateikta tapatybė nenurodyta.`),s.append(c,l,u);let d=document.createElement(`div`);d.className=`profile-actions`;let f=document.createElement(`a`);f.className=`primary-button`,f.href=`/gauti-pasiulymus?gamintojas=${encodeURIComponent(e.slug)}`,f.dataset.internalLink=`true`,f.textContent=`Įtraukti į projekto užklausą`;let p=document.createElement(`a`);p.className=`profile-guide-link`,p.href=`/gidas`,p.dataset.internalLink=`true`,p.textContent=`Prieš kreipdamiesi peržiūrėkite pirkėjo gidą →`;let m=Ft(e),h=document.createElement(`a`);h.className=`profile-guide-link`,h.href=`/gidas/${m.slug}`,h.dataset.internalLink=`true`,h.textContent=`${m.label} →`,d.append(f,p,h),o.append(s,d);let g=document.createElement(`div`);g.className=`profile-note`,g.innerHTML=`
  ...
  `;let y=document.createElement(`dl`);y.className=`profile-facts`,y.append($(`Viešas / prekinis pavadinimas`,r),$(`Juridinis pavadinimas`,G(e.legal_name,`Viešame šaltinyje juridinis pavadinimas nenurodytas.`)),$(`Šaltinyje pateikta tapatybė`,G(e.source_identity)),$(`Vietovė šaltinyje`,G(e.location)),$(`Miestas ar vietovė`,G(e.city)),$(`Šaltinio regiono grupė`,G(e.region_label)),$(`Kategorijos`,K(e.category_labels).join(`, `)||`Kategorijos viešuose šaltiniuose nenurodytos.`),$(`Aprašymas`,G(e.description_lt,`Trumpas aprašymas šaltiniuose nepateiktas.`)),$(`Šaltinyje aprašyta veiklos apimtis`,G(e.scope_evidence,`Papildomas veiklos apimties aprašymas šaltinyje nepateiktas.`)),Jt(`Svetainė`,e.website),Jt(`Viešai nurodytas kontaktinis adresas`,e.public_contact_url)),_.append(v,y);let b=Zt(e),x=Xt(e),S=Lt(e);a.append(o),b&&a.append(b),a.append(g,_),x&&a.append(x),S&&a.append(S),a.append($t(e),Qt(e),en(e)),t.append(n,a)}function dn(){if(!B)return;let e=[{question:`Ar katalogo įrašas yra gamintojo rekomendacija?`,answer:`Ne. Katalogas pateikia viešuose šaltiniuose rastus nepatvirtintus kandidatus ir palieka tapatybės, apimties bei pasiūlymo patikrą pirkėjui.`},{question:`Ar galima lyginti tik galutinę pasiūlymo kainą?`,answer:`Ne. Kainą reikia lyginti kartu su medžiagomis, furnitūra, matavimu, projektavimu, pristatymu, montavimu, terminais ir aiškiai nurodytomis išimtimis.`},{question:`Kaip patikrinti siūlomą gamybos terminą?`,answer:`Paprašykite grafiko etapais ir raštu patvirtinkite, nuo kokio įvykio terminas skaičiuojamas, kokios jo prielaidos ir kas nutinka pasikeitus apimčiai.`}];F({title:`Pirkėjo gidas | Baldai pagal užsakymą Lietuvoje`,description:`Lietuviški pirkėjo gidai apie baldų gamintojo pasirinkimą, realistiškas kainų nuorodas, projekto etapus, medžiagas, sutartį, avansą ir garantiją.`,path:`/gidas`,structuredData:[P([{name:`Gamintojų katalogas`,path:`/`},{name:`Pirkėjo gidas`,path:`/gidas`}]),we(e)]});let t=e=>`
  ...
  `;B.innerHTML=`
    ...
    ${X()}
  `}function fn(e,t){if(!B)return;let n=`/gidas/${e.slug}`;F({title:`${e.title} | Pirkėjo gidas`,description:e.summary,path:n,type:`article`,structuredData:[P([{name:`Gamintojų katalogas`,path:`/`},{name:`Pirkėjo gidas`,path:`/gidas`},{name:e.title,path:n}])]}),B.innerHTML=`
    ${Y(`guide`)}
  ...
  `)}function gn(){B&&(F({title:`Gido straipsnis nerastas | Pirkėjo gidas`,description:`Prašomas pirkėjo gido straipsnis nerastas.`,path:window.location.pathname,robots:`noindex, follow`}),B.innerHTML=`
    ${Y(`guide`)}
    <main class="article-main">
      <a class="back-link" href="/gidas" data-internal-link="true">← Grįžti į pirkėjo gidą</a>
    ...
    ${X()}
  `)}function _n(){let e=J();if(e===`/gidas`){dn();return}let t=decodeURIComponent(e.split(`/`).filter(Boolean)[1]??``);if(V.find(e=>e.slug===t)?.featured){if(B?.querySelector(`.guide-article`))return;window.location.replace(`${e}/`);return}if(t===`trumpasis-sarasas`){pn();return}if(t===`uzklausa-ir-pasiulymas`){mn();return}if(t===`terminai`){hn();return}gn()}function vn(){F({title:`Baldai pagal užsakymą Lietuvoje | Gamintojų katalogas`,description:`Viešais šaltiniais paremtas nepatvirtintų Lietuvos nestandartinių baldų gamintojų kandidatų katalogas su paieška pagal kategoriją ir vietą.`,path:`/`,robots:window.location.search?`noindex, follow`:`index, follow`})}function yn(){let e=xt();if(e){cn(e);return}if(gt()){if(!B)return;Ie({root:B,renderHeader:Y,renderFooter:X});return}if(bt()){if(!B)return;Ke({root:B,renderHeader:Y,renderFooter:X});return}if(yt()){sn();return}if(_t()){_n();return}if(window.location.pathname.startsWith(`/gamintojas/`)){let e=decodeURIComponent(window.location.pathname.split(`/`).filter(Boolean)[1]??``);un(H.find(t=>t.slug===e));return}if(vt()){on(decodeURIComponent(window.location.pathname.split(`/`).filter(Boolean)[1]??``));return}if(window.location.pathname!==`/`){ln(`Tokio puslapio nėra`,`Patikrinkite adresą arba grįžkite į gamintojų katalogą.`);return}vn(),Ut(),Kt()}async function bn(){if(!ct){ct=!0,window.location.pathname===`/`?B?.hasChildNodes()||(Ut(),Wt()):window.location.pathname.startsWith(`/gamintojas/`)?B&&!B.hasChildNodes()&&(B.innerHTML=`
        ${Y(`directory`)}
        <main class="profile-main">
          <div class="loading-state" role="status" aria-live="polite">
            <span class="loading-mark" aria-hidden="true"><span></span><span></span><span></span></span>
        ...
        ${X()}
      `):yt()&&B&&!B.hasChildNodes()&&(B.innerHTML=`
        ${Y(`request`)}
        ...
        ${X()}
      `);try{H=await Ct(),lt=!0,U=ht(),yn()}catch(e){console.error(`Nepavyko gauti gamintojų katalogo.`,e),Gt()}finally{ct=!1}}}function xn(){if(U=ht(),_t()||St()||bt()){yn();return}if(lt){yn();return}bn()}function Sn(e,t,n=!1){let r=e.querySelector(`.navigation-toggle`);if(!r)return;t?e.dataset.menuOpen=`true`:delete e.dataset.menuOpen,r.setAttribute(`aria-expanded`,String(t));let i=r.querySelector(`.navigation-current`)?.textContent?.trim()??``;r.setAttribute(`aria-label`,`${t?`Užverti`:`Atverti`} pagrindinį meniu. Dabartinis skyrius: ${i}`),n&&r.focus()}document.addEventListener(`click`,e=>{let t=e.target;if(!(t instanceof Element))return;let n=t.closest(`.navigation-toggle`);if(n){let e=n.closest(`.site-header`);if(!e)return;Sn(e,n.getAttribute(`aria-expanded`)!==`true`);return}let r=document.querySelector(`.site-header[data-menu-open="true"]`);r&&!r.contains(t)&&Sn(r,!1)}),document.addEventListener(`keydown`,e=>{if(e.key!==`Escape`)return;let t=document.querySelector(`.site-header[data-menu-open="true"]`);t&&(e.preventDefault(),Sn(t,!1,!0))}),document.addEventListener(`click`,e=>{let t=e.target;if(!(t instanceof Element))return;let n=t.closest(`a[data-internal-link="true"]`);if(!n||e.defaultPrevented||e.button!==0||e.metaKey||e.ctrlKey||e.shiftKey||e.altKey)return;let r=new URL(n.href,window.location.origin);r.origin===window.location.origin&&(e.preventDefault(),window.history.pushState({},``,`${r.pathname}${r.search}${r.hash}`),xn(),window.scrollTo({top:0,behavior:`auto`}))}),window.addEventListener(`popstate`,xn),_t()||St()||bt()?yn():bn();