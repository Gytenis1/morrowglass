...
Šalių parašų vietos ir data: [įrašykite]`}];function Be(){return Object.fromEntries(ze.map(e=>[e.id,e.value]))}function Ve(){let e=Be();try{let t=localStorage.getItem(Pe);if(!t)return e;let n=JSON.parse(t);ze.forEach(t=>{typeof n[t.id]==`string`&&(e[t.id]=n[t.id])})}catch{return e}return e}function He({root:e,renderHeader:t,renderFooter:n}){P({title:`Baldų pirkimo sutarties šablonas | Pirkėjo įrankiai`,description:`Naršyklėje redaguojamas ir spausdinamas B2C baldų pirkimo sutarties struktūros šablonas su aiškiu teisinės konsultacijos ribojimu.`,path:Ae,type:`article`,structuredData:[N([{name:`Gamintojų katalogas`,path:`/`},{name:`Pirkėjo gidas`,path:`/gidas`},{name:`Baldų pirkimo sutarties šablonas`,path:Ae}])]});let r=Ve();e.innerHTML=`
    ${t(`guide`)}
    ...
    <fieldset class="offer-editor" data-offer-id="${F(e.id)}">
      ...
      <div class="offer-primary-fields">
        <div class="form-field">
          ...
          <input id="provider-${e.id}" data-field="provider" type="text" maxlength="160" value="${F(e.provider)}" placeholder="Pasiūlyme nurodytas pavadinimas">
        </div>
        <div class="form-field">
          <label for="total-${e.id}">Bendra suma su PVM, €</label>
          <input id="total-${e.id}" data-field="totalInclVat" type="number" min="0" step="0.01" inputmode="decimal" value="${F(e.totalInclVat)}" placeholder="0,00">
        </div>
      </div>
      <div class="offer-criteria">
        ${I.map(t=>`
          <section class="offer-criterion" data-criterion="${t.id}">
            <div class="form-field">
              <label for="evidence-${e.id}-${t.id}">${F(t.evidenceLabel)}</label>
              <textarea id="evidence-${e.id}-${t.id}" data-evidence="${t.id}" rows="3" maxlength="3000" placeholder="${F(t.placeholder)}">${F(e.evidence[t.id])}</textarea>
            </div>
      ...
      <div class="form-field">
        ...
        <textarea id="notes-${e.id}" data-field="notes" rows="3" maxlength="3000" placeholder="Klausimai, versijos data, ką dar reikia patikslinti">${F(e.notes)}</textarea>
      </div>
    </fieldset>
  `}function Ye(e){return{weights:Object.fromEntries(I.map(t=>{let n=e.querySelector(`[data-weight="${t.id}"]`);return[t.id,Math.min(5,Math.max(0,Number(n?.value)||0))]})),offers:Array.from(e.querySelectorAll(`[data-offer-id]`)).map((e,t)=>{let n=We(t);return n.id=e.dataset.offerId??n.id,n.provider=e.querySelector(`[data-field="provider"]`)?.value.trim()??``,n.totalInclVat=e.querySelector(`[data-field="totalInclVat"]`)?.value??``,n.notes=e.querySelector(`[data-field="notes"]`)?.value.trim()??``,I.forEach(t=>{n.evidence[t.id]=e.querySelector(`[data-evidence="${t.id}"]`)?.value.trim()??``,n.ratings[t.id]=Number(e.querySelector(`[data-rating="${t.id}"]`)?.value)||0}),n})}}function Xe(e,t){try{localStorage.setItem(Fe,JSON.stringify(e)),t&&(t.textContent=`Palyginimo duomenys išsaugoti tik šioje naršyklėje.`)}catch{t&&(t.textContent=`Naršyklė neleido išsaugoti. Duomenys liks tik iki puslapio uždarymo.`)}}function Ze(e,t){let n=e.querySelector(`#comparison-results`);if(!n)return;let r=I.reduce((e,n)=>e+t.weights[n.id],0);if(r===0){n.innerHTML=`<div class="form-status form-status--error" role="alert">Bent vienam kriterijui nustatykite didesnį nei 0 svorį.</div>`,n.focus();return}let i=t.offers.map((e,n)=>{let i=I.reduce((n,r)=>n+e.ratings[r.id]*t.weights[r.id],0),a=r*3,o=a?i/a*100:0,s=[];return e.provider||s.push({type:`missing`,label:`trūksta tiekėjo pavadinimo`}),(!e.totalInclVat||Number(e.totalInclVat)<0)&&s.push({type:`missing`,label:`trūksta bendros sumos su PVM`}),I.forEach(t=>{e.evidence[t.id]||s.push({type:`missing`,label:`trūksta: ${t.evidenceLabel.toLocaleLowerCase(`lt-LT`)}`}),e.ratings[t.id]===1&&s.push({type:`nonComparable`,label:`nepalyginama: ${t.label.toLocaleLowerCase(`lt-LT`)}`})}),{offer:e,index:n,numerator:i,denominator:a,score:o,issues:s}}).sort((e,t)=>t.score-e.score||e.index-t.index),a=new Map;i.forEach(e=>{let t=e.score.toFixed(6);a.set(t,(a.get(t)??0)+1)});let o=new Intl.NumberFormat(`lt-LT`,{style:`currency`,currency:`EUR`});n.innerHTML=`
    ...
    <div class="comparison-result-list">
      ${i.map(e=>{let n=e.offer.provider||`Pasiūlymas ${e.index+1}`,r=(a.get(e.score.toFixed(6))??0)>1,i=e.offer.totalInclVat&&Number.isFinite(Number(e.offer.totalInclVat))?o.format(Number(e.offer.totalInclVat)):`Suma nepateikta`,s=e.issues.filter(e=>e.type===`missing`),c=e.issues.filter(e=>e.type===`nonComparable`);return`
          <article class="comparison-result" aria-labelledby="result-${e.offer.id}">
            <header>
              <div>
                <h3 id="result-${e.offer.id}">${F(n)}</h3>
                <p>${F(i)} · skaičiavimas ${e.numerator} / ${e.denominator}</p>
              </div>
            ...
            ${s.length||c.length?`
              <div class="comparison-issues">
                ${s.length?`<p><strong>Trūksta:</strong> ${F(s.map(e=>e.label.replace(/^trūksta:?\s*/i,``)).join(`; `))}.</p>`:``}
                ${c.length?`<p><strong>Nepalyginama:</strong> ${F(c.map(e=>e.label.replace(/^nepalyginama:?\s*/i,``)).join(`; `))}.</p>`:``}
              </div>
            ...
            <dl class="comparison-evidence-list">
              ${I.map(n=>`
                <div class="${e.offer.evidence[n.id]?e.offer.ratings[n.id]===1?`is-non-comparable`:``:`is-missing`}">
                  <dt>${F(n.label)} <span>Svoris ${t.weights[n.id]} · vertinimas ${e.offer.ratings[n.id]}</span></dt>
                  <dd>${F(e.offer.evidence[n.id]||`Duomenų nepateikta.`)}</dd>
                </div>
              `).join(``)}
              <div><dt>Pastabos</dt><dd>${F(e.offer.notes||`Pastabų nepateikta.`)}</dd></div>
            </dl>
          </article>
        `}).join(``)}
    </div>
  `,n.focus()}function Qe({root:e,renderHeader:t,renderFooter:n}){P({title:`Palyginti baldų pasiūlymus | Pirkėjo įrankiai`,description:`Deterministinis 2–5 baldų pasiūlymų palyginimas pagal pirkėjo svorius, aiškią formulę, įrodymus, trūkstamus ir nepalyginamus laukus.`,path:je,structuredData:[N([{name:`Gamintojų katalogas`,path:`/`},{name:`Pirkėjo įrankiai`,path:`/gidas`},{name:`Pasiūlymų palyginimas`,path:je}])]});let r=Ke(),i=()=>{e.innerHTML=`
      ${t(`guide`)}
      <main class="buyer-tool-main comparison-tool-main">
        ...
        <section class="comparison-offers" aria-labelledby="offers-title">
          ...
          <div class="offer-editor-list">
            ${r.offers.map((e,t)=>Je(e,t,r.offers.length>2)).join(``)}
          </div>
      ...
      ${n()}
    `;let a=e.querySelector(`#comparison-save-status`),o=0,s=()=>{r=Ye(e),Xe(r,a)};e.querySelectorAll(`input, textarea, select`).forEach(e=>{e.addEventListener(`input`,()=>{a&&(a.textContent=`Saugoma…`),window.clearTimeout(o),o=window.setTimeout(s,220)}),e.addEventListener(`change`,s)}),e.querySelector(`#add-offer`)?.addEventListener(`click`,()=>{r=Ye(e),!(r.offers.length>=5)&&(r.offers.push(We(r.offers.length)),Xe(r),i())}),e.querySelectorAll(`.remove-offer`).forEach(t=>{t.addEventListener(`click`,()=>{if(r=Ye(e),r.offers.length<=2)return;let n=t.closest(`[data-offer-id]`);r.offers=r.offers.filter(e=>e.id!==n?.dataset.offerId),Xe(r),i()})}),e.querySelector(`#calculate-comparison`)?.addEventListener(`click`,()=>{r=Ye(e),Xe(r,a),Ze(e,r)}),e.querySelector(`#reset-comparison`)?.addEventListener(`click`,()=>{if(window.confirm(`Išvalyti visus šioje naršyklėje išsaugotus pasiūlymus ir svorius?`)){try{localStorage.removeItem(Fe)}catch{}r=Ge(),i()}})};i()}var $e=[`Virtuvės baldai`,`Spintos ar įmontuojami baldai`,`Miegamojo ar vonios baldai`,`Biuro ar komerciniai baldai`,`HoReCa ar prekybos baldai`,`Minkšti baldai`,`Medžio masyvo ar kiti nestandartiniai baldai`,`Kitas baldų projektas`],et=[`Idėja ir poreikių formavimas`,`Yra preliminarūs matmenys`,`Yra patalpos planas ar projektas`,`Parinktos pagrindinės medžiagos`,`Objektas parengtas galutiniam matavimui`,`Reikia pakeisti ar papildyti esamą projektą`];function tt(e,t){return`<option value="">${F(t)}</option>${e.map(e=>`<option value="${F(e)}">${F(e)}</option>`).join(``)}`}function L(e){return`<p class="field-error" id="error-${e}" aria-live="polite"></p>`}function R(e,t,n=``){let r=e.querySelector(`[name="${t}"]`),i=e.querySelector(`#error-${t.replaceAll(`_`,`-`)}`);r&&(r.setAttribute(`aria-invalid`,n?`true`:`false`),r.setCustomValidity(n)),i&&(i.textContent=n)}function nt(e){e.querySelectorAll(`.field-error`).forEach(e=>{e.textContent=``}),e.querySelectorAll(`[aria-invalid]`).forEach(e=>{e.setAttribute(`aria-invalid`,`false`),e.setCustomValidity(``)});let t=e.querySelector(`#rfq-error-summary`);t&&(t.hidden=!0,t.innerHTML=``)}function rt(e,t){nt(e);let n=[],r=(e,r,i)=>{let a=t.elements.namedItem(e);(!a||a.value.trim().length<r)&&n.push({field:e,message:`${i}: įrašykite bent ${r} ženkl${r===1?`ą`:`ų`}.`})};r(`full_name`,2,`Vardas ir pavardė`),r(`category`,2,`Projekto kategorija`),r(`municipality`,2,`Savivaldybė`),r(`service_region`,2,`Paslaugos vieta`),r(`project_stage`,2,`Projekto etapas`),r(`project_scope`,10,`Projekto apimtis`),r(`dimensions_room_count`,1,`Matmenys ir patalpų skaičius`),r(`materials_requirements`,1,`Medžiagų reikalavimai`),r(`installation_access_constraints`,1,`Montavimo ir patekimo sąlygos`),t.elements.namedItem(`email`)?.validity.valid||n.push({field:`email`,message:`Nurodykite galiojantį el. pašto adresą.`});let i=t.elements.namedItem(`phone`);i?.value&&!/^[0-9+().\-\s]+$/.test(i.value.trim())&&n.push({field:`phone`,message:`Telefono numeryje naudokite tik skaitmenis ir įprastus skyrybos ženklus.`});let a=t.elements.namedItem(`budget_min`),o=t.elements.namedItem(`budget_max`),s=Number(a?.value),c=Number(o?.value);if((!a?.value||!Number.isFinite(s)||s<0)&&n.push({field:`budget_min`,message:`Nurodykite neneigiamą mažiausią biudžetą.`}),(!o?.value||!Number.isFinite(c)||c<0)&&n.push({field:`budget_max`,message:`Nurodykite neneigiamą didžiausią biudžetą.`}),a?.value&&o?.value&&Number.isFinite(s)&&Number.isFinite(c)&&c<s&&n.push({field:`budget_max`,message:`Didžiausias biudžetas negali būti mažesnis už mažiausią.`}),t.elements.namedItem(`desired_completion_date`)?.value||n.push({field:`desired_completion_date`,message:`Nurodykite pageidaujamą užbaigimo datą.`}),e.querySelectorAll(`[name="preferred_shortlist"]:checked`).length>8&&n.push({field:`preferred_shortlist`,message:`Pasirinkite ne daugiau kaip 8 gamintojų kandidatus.`}),n.forEach(t=>R(e,t.field,t.message)),!n.length&&t.checkValidity())return!0;let l=e.querySelector(`#rfq-error-summary`);return l&&(l.hidden=!1,l.innerHTML=`<strong>Patikrinkite formą:</strong><ul>${n.map(e=>`<li>${F(e.message)}</li>`).join(``)}</ul>`,l.focus()),n.length||t.reportValidity(),!1}function it(e,t){if(!t||typeof t!=`object`)return!1;let n=t.data;if(!n||typeof n!=`object`)return!1;let r=!1;return Object.entries(n).forEach(([t,n])=>{if(!n||typeof n!=`object`)return;let i=n.message;typeof i==`string`&&(R(e,t,i),r=!0)}),r}function at({root:e,renderHeader:t,renderFooter:n,manufacturers:r=[]}){let i=new URLSearchParams(window.location.search),a=i.get(`saltinis`)===`kainos-skaiciuokle`,o=a?{category:i.get(`category`)?.trim().slice(0,160)??``,municipality:i.get(`municipality`)?.trim().slice(0,160)??``,service_region:i.get(`service_region`)?.trim().slice(0,160)??``,project_stage:i.get(`project_stage`)?.trim().slice(0,160)??``,project_scope:i.get(`project_scope`)?.trim().slice(0,3e3)??``,dimensions_room_count:i.get(`dimensions_room_count`)?.trim().slice(0,1200)??``,materials_requirements:i.get(`materials_requirements`)?.trim().slice(0,3e3)??``,budget_min:i.get(`budget_min`)?.trim().slice(0,20)??``,budget_max:i.get(`budget_max`)?.trim().slice(0,20)??``}:void 0;P({title:`Pateikite saugią baldų projekto užklausą | Baldai pagal užsakymą Lietuvoje`,description:`Struktūruota baldų projekto pasiūlymo užklausa operatoriaus peržiūrai, be automatinio siuntimo gamintojams ir su atskiru patvirtinimu prieš išsiuntimą.`,path:Me,robots:window.location.search?`noindex, follow`:`index, follow`,structuredData:[N([{name:`Gamintojų katalogas`,path:`/`},{name:`Projekto užklausa`,path:Me}])]});let s=i.get(`gamintojas`)?.trim()??``,c=s?r.find(e=>e.slug===s):void 0,l=!!(s&&!c),u=(c?[c,...r.filter(e=>e.slug!==c.slug)]:r).map(e=>{let t=[e.city?.trim(),(e.category_labels??[]).slice(0,2).join(`, `)].filter(Boolean).join(` · `);return`
      <label class="manufacturer-choice">
        <input type="checkbox" name="preferred_shortlist" value="${F(e.slug)}"${e.slug===c?.slug?` checked`:``}>
        <span><strong>${F(e.trading_name)}</strong><small>${F(t||`Vieta ir kategorijos viešame įraše nenurodytos`)}</small></span>
      </label>
    `}).join(``);e.innerHTML=`
    ${t(`request`)}
    <main class="request-main rfq-main">
      ...
      <div class="request-layout">
        <section class="request-form-section" aria-labelledby="request-form-title">
          ...
          ${a?`<div class="rfq-prefill-note" role="status"><strong>Skaičiuoklės duomenys perkelti.</strong><span>Patikrinkite apimtį, medžiagas ir biudžeto intervalą, tada užpildykite likusius laukus.</span></div>`:``}
          ...
          <form class="buyer-request-form rfq-form" id="rfq-form" novalidate>
            ...
            <div class="form-field">
              ...
              <div class="select-wrap"><select id="category" name="category" required aria-describedby="error-category">${tt($e,`Pasirinkite kategoriją`)}</select></div>
              ${L(`category`)}
            </div>
            <div class="form-field">
            ...
            <div class="form-field form-field--wide">
              ...
              <div class="select-wrap"><select id="project-stage" name="project_stage" required aria-describedby="error-project-stage">${tt(et,`Pasirinkite dabartinį etapą`)}</select></div>
              ${L(`project-stage`)}
            </div>
            <div class="form-field form-field--wide">
            ...
            <div class="form-field">
              ...
              ${L(`installation-access-constraints`)}
            </div>
            ...
            <fieldset class="manufacturer-fieldset form-field--wide">
              ...
              ${l?`<p class="selection-notice" role="status">Nuorodoje nurodyto gamintojo kataloge nerasta. Galite pasirinkti kitą kandidatą.</p>`:``}
              <div class="manufacturer-picker">
                <div class="manufacturer-picker-toolbar">
                  ...
                  <p id="manufacturer-selection-count" aria-live="polite">${c?`Pasirinktas 1 kandidatas iš 8`:`Kandidatų nepasirinkta`}</p>
                </div>
        ...
        <aside class="request-guidance" aria-labelledby="request-guidance-title">
          ...
          <a href="${je}" data-internal-link="true">Kaip vėliau palyginti pasiūlymus →</a>
          <a href="${Ae}" data-internal-link="true">Atverti sutarties struktūros šabloną →</a>
        </aside>
      </div>
    </main>
    ${n()}
  `;let d=e.querySelector(`#rfq-form`),f=e.querySelector(`#manufacturer-search`),p=e.querySelector(`#manufacturer-choice-list`),m=e.querySelector(`#manufacturer-empty`),h=e.querySelector(`#manufacturer-selection-count`),g=e.querySelector(`#rfq-status`),_=d?.querySelector(`button[type="submit"]`);if(!d||!f||!p||!m||!h||!g||!_)return;o&&Object.entries(o).forEach(([e,t])=>{let n=d.elements.namedItem(e);(n instanceof HTMLInputElement||n instanceof HTMLTextAreaElement||n instanceof HTMLSelectElement)&&(n.value=t)});let v=Array.from(p.querySelectorAll(`[name="preferred_shortlist"]`)),y=t=>{let n=v.filter(e=>e.checked);n.length>8&&t?(t.checked=!1,n=v.filter(e=>e.checked),R(e,`preferred_shortlist`,`Galima pasirinkti ne daugiau kaip 8 gamintojų kandidatus.`)):R(e,`preferred_shortlist`),h.textContent=n.length===0?`Kandidatų nepasirinkta`:n.length===1?`Pasirinktas 1 kandidatas iš 8`:`Pasirinkta kandidatų: ${n.length} iš 8`};v.forEach(e=>e.addEventListener(`change`,()=>y(e))),f.addEventListener(`input`,()=>{let e=f.value.trim().toLocaleLowerCase(`lt-LT`),t=0;p.querySelectorAll(`.manufacturer-choice`).forEach(n=>{let r=!e||(n.textContent??``).toLocaleLowerCase(`lt-LT`).includes(e);n.hidden=!r,r&&(t+=1)}),m.hidden=t>0}),d.querySelectorAll(`input, textarea, select`).forEach(t=>{t.name!==`preferred_shortlist`&&(t.setAttribute(`aria-invalid`,`false`),t.addEventListener(`input`,()=>R(e,t.name)),t.addEventListener(`blur`,()=>{t.validity.valid||R(e,t.name,t.validationMessage||`Patikrinkite šį lauką.`)}))}),d.addEventListener(`submit`,async t=>{if(t.preventDefault(),!rt(e,d))return;let n=new FormData(d),r={full_name:String(n.get(`full_name`)??``).trim(),email:String(n.get(`email`)??``).trim(),phone:String(n.get(`phone`)??``).trim(),category:String(n.get(`category`)??``).trim(),municipality:String(n.get(`municipality`)??``).trim(),service_region:String(n.get(`service_region`)??``).trim(),project_stage:String(n.get(`project_stage`)??``).trim(),project_scope:String(n.get(`project_scope`)??``).trim(),dimensions_room_count:String(n.get(`dimensions_room_count`)??``).trim(),materials_requirements:String(n.get(`materials_requirements`)??``).trim(),budget_min:Number(n.get(`budget_min`)),budget_max:Number(n.get(`budget_max`)),desired_completion_date:String(n.get(`desired_completion_date`)??``),installation_access_constraints:String(n.get(`installation_access_constraints`)??``).trim(),preferred_shortlist:v.filter(e=>e.checked).map(e=>e.value)};_.disabled=!0,_.setAttribute(`aria-busy`,`true`),_.textContent=`Pateikiama…`,g.className=`form-status`,g.textContent=`Pasiūlymo užklausa kuriama operatoriaus peržiūrai. Gamintojai nekontaktuojami.`;try{let t=await fetch(`${fe.replace(/\/+$/,``)}/api/public/rfqs`,{method:`POST`,credentials:`omit`,headers:{"Content-Type":`application/json`},body:JSON.stringify(r)}),n=await t.json().catch(()=>null);if(!t.ok){if(it(e,n)){let t=e.querySelector(`#rfq-error-summary`);if(t){let n=Array.from(e.querySelectorAll(`.field-error`)).map(e=>e.textContent).filter(Boolean);t.hidden=!1,t.innerHTML=`<strong>Serveris paprašė pataisyti:</strong><ul>${n.map(e=>`<li>${F(e??``)}</li>`).join(``)}</ul>`,t.focus()}}throw Error(n?.message||`Pasiūlymo užklausos pateikti nepavyko.`)}if(!n?.reference)throw Error(`Serveris negrąžino pasiūlymo užklausos numerio.`);let i=e.querySelector(`#rfq-success`),a=e.querySelector(`#rfq-reference`);a&&(a.textContent=n.reference),i&&(i.hidden=!1,i.focus()),g.className=`form-status form-status--success`,g.textContent=`Pasiūlymo užklausa ${n.reference} gauta operatoriaus peržiūrai. Gamintojai nebuvo kontaktuoti automatiškai.`,_.textContent=`Pasiūlymo užklausa pateikta`,_.removeAttribute(`aria-busy`),d.querySelectorAll(`input, textarea, select`).forEach(e=>{e.disabled=!0});return}catch(e){console.error(`Nepavyko pateikti pasiūlymo užklausos.`,e),g.className=`form-status form-status--error`,g.setAttribute(`role`,`alert`),g.textContent=`Pasiūlymo užklausos pateikti nepavyko. Įvesti duomenys liko formoje. Patikrinkite pažymėtus laukus ir interneto ryšį, tada bandykite dar kartą.`,g.focus()}finally{(!_.disabled||_.textContent!==`Pasiūlymo užklausa pateikta`)&&(_.disabled=!1,_.removeAttribute(`aria-busy`),_.textContent=`Pateikti pasiūlymo užklausą operatoriaus peržiūrai`)}})}var ot=`/baldu-kainos-skaiciuokle`,st=[300,548,800],z=[{value:`kitchen`,label:`Virtuvė`,requestCategory:`Virtuvės baldai`,measureLabel:`Preliminarus baldų ilgis metrais`,measureShort:`bėginiai metrai`,min:1,max:20,step:.5,initial:4,included:`virtuvės korpusų, fasadų ir bazinės furnitūros gamybos apimtis`},{value:`wardrobe`,label:`Spinta ar įmontuojami baldai`,requestCategory:`Spintos ar įmontuojami baldai`,measureLabel:`Preliminarus baldų plotis metrais`,measureShort:`pločio metrai`,min:1,max:15,step:.5,initial:3,included:`korpusų, durų ir bazinio vidaus suplanavimo gamybos apimtis`},{value:`bed-bath`,label:`Miegamojo ar vonios baldai`,requestCategory:`Miegamojo ar vonios baldai`,measureLabel:`Preliminarus atskirų baldų ar modulių skaičius`,measureShort:`moduliai`,min:1,max:30,step:1,initial:3,included:`nurodyto baldų ar modulių kiekio korpusų ir fasadų gamybos apimtis`},{value:`office`,label:`Biuro ar komerciniai baldai`,requestCategory:`Biuro ar komerciniai baldai`,measureLabel:`Preliminarus darbo vietų ar baldų modulių skaičius`,measureShort:`moduliai`,min:1,max:50,step:1,initial:6,included:`nurodyto darbo vietų ar baldų modulių kiekio gamybos apimtis`},{value:`other`,label:`Kitas nestandartinis projektas`,requestCategory:`Kitas baldų projektas`,measureLabel:`Preliminarus atskirų baldų ar modulių skaičius`,measureShort:`moduliai`,min:1,max:30,step:1,initial:3,included:`nurodyto nestandartinių baldų ar modulių kiekio gamybos apimtis`}],ct=[{value:`practical`,label:`Praktiškas`,detail:`LMDP korpusai ir paprastesni lygūs fasadai`,factors:[.9,.95,1]},{value:`balanced`,label:`Subalansuotas`,detail:`LMDP korpusai ir MDF ar panašaus sudėtingumo fasadai`,factors:[1,1,1]},{value:`detailed`,label:`Sudėtingesnė apdaila`,detail:`Dažytas ar frezuotas MDF, daugiau matomų apdailos detalių`,factors:[1.05,1.15,1.3]},{value:`natural`,label:`Natūrali mediena`,detail:`Faneruoti ar medžio masyvo fasadai ir jų apdaila`,factors:[1.1,1.3,1.55]}],lt=[{value:`none`,label:`Netaikoma arba atskira sąmata`,detail:`Stalviršio pasirinkimas baldų intervalo nekeičia`,factors:[1,1,1]},{value:`laminate`,label:`Laminuotas stalviršis`,detail:`Pažymimas projekto apimtyje; atskiro tarifo skaičiuoklė neprideda`,factors:[1,1,1.04]},{value:`compact-wood`,label:`Kompaktinis laminatas arba medis`,detail:`Didesnė apimties nežinomybė parodoma platesne viršutine riba`,factors:[1,1.04,1.1]},{value:`quartz`,label:`Kompozicinis kvarcas`,detail:`Šablonavimas, išpjovos, sujungimai ir montavimas turi būti įkainoti atskirai`,factors:[1,1.05,1.15]},{value:`stone`,label:`Natūralus akmuo`,detail:`Konkreti plokštė ir montavimo apimtis turi būti įkainoti atskirai`,factors:[1,1.08,1.2]}],ut=[`Vilnius`,`Kaunas`,`Klaipėda`,`Šiauliai`,`Panevėžys`,`Alytus`,`Marijampolė`,`Kitas miestas ar rajonas`];function dt(e){return e.replaceAll(`&`,`&amp;`).replaceAll(`<`,`&lt;`).replaceAll(`>`,`&gt;`).replaceAll(`"`,`&quot;`).replaceAll(`'`,`&#039;`)}function ft(e,t,n=!1){return`<option value="${dt(e)}"${n?` selected`:``}>${dt(t)}</option>`}function pt(e){return Math.max(100,Math.round(e/50)*50)}function B(e){return new Intl.NumberFormat(`lt-LT`,{style:`currency`,currency:`EUR`,maximumFractionDigits:0}).format(e)}function mt(e,t,n){let r=st.map((r,i)=>pt(r*e*t.factors[i]*n.factors[i]));return{low:r[0],typical:r[1],high:Math.max(r[2],r[1])}}function ht(e,t,n,r,i,a){return`/gauti-pasiulymus?${new URLSearchParams({saltinis:`kainos-skaiciuokle`,category:e.requestCategory,municipality:i,service_region:i,project_stage:`Yra preliminarūs matmenys`,project_scope:`${e.label}. Preliminari apimtis: ${t.toLocaleString(`lt-LT`)} ${e.measureShort}. Skaičiuoklės orientyras: ${B(a.low)}–${B(a.high)}, tipinė modelio reikšmė ${B(a.typical)}. Tai nėra pasiūlymas.`,dimensions_room_count:`${t.toLocaleString(`lt-LT`)} ${e.measureShort}; galutinius matmenis reikia patikrinti objekte.`,materials_requirements:`${n.label}: ${n.detail}. Stalviršis: ${r.label}. ${r.detail}.`,budget_min:String(a.low),budget_max:String(a.high)}).toString()}`}function gt({root:e,renderHeader:t,renderFooter:n}){P({title:`Baldų kainos skaičiuoklė | Orientacinė sąmata Lietuvoje`,description:`Orientacinė nestandartinių baldų kainos skaičiuoklė pagal baldų tipą, apimtį, fasadų lygį, stalviršį ir miestą, paremta Baldininkai.org gidais.`,path:ot,structuredData:[N([{name:`Gamintojų katalogas`,path:`/`},{name:`Pirkėjo gidas`,path:`/gidas`},{name:`Baldų kainos skaičiuoklė`,path:ot}]),{"@context":`https://schema.org`,"@type":`WebApplication`,name:`Baldų kainos skaičiuoklė`,url:`${j}${ot}/`,applicationCategory:`FinanceApplication`,operatingSystem:`Any`,inLanguage:`lt-LT`,description:`Orientacinis nestandartinių baldų kainos intervalo skaičiavimas pagal projekto apimtį ir pasirinktus sprendinius.`}]}),e.innerHTML=`
    ${t(`guide`)}
    <main class="buyer-tool-main estimator-main">
      <nav class="tool-navigation" aria-label="Pirkėjo įrankiai">
        <strong>Pirkėjo įrankiai</strong>
        <div>
          <a href="${ot}" data-internal-link="true" aria-current="page">Kainos skaičiuoklė</a>
        ...
        <form class="estimator-form" id="price-estimator-form">
          <fieldset>
            ...
            <div class="estimator-field">
              ...
              <div class="select-wrap"><select id="estimator-type" name="type">${z.map((e,t)=>ft(e.value,e.label,t===0)).join(``)}</select></div>
            </div>
            <div class="estimator-field">
              <label for="estimator-size" id="estimator-size-label">${z[0].measureLabel}</label>
              <div class="estimator-number-row">
                <input id="estimator-size" name="size" type="number" inputmode="decimal" min="${z[0].min}" max="${z[0].max}" step="${z[0].step}" value="${z[0].initial}" aria-describedby="estimator-size-hint">
                <span id="estimator-size-unit">${z[0].measureShort}</span>
              </div>
            ...
            <div class="estimator-field">
              ...
              <datalist id="estimator-city-options">${ut.map(e=>ft(e,e)).join(``)}</datalist>
          ...
          <fieldset>
            ...
            <div class="estimator-field">
              ...
              <div class="select-wrap"><select id="estimator-tier" name="tier">${ct.map((e,t)=>ft(e.value,e.label,t===1)).join(``)}</select></div>
              <p class="field-hint" id="estimator-tier-detail">${ct[1].detail}</p>
            </div>
            <div class="estimator-field">
              <label for="estimator-worktop">Stalviršio pasirinkimas</label>
              <div class="select-wrap"><select id="estimator-worktop" name="worktop">${lt.map(e=>ft(e.value,e.label)).join(``)}</select></div>
              <p class="field-hint" id="estimator-worktop-detail">${lt[0].detail}</p>
            </div>
          </fieldset>
        </form>
  ...
  `;let r=e.querySelector(`#price-estimator-form`),i=e.querySelector(`#estimator-type`),a=e.querySelector(`#estimator-size`),o=e.querySelector(`#estimator-size-label`),s=e.querySelector(`#estimator-size-unit`),c=e.querySelector(`#estimator-tier`),l=e.querySelector(`#estimator-tier-detail`),u=e.querySelector(`#estimator-worktop`),d=e.querySelector(`#estimator-worktop-detail`),f=e.querySelector(`#estimator-city`),p=e.querySelector(`#estimate-low`),m=e.querySelector(`#estimate-typical`),h=e.querySelector(`#estimate-high`),g=e.querySelector(`#estimate-summary`),_=e.querySelector(`#estimate-included`),v=e.querySelector(`#estimate-rfq-link`);if(!r||!i||!a||!o||!s||!c||!l||!u||!d||!f||!p||!m||!h||!g||!_||!v)return;let y=(e=!1)=>{let t=z.find(e=>e.value===i.value)??z[0],n=ct.find(e=>e.value===c.value)??ct[1],r=lt.find(e=>e.value===u.value)??lt[0];e&&(a.value=String(t.initial)),a.min=String(t.min),a.max=String(t.max),a.step=String(t.step),o.textContent=t.measureLabel,s.textContent=t.measureShort,l.textContent=n.detail,d.textContent=r.detail;let y=Number(a.value),b=Number.isFinite(y)?Math.min(t.max,Math.max(t.min,y)):t.initial,x=mt(b,n,r);p.textContent=B(x.low),m.textContent=B(x.typical),h.textContent=B(x.high);let S=f.value.trim()||`Miestas nenurodytas`;g.textContent=`${t.label}: ${b.toLocaleString(`lt-LT`)} ${t.measureShort}, medžiagų lygis – ${n.label.toLocaleLowerCase(`lt-LT`)}, stalviršis – ${r.label.toLocaleLowerCase(`lt-LT`)}, ${S}.`,_.innerHTML=`
    ...
    `,v.href=ht(t,b,n,r,S,x)};i.addEventListener(`change`,()=>y(!0)),a.addEventListener(`blur`,()=>{let e=z.find(e=>e.value===i.value)??z[0],t=Number(a.value),n=Number.isFinite(t)?Math.min(e.max,Math.max(e.min,t)):e.initial,r=Math.round((n-e.min)/e.step)*e.step+e.min;a.value=String(Number(r.toFixed(+(e.step<1)))),y()}),r.addEventListener(`input`,()=>y()),r.addEventListener(`change`,()=>y()),y()}var _t=50,vt=[{value:`0`,label:`0 darbuotojų`},{value:`1-9`,label:`1–9 darbuotojai`},{value:`10-49`,label:`10–49 darbuotojai`},{value:`50-249`,label:`50–249 darbuotojai`},{value:`250+`,label:`250 ir daugiau darbuotojų`}],yt=[{value:`iki-1999`,label:`Iki 1999 m.`},{value:`2000-2009`,label:`2000–2009 m.`},{value:`2010-2019`,label:`2010–2019 m.`},{value:`nuo-2020`,label:`2020 m. ir vėliau`}],bt=40,xt=2e3,St=[`Virtuvės baldai`,`Spintos ar įmontuojami baldai`,`Miegamojo ar vonios baldai`,`Biuro ar komerciniai baldai`,`Minkšti baldai`,`Medžio masyvo ar kiti nestandartiniai baldai`,`Kitas projektas`],Ct=new Intl.Collator(`lt`,{sensitivity:`base`}),V=document.querySelector(`#app`),H=[{slug:`trumpasis-sarasas`,title:`Kaip sudaryti pagrįstą trumpąjį sąrašą`,summary:`Atrankos seka, patikrinami kriterijai ir klausimai prieš priimant pasiūlymą.`,readingLabel:`Atranka ir patikra`},{slug:`uzklausa-ir-pasiulymas`,title:`Kaip parengti užklausą ir palyginti pasiūlymus`,summary:`Ką aprašyti, kad gamintojai vertintų tą pačią apimtį, ir kas dažniausiai keičia kainą.`,readingLabel:`Užklausa ir apimtis`},{slug:`terminai`,title:`Kaip prašyti realistiško darbų grafiko`,summary:`Terminą lemiantys kintamieji, etapai ir klausimai, padedantys valdyti neapibrėžtumą.`,readingLabel:`Terminai ir eiga`},{slug:`kaip-pasirinkti-baldu-gamintoja`,title:`Kaip pasirinkti ir palyginti baldų gamintoją`,summary:`Ką paklausti, kokius įspėjamuosius ženklus pastebėti ir kaip atsargiai skaityti nepatvirtintus katalogo įrašus.`,readingLabel:`Atranka ir patikra`,featured:!0},{slug:`virtuves-baldu-kainos`,title:`Virtuvės baldų kainos: ribos ir kainą keičiantys sprendimai`,summary:`Dvi aiškiai atskirtos viešų šaltinių nuorodos, jų datos ir praktinis sąrašas, kas keičia individualaus projekto kainą.`,readingLabel:`Kaina ir apimtis`,featured:!0},{slug:`virtuves-ir-imontuojamu-baldu-projekto-eiga`,title:`Virtuvės ir įmontuojamų baldų projekto eiga`,summary:`Tipinė etapų seka nuo matavimo iki montavimo ir kontrolinis sąrašas sprendimams, kurie veikia grafiką.`,readingLabel:`Projekto eiga`,featured:!0},{slug:`medziagos-sutartis-avansas-garantija`,title:`Medžiagos, sutartis, avansas ir garantija: ką aptarti`,summary:`LMDP ir MDF, faneruotės, masyvo, stalviršių, furnitūros, briaunų, sutarties ir garantinio aptarnavimo klausimai.`,readingLabel:`Dokumentai ir atsakomybės`,featured:!0},{slug:`spintos-ir-drabuzines-kaina`,title:`Spintos ir drabužinės kaina: ką apibrėžti prieš lyginant pasiūlymus`,summary:`Apimtis, vidaus įranga ir montavimo sąlygos, kurios padeda palyginti pasiūlymus.`,readingLabel:`Spintos ir drabužinės`,featured:!0,buyerIntent:!0},{slug:`mdf-faneruote-masyvas-fasadai`,title:`MDF, faneruotė ar masyvas fasadams: klausimai prieš pasirenkant`,summary:`Fasadų specifikacijos, pavyzdžiai, priežiūra ir kompromisai.`,readingLabel:`Fasadai ir medžiagos`,featured:!0,buyerIntent:!0},{slug:`kvarcas-ar-akmuo-stalvirsiui`,title:`Kvarcas ar natūralus akmuo stalviršiui: apimtis, priežiūra ir klausimai`,summary:`Šablonavimo, išpjovų, sujungimų ir montavimo kontrolinis sąrašas.`,readingLabel:`Stalviršiai`,featured:!0,buyerIntent:!0},{slug:`matavimas-ir-montavimas-kontrole`,title:`Galutinis matavimas ir montavimo diena: kontrolinis sąrašas`,summary:`Objekto parengtis, dokumentai ir priėmimo patikra.`,readingLabel:`Matavimas ir montavimas`,featured:!0,buyerIntent:!0},{slug:`baldu-defektai-ir-garantinis-aptarnavimas`,title:`Baldų defektai ir garantinis aptarnavimas: kaip fiksuoti ir sekti`,summary:`Dokumentavimas, pranešimas ir sutartos korekcijos sekimas.`,readingLabel:`Defektai ir aptarnavimas`,featured:!0,buyerIntent:!0},{slug:`mazo-buto-irengimas-pagal-uzsakyma`,title:`Mažo buto įrengimas pagal užsakymą: prioritetai ir užklausos sąrašas`,summary:`Saugojimo, judėjimo, matavimo ir montavimo prioritetai.`,readingLabel:`Mažas butas`,featured:!0,buyerIntent:!0}],wt=[{path:`/privatumas`,title:`Privatumo pranešimas | Baldai pagal užsakymą Lietuvoje`,heading:`Privatumo pranešimas`,description:`Kaip GG Ventures UAB tvarko Baldininkai.org katalogo užklausų, atsiliepimų ir įrašų pataisymo ar atstovavimo formų asmens duomenis.`,summary:`Šiame pranešime paaiškiname, kokius asmens duomenis gauname per katalogo formas, kam juos naudojame, kiek laiko saugome ir kokias teises turite.`,content:`
      ...
      <section><h2>Duomenys ir kontaktas</h2><p>Prašymo informaciją naudojame tik katalogo peržiūrai, ryšiui ir galimiems teisiniams poreikiams pagal <a href="/privatumas" data-internal-link="true">privatumo pranešimą</a>. Tvarką administruoja <strong>GG Ventures UAB</strong>, įmonės kodas <strong>305442420</strong>; kontaktinis el. paštas <a href="mailto:info@baldininkai.org">info@baldininkai.org</a>.</p></section>`}],U=[],W=Nt(),Tt=!1,Et=!1;function Dt(e){return e.trim().toLocaleLowerCase(`lt-LT`)}function G(e){return e.replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`).replace(/"/g,`&quot;`).replace(/'/g,`&#039;`)}function K(e,t=`Viešuose šaltiniuose nenurodyta.`){return e?.trim()||t}function Ot(e){return{0:`Viešame darbuotojų skaičiaus įraše – 0 darbuotojų`,"1-9":`Labai maža komanda – 1–9 darbuotojai`,"10-49":`Nedidelė įmonė – 10–49 darbuotojai`,"50-249":`Didesnė įmonė – 50–249 darbuotojai`,"250+":`Didelė įmonė – 250 ar daugiau darbuotojų`}[e]??`${e} darbuotojų (viešo šaltinio grupė)`}function kt(e){let t=e%100,n=e%10;return t>=11&&t<=19||n===0?`${e} patvirtintų atsiliepimų`:n===1?`${e} patvirtintas atsiliepimas`:`${e} patvirtinti atsiliepimai`}function At(e){let t=e?new Date(e):null;return!t||Number.isNaN(t.getTime())?null:{iso:t.toISOString(),label:new Intl.DateTimeFormat(`lt-LT`,{year:`numeric`,month:`long`,day:`numeric`}).format(t)}}function jt(e){let t=e?.trim()??``,n=/^(\d{4})-(\d{2})-(\d{2})$/.exec(t);if(!n)return null;let r=Number(n[1]),i=Number(n[2]),a=Number(n[3]),o=new Date(Date.UTC(r,i-1,a));return o.getUTCFullYear()!==r||o.getUTCMonth()!==i-1||o.getUTCDate()!==a?null:{iso:t,label:new Intl.DateTimeFormat(`lt-LT`,{year:`numeric`,month:`long`,day:`numeric`,timeZone:`UTC`}).format(o)}}function Mt(e){let t=e?.trim()??``,n=/^(\d{4}-\d{2}-\d{2})/.exec(t)?.[1];if(!n)return null;let r=jt(n);return r?{iso:t,label:r.label}:null}function q(e){return Array.isArray(e)?e.filter(e=>typeof e==`string`&&e.trim().length>0):[]}function Nt(){let e=new URLSearchParams(window.location.search);return{query:e.get(`q`)?.trim()??``,category:e.get(`kategorija`)??``,city:e.get(`miestas`)??``,region:e.get(`regionas`)??``,employeeBand:e.get(`dydis`)??``,foundedPeriod:e.get(`ikurta`)??``,registryCheckedOnly:e.get(`registras`)===`patikrinta`}}function J(e){let t=new URLSearchParams;W.query&&t.set(`q`,W.query),W.category&&t.set(`kategorija`,W.category),W.city&&t.set(`miestas`,W.city),W.region&&t.set(`regionas`,W.region),W.employeeBand&&t.set(`dydis`,W.employeeBand),W.foundedPeriod&&t.set(`ikurta`,W.foundedPeriod),W.registryCheckedOnly&&t.set(`registras`,`patikrinta`);let n=t.toString(),r=`${window.location.pathname}${n?`?${n}`:``}`;r!==`${window.location.pathname}${window.location.search}`&&window.history[e===`push`?`pushState`:`replaceState`]({},``,r)}function Y(e=window.location.pathname){return e.replace(/\/index\.html$/,``).replace(/\/+$/,``)||`/`}function Pt(e=window.location.pathname){return Y(e)===`/gidas/baldu-pirkimo-sutarties-sablonas`}function Ft(e=window.location.pathname){let t=Y(e);return t===`/gidas`||t.startsWith(`/gidas/`)}function It(e=window.location.pathname){return Y(e).startsWith(`/baldai-pagal-uzsakyma/`)}function Lt(e=window.location.pathname){return Y(e)===`/gauti-pasiulymus`}function Rt(e=window.location.pathname){return Y(e)===`/palyginti-pasiulymus`}function zt(e=window.location.pathname){return Y(e)===ot}function Bt(e=window.location.pathname){let t=e.replace(/\/+$/,``)||`/`;return wt.find(e=>e.path===t)}function Vt(e=window.location.pathname){return!!Bt(e)}async function Ht(){let e=[],t=1,n=1;do{let r=await de.collection(`manufacturers`).getList(t,_t,{sort:`trading_name`,requestKey:`manufacturers-page-${t}`});e.push(...r.items),n=r.totalPages,t+=1}while(t<=n);return e.sort((e,t)=>Ct.compare(e.trading_name,t.trading_name))}function Ut(e){let t=new Map;return e.forEach(e=>{q(e.category_codes).forEach((n,r)=>{let i=q(e.category_labels)[r];n&&i&&t.set(n,i)})}),Array.from(t,([e,t])=>({value:e,label:t})).sort((e,t)=>Ct.compare(e.label,t.label))}function Wt(e,t,n){let r=new Map;return e.forEach(e=>{let i=e[t]?.trim(),a=e[n]?.trim();i&&a&&r.set(i,a)}),Array.from(r,([e,t])=>({value:e,label:t})).sort((e,t)=>Ct.compare(e.label,t.label))}function Gt(e=[],t=[],n=[],r=!0){let i=(e,t)=>e.some(e=>e.value===t),a=!1;r&&W.category&&!i(e,W.category)&&(W.category=``,a=!0),r&&W.city&&!i(t,W.city)&&(W.city=``,a=!0),r&&W.region&&!i(n,W.region)&&(W.region=``,a=!0),W.employeeBand&&!i(vt,W.employeeBand)&&(W.employeeBand=``,a=!0),W.foundedPeriod&&!i(yt,W.foundedPeriod)&&(W.foundedPeriod=``,a=!0),a&&J(`replace`)}function Kt(e,t){return t?Number.isInteger(e)?t===`iki-1999`?Number(e)<=1999:t===`2000-2009`?Number(e)>=2e3&&Number(e)<=2009:t===`2010-2019`?Number(e)>=2010&&Number(e)<=2019:t===`nuo-2020`?Number(e)>=2020:!1:!1:!0}function qt(e){return Dt(e.financial_verification_status??``)===`patikrinta`}function Jt(e,t=!0){let n=Dt(W.query);return e.filter(e=>{let r=!n||[e.trading_name,e.legal_name??``,e.source_identity,e.description_lt,...q(e.category_labels),...q(e.category_codes)].some(e=>Dt(e??``).includes(n)),i=!t||!W.category||q(e.category_codes).includes(W.category),a=!t||!W.city||e.city===W.city,o=!t||!W.region||e.region===W.region,s=!W.employeeBand||e.employee_count_band?.trim()===W.employeeBand,c=Kt(e.founded_year,W.foundedPeriod),l=!W.registryCheckedOnly||qt(e);return r&&i&&a&&o&&s&&c&&l})}function Yt(){return!!(W.employeeBand||W.foundedPeriod||W.registryCheckedOnly)}function Xt(){W={query:``,category:``,city:``,region:``,employeeBand:``,foundedPeriod:``,registryCheckedOnly:!1}}function Zt(e){let t=e%100,n=e%10;return t>=11&&t<=19?`${e} gamintojų kandidatų`:n===1?`${e} gamintojas kandidatas`:n>=2&&n<=9?`${e} gamintojai kandidatai`:`${e} gamintojų kandidatų`}function Qt(){let e=(e,t)=>e.find(e=>e.value===t)?.label??t,t=[];return W.query&&t.push(`Paieška: „${W.query}“`),W.category&&t.push(`Kategorija: ${e(Ut(U),W.category)}`),W.city&&t.push(`Miestas: ${e(Wt(U,`city`,`city`),W.city)}`),W.region&&t.push(`Regiono grupė: ${e(Wt(U,`region`,`region_label`),W.region)}`),W.employeeBand&&t.push(`Įmonės dydis: ${e(vt,W.employeeBand)}`),W.foundedPeriod&&t.push(`Įkurta: ${e(yt,W.foundedPeriod)}`),W.registryCheckedOnly&&t.push(`Registro duomenys patikrinti`),t}function $t(e){let t=e.legal_name?.trim()??``;if(!t)return``;let n=e=>Dt(e).replace(/\b(uab|ab|mb|všį|iį|įi|kib|tūb)\b/g,``).replace(/[^\p{L}\p{N}]+/gu,` `).trim(),r=n(t),i=n(e.trading_name);return!r||r===i||r.includes(i)||i.includes(r)?``:t}function X(t){let n={directory:`Katalogas`,request:`Projekto užklausa`,guide:`Pirkėjo gidas`,policy:`Informacija`};return`
  ...
  `}function en(e){let t=new Set(q(e.category_codes));return t.has(`W`)?{slug:`spintos-ir-drabuzines-kaina`,label:`Spintų ir drabužinių kainos bei apimties klausimai`}:t.has(`K`)?{slug:`kvarcas-ar-akmuo-stalvirsiui`,label:`Stalviršio medžiagos ir apimties klausimai`}:t.has(`OC`)||t.has(`HR`)?{slug:`matavimas-ir-montavimas-kontrole`,label:`Matavimo ir montavimo kontrolinis sąrašas`}:t.has(`SW`)?{slug:`mdf-faneruote-masyvas-fasadai`,label:`Medžiagų ir apdailos klausimai`}:{slug:`matavimas-ir-montavimas-kontrole`,label:`Matavimo ir montavimo kontrolinis sąrašas`}}function tn(e){let t=[],n=e.city?.trim(),r=n?Ce(U).find(e=>e.city===n):void 0;r&&t.push({kind:`Miestas`,slug:r.slug,label:`Baldų gamintojų kandidatai: ${r.city}`});let i=new Set(q(e.category_codes));return _e.forEach(e=>{i.has(e.code)&&t.push({kind:`Kategorija`,slug:e.slug,label:e.title})}),t}function nn(e){let t=tn(e);if(!t.length)return null;let n=document.createElement(`section`);return n.className=`profile-landings`,n.setAttribute(`aria-labelledby`,`profile-landings-title`),n.innerHTML=`
    <div class="section-heading">
  ...
  `,n}function rn(){let e=Ce(U);return`
    <section class="landing-directory" aria-labelledby="landing-directory-title">
      ...
      <div class="landing-link-groups">
        <div>
          ...
          <ul>
            ${_e.map(e=>`<li><a href="/baldai-pagal-uzsakyma/${e.slug}" data-internal-link="true">${G(e.title)}</a></li>`).join(``)}
          </ul>
        </div>
        ${e.length?`
          <div>
  ...
  `}function an(e,t,n,r,i){let a=document.createElement(`div`);a.className=`filter-field`;let o=document.createElement(`label`);o.htmlFor=e,o.textContent=t;let s=document.createElement(`div`);s.className=`select-wrap`;let c=document.createElement(`select`);c.id=e,c.name=e;let l=document.createElement(`option`);return l.value=``,l.textContent=n,c.append(l),r.forEach(e=>{let t=document.createElement(`option`);t.value=e.value,t.textContent=e.label,t.selected=e.value===i,c.append(t)}),s.append(c),a.append(o,s),a}function on(e){let t=document.createElement(`div`);t.className=`filter-toggle`;let n=document.createElement(`input`);n.id=e,n.name=e,n.type=`checkbox`,n.checked=W.registryCheckedOnly;let r=document.createElement(`label`);r.htmlFor=e;let i=document.createElement(`strong`);i.textContent=`Tik patikrinti registro duomenys`;let a=document.createElement(`span`);return a.textContent=`Tai duomenų būsenos žyma, ne kokybės ar prieinamumo garantija.`,r.append(i,a),t.append(n,r),t}function sn(e){let t=document.createElement(`div`);t.className=`filter-grid filter-grid--advanced`;let n=an(`${e}-size-filter`,`Įmonės dydis`,`Visi darbuotojų skaičiai`,vt,W.employeeBand),r=an(`${e}-founded-filter`,`Įkūrimo laikotarpis`,`Visi įkūrimo metai`,yt,W.foundedPeriod),i=on(`${e}-registry-filter`);return t.append(n,r,i),{fields:t,employeeBandSelect:n.querySelector(`select`),foundedPeriodSelect:r.querySelector(`select`),registryToggle:i.querySelector(`input`)}}function cn(e){if(!qt(e))return null;let t=document.createElement(`p`);t.className=`registry-card-status`;let n=document.createElement(`span`);n.className=`registry-check-mark`,n.setAttribute(`aria-hidden`,`true`);let r=document.createElement(`strong`);r.textContent=`Registro duomenys patikrinti`,t.append(n,r);let i=jt(e.verified_at);if(i){let e=document.createTextNode(` · `),n=document.createElement(`time`);n.dateTime=i.iso,n.textContent=i.label,t.append(e,n)}return t}function ln(e){let t=document.createElement(`article`);t.className=`manufacturer-card`;let n=document.createElement(`div`);n.className=`card-heading`;let r=document.createElement(`h3`);r.textContent=K(e.trading_name,`Pavadinimas nenurodytas`);let i=$t(e);if(i){let e=document.createElement(`p`);e.className=`legal-name`,e.textContent=i,n.append(r,e)}else n.append(r);let a=document.createElement(`p`);a.className=`location-line`;let o=document.createElement(`strong`);o.textContent=K(e.city,`Miestas nenurodytas`);let s=document.createElement(`span`);s.setAttribute(`aria-hidden`,`true`),s.textContent=` · `;let c=document.createElement(`span`);c.textContent=`Regiono grupė: ${K(e.region_label,`nenurodyta`)}`,a.append(o,s,c);let l=document.createElement(`p`);l.className=e.description_lt?.trim()?`description`:`description description--fallback`,l.textContent=K(e.description_lt,`Trumpas aprašymas šaltiniuose nepateiktas.`);let u=document.createElement(`ul`);u.className=`category-list`,u.setAttribute(`aria-label`,`Gaminamų baldų kategorijos`);let d=q(e.category_labels);(d.length?d:[`Kategorijos šaltiniuose nenurodytos`]).forEach(e=>{let t=document.createElement(`li`);t.textContent=e,u.append(t)});let f=document.createElement(`a`);f.className=`profile-link`,f.href=`/gamintojas/${encodeURIComponent(e.slug)}${window.location.search}`,f.dataset.internalLink=`true`,f.textContent=`Peržiūrėti katalogo įrašą`;let p=document.createElement(`span`);p.setAttribute(`aria-hidden`,`true`),p.textContent=` →`,f.append(p),t.append(n);let m=cn(e);return m&&t.append(m),t.append(a,l,u,f),t}function un(){V&&(V.innerHTML=`
    ...
    ${Z()}
  `)}function dn(){let e=document.querySelector(`#browse-content`);e&&(e.innerHTML=`
  ...
  `)}function fn(){V&&(P({title:`Katalogas nepasiekiamas | Baldai pagal užsakymą Lietuvoje`,description:`Gamintojų katalogo duomenų šiuo metu nepavyko gauti.`,path:window.location.pathname,robots:`noindex, follow`}),V.innerHTML=`
    ${X(`directory`)}
    ...
    ${Z()}
  `,document.querySelector(`#retry-button`)?.addEventListener(`click`,()=>void Un()))}function pn(){let e=document.querySelector(`#home-filter-controls`),t=document.querySelector(`#browse-content`);if(!e||!t)return;let n=Ut(U),r=Wt(U,`city`,`city`),i=Wt(U,`region`,`region_label`);Gt(n,r,i),e.replaceChildren(),t.replaceChildren();let a=document.createElement(`div`);a.className=`browse-controls`;let o=document.createElement(`form`);o.className=`filter-form`,o.setAttribute(`role`,`search`),o.setAttribute(`aria-label`,`Gamintojų katalogo paieška`);let s=document.createElement(`div`);s.className=`filter-field filter-field--search`;let c=document.createElement(`label`);c.htmlFor=`directory-search`,c.textContent=`Poreikis arba pavadinimas`;let l=document.createElement(`input`);l.id=`directory-search`,l.name=`paieska`,l.type=`search`,l.autocomplete=`off`,l.placeholder=`Pvz., virtuvės baldai arba įmonės pavadinimas`,l.value=W.query,s.append(c,l);let u=document.createElement(`div`);u.className=`filter-grid filter-grid--primary`,u.append(s,an(`category-filter`,`Baldų kategorija`,`Visos kategorijos`,n,W.category),an(`city-filter`,`Miestas`,`Visi miestai`,r,W.city));let d=sn(`directory`),f=document.createElement(`details`);f.className=`advanced-filters`,f.open=!!(W.region||Yt());let p=document.createElement(`summary`),m=document.createElement(`span`);m.textContent=`Daugiau filtrų`;let h=document.createElement(`span`);h.className=`advanced-filter-status`,p.append(m,h);let g=document.createElement(`div`);g.className=`filter-grid filter-grid--advanced`,g.append(an(`region-filter`,`Šaltinio regiono grupė`,`Visos regiono grupės`,i,W.region),...Array.from(d.fields.children)),f.append(p,g);let _=document.createElement(`div`);_.className=`filter-actions`;let v=document.createElement(`button`);v.className=`primary-button home-search-submit`,v.type=`submit`,v.textContent=`Rodyti gamintojus`;let y=document.createElement(`button`);y.className=`text-button`,y.id=`clear-filters`,y.type=`button`,y.textContent=`Išvalyti filtrus`,_.append(v,y),o.append(u,f,_),a.append(o);let b=document.createElement(`div`);b.className=`results-area`,b.id=`results-area`,e.append(a),t.append(b);let x=()=>{let e=[W.region,W.employeeBand,W.foundedPeriod].filter(Boolean).length+ +!!W.registryCheckedOnly;h.textContent=e?`${e} pasirinkta`:`Nebūtina`};x(),o.addEventListener(`submit`,e=>{e.preventDefault();let t=document.querySelector(`.result-count`);document.querySelector(`#gamintojai`)?.scrollIntoView({behavior:window.matchMedia(`(prefers-reduced-motion: reduce)`).matches?`auto`:`smooth`,block:`start`}),t?.focus({preventScroll:!0})}),l.addEventListener(`input`,()=>{W.query=l.value.trimStart(),J(`replace`),Q()}),document.querySelector(`#category-filter`)?.addEventListener(`change`,e=>{W.category=e.currentTarget.value,J(`push`),Q()}),document.querySelector(`#city-filter`)?.addEventListener(`change`,e=>{W.city=e.currentTarget.value,J(`push`),Q()}),document.querySelector(`#region-filter`)?.addEventListener(`change`,e=>{W.region=e.currentTarget.value,J(`push`),x(),Q()}),d.employeeBandSelect.addEventListener(`change`,()=>{W.employeeBand=d.employeeBandSelect.value,J(`push`),x(),Q()}),d.foundedPeriodSelect.addEventListener(`change`,()=>{W.foundedPeriod=d.foundedPeriodSelect.value,J(`push`),x(),Q()}),d.registryToggle.addEventListener(`change`,()=>{W.registryCheckedOnly=d.registryToggle.checked,J(`push`),x(),Q()}),y.addEventListener(`click`,()=>{Xt(),J(`push`),pn(),document.querySelector(`#directory-search`)?.focus()}),Q()}function Q(){let e=document.querySelector(`#results-area`);if(!e)return;window.location.pathname===`/`&&Vn();let t=Jt(U),n=!!(W.query||W.category||W.city||W.region||Yt());e.replaceChildren();let r=document.createElement(`div`);r.className=`result-header`;let i=document.createElement(`p`);if(i.className=`result-count`,i.tabIndex=-1,i.setAttribute(`role`,`status`),i.setAttribute(`aria-live`,`polite`),i.textContent=n?`Rodoma įrašų: ${t.length}. Iš viso kataloge: ${U.length}.`:`Kataloge – ${Zt(U.length)}.`,r.append(i),n){let e=document.createElement(`ul`);e.className=`active-filters`,e.setAttribute(`aria-label`,`Aktyvūs paieškos kriterijai`),Qt().forEach(t=>{let n=document.createElement(`li`);n.textContent=t,e.append(n)}),r.append(e)}if(e.append(r),t.length===0){let t=document.createElement(`div`);t.className=`message-state`;let n=document.createElement(`p`);n.className=`state-label`,n.textContent=`Rezultatų nėra`;let r=document.createElement(`h3`);r.textContent=`Pagal šiuos kriterijus įrašų nerasta`;let i=document.createElement(`p`);i.textContent=`Pakeiskite paieškos žodį, pasirinkite platesnę vietovę arba išvalykite filtrus.`;let a=document.createElement(`button`);a.className=`primary-button`,a.type=`button`,a.textContent=`Išvalyti visus kriterijus`,a.addEventListener(`click`,()=>{Xt(),J(`push`),pn()}),t.append(n,r,i,a),e.append(t);return}let a=document.createElement(`div`);a.className=`manufacturer-list`,t.forEach(e=>a.append(ln(e))),e.append(a)}function mn(e,t){let n=document.createElement(`div`),r=document.createElement(`dt`);r.textContent=e;let i=document.createElement(`dd`);return i.textContent=t,n.append(r,i),n}function $(e,t,n=`Viešuose šaltiniuose nenurodyta.`){let r=t?.trim()??``,i=mn(e,r||n);return r||i.querySelector(`dd`)?.classList.add(`unknown-value`),i}function hn(e,t){let n=document.createElement(`div`),r=document.createElement(`dt`);r.textContent=`Įmonės dydžio signalas`;let i=document.createElement(`dd`),a=document.createElement(`strong`);a.textContent=Ot(t);let o=document.createElement(`p`);o.className=`fact-explanation`,o.textContent=`Tai viešame įmonės įraše nurodyta darbuotojų skaičiaus grupė. Ji neparodo darbų kokybės, dabartinio užimtumo ar galimybės priimti jūsų projektą.`,i.append(a,o);let s=q(e.public_details_source_urls).filter(e=>e.includes(`rekvizitai.vz.lt`));if(s.length){let e=document.createElement(`ul`);e.className=`fact-source-list`,s.forEach((t,n)=>{let r=document.createElement(`li`),i=document.createElement(`a`);i.href=t,i.target=`_blank`,i.rel=`noopener noreferrer`,i.textContent=s.length===1?`Atverti viešą darbuotojų skaičiaus šaltinį`:`Atverti viešą šaltinį ${n+1}`,r.append(i),e.append(r)}),i.append(e)}return n.append(r,i),n}function gn(e,t){let n=document.createElement(`div`),r=document.createElement(`dt`);r.textContent=e;let i=document.createElement(`dd`),a=t?.trim();if(a){let e=document.createElement(`a`);e.href=a,e.target=`_blank`,e.rel=`noopener noreferrer`,e.textContent=a,i.append(e)}else i.textContent=`Viešuose šaltiniuose nenurodyta.`,i.className=`unknown-value`;return n.append(r,i),n}function _n(e,t){let n=document.createElement(`div`),r=document.createElement(`dt`);r.textContent=e;let i=document.createElement(`dd`),a=document.createElement(`a`);return a.href=`tel:${t.replace(/[^+\d]/g,``).replace(/(?!^)\+/g,``)}`,a.textContent=t,i.append(a),n.append(r,i),n}function vn(e){let t=[],n=e.company_code?.trim(),r=e.public_phone?.trim(),i=e.street_address?.trim(),a=e.postcode?.trim(),o=e.employee_count_band?.trim();if(n&&t.push(mn(`Įmonės kodas`,n)),i&&t.push(mn(`Registracijos adresas`,i)),a&&t.push(mn(`Pašto kodas`,a)),Number.isInteger(e.founded_year)&&Number(e.founded_year)>0&&t.push(mn(`Įkurta`,String(e.founded_year))),o&&t.push(hn(e,o)),r&&t.push(_n(`Viešas telefono numeris`,r)),!t.length)return null;let s=document.createElement(`section`);s.className=`profile-details profile-public-details`,s.setAttribute(`aria-labelledby`,`profile-public-details-title`),s.innerHTML=`
    <div class="section-heading">
  ...
  `;let c=document.createElement(`dl`);return c.className=`profile-facts`,c.append(...t),s.append(c),s}function yn(e){if(!qt(e))return null;let t=document.createElement(`section`);t.className=`profile-registry-status`,t.setAttribute(`aria-labelledby`,`profile-registry-status-title`);let n=document.createElement(`span`);n.className=`registry-check-mark registry-check-mark--large`,n.setAttribute(`aria-hidden`,`true`);let r=document.createElement(`div`),i=document.createElement(`h2`);i.id=`profile-registry-status-title`,i.textContent=`Registro duomenys patikrinti`;let a=jt(e.verified_at),o=document.createElement(`p`);if(a){o.append(`Paskutinė registro duomenų patikra: `);let e=document.createElement(`time`);e.dateTime=a.iso,e.textContent=a.label,o.append(e,`. `)}else o.append(`Paskutinės patikros data viešame įraše nenurodyta. `);return o.append(`Ši žyma nurodo tik registro duomenų peržiūros būseną; ji nepatvirtina darbų kokybės, užimtumo ar paslaugų prieinamumo.`),r.append(i,o),t.append(n,r),t}function bn(e){let t=document.createElement(`section`);t.className=`profile-overview`,t.setAttribute(`aria-labelledby`,`profile-overview-title`);let n=document.createElement(`div`);n.className=`profile-overview-heading`,n.innerHTML=`
  ...
  `;let r=e.city?.trim()||e.location?.trim()||``,i=e.region_label?.trim()||``,a=e.website?.trim()||``,o=e.public_contact_url?.trim()||``,s=e.public_phone?.trim()||``,c=[];s&&c.push(`telefono numeris`),a&&c.push(`svetainė`),o&&o!==a&&c.push(`kontaktų puslapis`);let l=[e.legal_name?.trim(),e.description_lt?.trim(),e.scope_evidence?.trim(),r,i,q(e.category_labels).length>0,a,o,e.company_code?.trim(),s,e.street_address?.trim(),e.postcode?.trim(),Number.isInteger(e.founded_year)&&Number(e.founded_year)>0,e.employee_count_band?.trim()],u=l.filter(Boolean).length,d=Mt(e.source_collection_date),f=document.createElement(`dl`);f.className=`profile-overview-list`;let p=(e,t,n,r,i)=>{let a=document.createElement(`div`);a.dataset.state=r;let o=document.createElement(`dt`);o.textContent=e;let s=document.createElement(`dd`),c=document.createElement(`span`);c.className=`profile-overview-marker`,c.setAttribute(`aria-hidden`,`true`);let l=document.createElement(`strong`);if(i){l.append(t);let e=document.createElement(`time`);e.dateTime=i.iso,e.textContent=i.label,l.append(e)}else l.textContent=t;let u=document.createElement(`span`);u.textContent=n,s.append(c,l,u),a.append(o,s),f.append(a)};return p(`Vieta`,r||(i?`Nurodyta tik regiono grupė`:`Nenurodyta`),r?i?`Šaltinio regiono grupė: ${i}.`:`Tai šaltinyje nurodyta vieta, ne aptarnavimo teritorija.`:i||`Viešuose šaltiniuose vieta nepateikta.`,r?`present`:i?`partial`:`missing`),p(`Vieši kontaktai`,c.length?`${c.length} ${c.length===1?`kontakto būdas`:`kontakto būdai`}`:`Nenurodyta`,c.length?`Kataloge pateikta: ${c.join(`, `)}.`:`Telefono, svetainės ar atskiros kontaktų nuorodos katalogo laukuose nėra.`,c.length?`present`:`missing`),p(`Duomenų apimtis`,`${u} iš ${l.length} pagrindinių laukų`,`Tai katalogo laukų užpildymo, o ne informacijos patikimumo rodiklis.`,u===l.length?`present`:u>0?`partial`:`missing`),p(`Šaltinių data`,d?`Surinkta `:`Nenurodyta`,d?`Data rodo šaltinių rinkimo laiką, ne dabartinės veiklos patvirtinimą.`:`Šaltinių surinkimo data katalogo įraše nepateikta.`,d?`present`:`missing`,d),t.append(n,f),t}function xn(e){let t=document.createElement(`section`);t.className=`provenance-section`,t.setAttribute(`aria-labelledby`,`provenance-title`);let n=document.createElement(`h2`);n.id=`provenance-title`,n.textContent=`Šaltiniai ir duomenų kilmė`;let r=document.createElement(`p`);r.textContent=`Įrašas sudarytas iš viešai prieinamų šaltinių. Katalogas šių duomenų netvirtino su gamintoju ir negarantuoja jų tikslumo, aktualumo, kokybės ar paslaugų prieinamumo.`;let i=Array.from(new Set([...q(e.source_urls),K(e.source_artifact_url,``),...q(e.public_details_source_urls)].filter(Boolean))),a=document.createElement(`ul`);if(a.className=`source-list`,i.length)i.forEach((e,t)=>{let n=document.createElement(`li`),r=document.createElement(`span`);r.textContent=t===0?`Viešas šaltinis`:`Papildomas šaltinis ${t+1}`;let i=document.createElement(`a`);i.href=e,i.target=`_blank`,i.rel=`noopener noreferrer`,i.textContent=e,n.append(r,i),a.append(n)});else{let e=document.createElement(`li`);e.className=`unknown-value`,e.textContent=`Šaltinio nuoroda viešame įraše nenurodyta.`,a.append(e)}let o=document.createElement(`p`);o.className=`collection-date`,o.textContent=`Šaltinių informacija surinkta: `;let s=Mt(e.source_collection_date);if(s){let e=document.createElement(`time`);e.dateTime=s.iso,e.textContent=s.label,o.append(e)}else{let e=document.createElement(`span`);e.className=`unknown-value`,e.textContent=`data katalogo įraše nenurodyta`,o.append(e)}return t.append(n,r,a,o),t}function Sn(e){let t=document.createElement(`section`);t.className=`profile-reviews`,t.setAttribute(`aria-labelledby`,`profile-reviews-title`);let n=document.createElement(`div`);n.className=`section-heading`,n.innerHTML=`
  ...
  `;let r=document.createElement(`div`);r.className=`review-content`;let i=document.createElement(`p`);i.className=`review-loading`,i.setAttribute(`role`,`status`),i.setAttribute(`aria-live`,`polite`),i.textContent=`Kraunami patvirtinti atsiliepimai…`,r.append(i);let a=document.createElement(`div`);a.className=`review-form-section`;let o=document.createElement(`div`);o.className=`review-form-heading`;let s=document.createElement(`h3`);s.id=`review-form-title`,s.textContent=`Pasidalykite naudinga patirtimi`;let c=document.createElement(`p`);c.textContent=`Atsiliepimas pirmiausia pateks moderavimui ir nebus paskelbtas iš karto. Rašykite apie konkretų projektą, susitarimų aiškumą, eigą ir rezultatą.`,o.append(s,c);let l=document.createElement(`form`);l.className=`review-form`,l.setAttribute(`aria-labelledby`,`review-form-title`);let u=document.createElement(`div`);u.className=`form-field`;let d=document.createElement(`label`);d.htmlFor=`review-rating`,d.textContent=`Įvertinimas nuo 1 iki 5 *`;let f=document.createElement(`div`);f.className=`select-wrap`;let p=document.createElement(`select`);p.id=`review-rating`,p.name=`rating`,p.required=!0;let m=document.createElement(`option`);m.value=``,m.textContent=`Pasirinkite įvertinimą`,m.disabled=!0,m.selected=!0,p.append(m),[[5,`5 – labai gerai`],[4,`4 – gerai`],[3,`3 – vidutiniškai`],[2,`2 – prastai`],[1,`1 – labai prastai`]].forEach(([e,t])=>{let n=document.createElement(`option`);n.value=String(e),n.textContent=String(t),p.append(n)}),f.append(p),u.append(d,f);let h=document.createElement(`div`);h.className=`form-field`;let g=document.createElement(`label`);g.htmlFor=`review-display-name`,g.textContent=`Rodomas vardas arba inicialai *`;let _=document.createElement(`input`);_.id=`review-display-name`,_.name=`display_name`,_.type=`text`,_.autocomplete=`name`,_.minLength=2,_.maxLength=80,_.required=!0,h.append(g,_);let v=document.createElement(`div`);v.className=`form-field form-field--wide`;let y=document.createElement(`label`);y.htmlFor=`review-project-type`,y.textContent=`Projekto rūšis (nebūtina)`;let b=document.createElement(`div`);b.className=`select-wrap`;let x=document.createElement(`select`);x.id=`review-project-type`,x.name=`project_type`;let S=document.createElement(`option`);S.value=``,S.textContent=`Nenurodyti`,x.append(S),St.forEach(e=>{let t=document.createElement(`option`);t.value=e,t.textContent=e,x.append(t)}),b.append(x),v.append(y,b);let C=document.createElement(`div`);C.className=`form-field form-field--wide`;let w=document.createElement(`label`);w.htmlFor=`review-text`,w.textContent=`Naudingas komentaras *`;let T=document.createElement(`p`);T.className=`field-hint`,T.id=`review-text-hint`,T.textContent=`Bent ${bt} ženklų. Nevartokite įžeidimų ir neskelbkite kitų žmonių asmens duomenų.`;let E=document.createElement(`textarea`);E.id=`review-text`,E.name=`review_text`,E.rows=6,E.minLength=bt,E.maxLength=xt,E.required=!0,E.setAttribute(`aria-describedby`,`review-text-hint`),C.append(w,T,E);let ee=document.createElement(`div`);ee.className=`form-field form-field--wide`;let te=document.createElement(`label`);te.htmlFor=`review-contact-email`,te.textContent=`Kontaktinis el. paštas *`;let ne=document.createElement(`p`);ne.className=`field-hint`,ne.id=`review-email-hint`,ne.textContent=`Naudojamas tik moderavimui ar patikslinimui; viešai nerodomas.`;let D=document.createElement(`input`);D.id=`review-contact-email`,D.name=`contact_email`,D.type=`email`,D.inputMode=`email`,D.autocomplete=`email`,D.maxLength=254,D.required=!0,D.placeholder=`vardas@pavyzdys.lt`,D.setAttribute(`aria-describedby`,`review-email-hint`),ee.append(te,ne,D);let re=document.createElement(`div`);re.className=`honeypot-field`,re.setAttribute(`aria-hidden`,`true`);let ie=document.createElement(`label`);ie.htmlFor=`review-website`,ie.textContent=`Interneto svetainė`;let O=document.createElement(`input`);O.id=`review-website`,O.name=`honeypot`,O.type=`text`,O.autocomplete=`off`,O.tabIndex=-1,O.maxLength=200,re.append(ie,O);let ae=document.createElement(`div`);ae.className=`review-submit form-field--wide`;let k=document.createElement(`button`);k.className=`primary-button`,k.type=`submit`,k.textContent=`Pateikti moderavimui`;let A=document.createElement(`p`);A.className=`form-status`,A.setAttribute(`role`,`status`),A.setAttribute(`aria-live`,`polite`),A.tabIndex=-1,ae.append(k,A);let oe=document.createElement(`p`);return oe.className=`form-privacy-note form-field--wide`,oe.innerHTML=`Kontaktinį el. paštą naudosime tik atsiliepimui moderuoti ar patikslinti. Skaitykite <a href="/privatumas" data-internal-link="true">privatumo pranešimą</a> ir <a href="/atsiliepimu-taisykles" data-internal-link="true">atsiliepimų taisykles</a>.`,l.append(u,h,v,C,ee,re,oe,ae),a.append(o,l),l.addEventListener(`submit`,async t=>{if(t.preventDefault(),l.reportValidity()){k.disabled=!0,k.setAttribute(`aria-busy`,`true`),k.textContent=`Pateikiama…`,A.className=`form-status`,A.setAttribute(`role`,`status`),A.textContent=`Atsiliepimas siunčiamas moderavimui.`;try{await de.collection(`manufacturer_reviews`).create({manufacturer:e.id,rating:Number(p.value),display_name:_.value.trim(),review_text:E.value.trim(),project_type:x.value,contact_email:D.value.trim(),honeypot:O.value,status:`pending`}),l.reset(),A.className=`form-status form-status--success`,A.textContent=`Ačiū. Atsiliepimas gautas ir bus paskelbtas tik tuo atveju, jei po moderavimo bus patvirtintas.`,A.focus()}catch(e){console.error(`Nepavyko pateikti atsiliepimo moderavimui.`,e),A.className=`form-status form-status--error`,A.setAttribute(`role`,`alert`),A.textContent=`Atsiliepimo pateikti nepavyko. Patikrinkite laukus ir interneto ryšį, tada bandykite dar kartą.`,A.focus()}finally{k.disabled=!1,k.removeAttribute(`aria-busy`),k.textContent=`Pateikti moderavimui`}}}),(async()=>{try{let t=await de.collection(`manufacturer_reviews`).getFullList({filter:de.filter(`manufacturer = {:manufacturer} && status = "approved"`,{manufacturer:e.id}),sort:`-created`});if(r.replaceChildren(),!t.length){let e=document.createElement(`p`);e.className=`review-empty`,e.textContent=`Patvirtintų atsiliepimų dar nėra. Suvestinė bus rodoma tik tada, kai bus bent vienas patvirtintas atsiliepimas.`,r.append(e);return}let n=t.reduce((e,t)=>e+Number(t.rating),0)/t.length,i=document.createElement(`div`);i.className=`review-summary`;let a=document.createElement(`strong`);a.textContent=`${n.toLocaleString(`lt-LT`,{minimumFractionDigits:1,maximumFractionDigits:1})} iš 5`;let o=document.createElement(`span`);o.textContent=kt(t.length);let s=document.createElement(`p`);s.textContent=`Suvestinė apskaičiuota tik iš šiame kataloge patvirtintų atsiliepimų.`,i.append(a,o,s);let c=document.createElement(`ol`);c.className=`review-list`,t.forEach(e=>{let t=document.createElement(`li`),n=document.createElement(`div`);n.className=`review-item-header`;let r=document.createElement(`strong`);r.textContent=K(e.display_name,`Vardas nenurodytas`);let i=document.createElement(`span`);i.className=`review-rating`,i.textContent=`Įvertinimas: ${Number(e.rating)} iš 5`,n.append(r,i);let a=document.createElement(`p`);a.className=`review-meta`;let o=e.project_type?.trim();o&&a.append(o);let s=At(e.created);if(s){o&&a.append(` · `);let e=document.createElement(`time`);e.dateTime=s.iso,e.textContent=s.label,a.append(e)}let l=document.createElement(`p`);l.className=`review-text`,l.textContent=e.review_text,t.append(n),a.textContent&&t.append(a),t.append(l),c.append(t)}),r.append(i,c)}catch(e){console.error(`Nepavyko įkelti patvirtintų atsiliepimų.`,e),r.replaceChildren();let t=document.createElement(`p`);t.className=`review-error`,t.setAttribute(`role`,`alert`),t.textContent=`Patvirtintų atsiliepimų šiuo metu įkelti nepavyko. Bandykite atnaujinti puslapį vėliau.`,r.append(t)}})(),t.append(n,r,a),t}function Cn(e){let t=document.createElement(`section`);t.className=`correction-section`,t.id=`pataisyti-profili`,t.setAttribute(`aria-labelledby`,`correction-title`),t.innerHTML=`
    ...
    <form class="correction-form" aria-labelledby="correction-title" novalidate>
      <p class="form-record-context">Katalogo įrašas: ${G(K(e.trading_name,`Pavadinimas nenurodytas`))}</p>
      <input type="hidden" name="manufacturer_slug" value="${G(e.slug)}" />

      <div class="form-field form-field--wide">
        <label for="claim-company-name">Įmonės pavadinimas *</label>
        <input id="claim-company-name" name="company_name" type="text" autocomplete="organization" maxlength="240" value="${G(e.trading_name)}" required aria-describedby="claim-company-name-error" />
  ...
  `;let n=t.querySelector(`form`),r=t.querySelector(`#claim-company-name`),i=t.querySelector(`#claimant-name`),a=t.querySelector(`#claimant-role`),o=t.querySelector(`#claimant-email`),s=t.querySelector(`#claimant-phone`),c=t.querySelector(`#claim-message`),l=t.querySelector(`#claim-consent`),u=t.querySelector(`#claim-website`),d=t.querySelector(`button[type="submit"]`),f=t.querySelector(`.form-status`);if(!n||!r||!i||!a||!o||!s||!c||!l||!u||!d||!f)return t;let p=[r,i,a,o,s,c,l],m=e=>{let n=(e.getAttribute(`aria-describedby`)?.split(` `)??[]).find(e=>e.endsWith(`-error`));return n?t.querySelector(`#${n}`):null},h=e=>{let t=``;e===r&&r.value.trim().length<2?t=`Įrašykite įmonės pavadinimą.`:e===i&&i.value.trim().length<2?t=`Įrašykite savo vardą ir pavardę.`:e===a&&a.value.trim().length<2?t=`Nurodykite savo pareigas arba ryšį su įmone.`:e===o&&!o.value.trim()?t=`Įrašykite el. pašto adresą.`:e===o&&o.validity.typeMismatch?t=`Įrašykite galiojantį el. pašto adresą.`:e===s&&s.value.trim()&&!/^[0-9+().\-\s]+$/.test(s.value.trim())?t=`Telefono numeriui naudokite tik skaitmenis ir įprastus numerio ženklus.`:e===c&&c.value.trim().length<10?t=`Aprašykite prašymą bent 10 ženklų.`:e===l&&!l.checked&&(t=`Norėdami pateikti prašymą, patvirtinkite šį sutikimą.`);let n=m(e);return n&&(n.textContent=t),t?e.setAttribute(`aria-invalid`,`true`):e.removeAttribute(`aria-invalid`),!t};return p.forEach(e=>{e.addEventListener(e===l?`change`:`blur`,()=>h(e)),e!==l&&e.addEventListener(`input`,()=>{e.hasAttribute(`aria-invalid`)&&h(e)})}),n.addEventListener(`submit`,async t=>{t.preventDefault();let g=!0;if(p.forEach(e=>{h(e)||(g=!1)}),!g){f.className=`form-status form-status--error`,f.setAttribute(`role`,`alert`),f.textContent=`Patikrinkite pažymėtus laukus ir bandykite dar kartą.`,p.find(e=>e.getAttribute(`aria-invalid`)===`true`)?.focus();return}d.disabled=!0,d.setAttribute(`aria-busy`,`true`),n.setAttribute(`aria-busy`,`true`),d.textContent=`Pateikiama…`,f.className=`form-status`,f.setAttribute(`role`,`status`),f.textContent=`Prašymas siunčiamas į katalogo peržiūros eilę.`;try{await de.collection(`manufacturer_claims`).create({manufacturer_slug:e.slug,company_name:r.value.trim(),claimant_name:i.value.trim(),role:a.value.trim(),email:o.value.trim().toLocaleLowerCase(`lt-LT`),phone:s.value.trim(),message:c.value.trim(),consent:l.checked,honeypot:u.value}),n.reset(),r.value=e.trading_name,p.forEach(e=>{e.removeAttribute(`aria-invalid`);let t=m(e);t&&(t.textContent=``)}),f.className=`form-status form-status--success`,f.setAttribute(`role`,`status`),f.textContent=`Ačiū. Prašymas gautas ir bus patikrintas prieš atliekant bet kokį viešą pakeitimą.`,f.focus()}catch(e){console.error(`Nepavyko pateikti gamintojo atstovavimo ar pataisos prašymo.`,e),f.className=`form-status form-status--error`,f.setAttribute(`role`,`alert`),f.textContent=`Prašymo pateikti nepavyko. Patikrinkite interneto ryšį ir laukus, tada bandykite dar kartą.`,f.focus()}finally{d.disabled=!1,d.removeAttribute(`aria-busy`),n.removeAttribute(`aria-busy`),d.textContent=`Pateikti įrašo peržiūrai`}}),t}function wn(e){return[{question:`Ar šiame puslapyje pateikti ${e.title.toLocaleLowerCase(`lt-LT`)} gamintojai yra rekomenduojami?`,answer:`Ne. Tai viešais šaltiniais paremtas nepatvirtintų kandidatų sąrašas, skirtas savarankiškai atrankai.`},{question:`Ar kategorijos žyma patvirtina, kad gamintojas priims mano užsakymą?`,answer:`Ne. Kategorija rodo tik šaltinių rinkinyje užfiksuotą veiklos kryptį. Dabartinę pasiūlą, užimtumą ir projekto tinkamumą reikia patvirtinti tiesiogiai.`},{question:`Kaip palyginti pasirinktus kandidatus?`,answer:`Siųskite vienodą projekto aprašymą ir raštu palyginkite medžiagas, furnitūrą, paslaugų apimtį, kainos sudėtį, terminų prielaidas bei priėmimo sąlygas.`}]}function Tn(e){return[{question:`Kodėl kandidatai pateikti ${e} puslapyje?`,answer:`Jų šaltinio įraše kaip bazės miestas ar vietovė nurodytas ${e}. Tai nėra teiginys apie aptarnavimo teritoriją.`},{question:`Ar visi šiame sąraše esantys gamintojai aptarnauja visą ${e} miestą ar aplinkinį regioną?`,answer:`Katalogas to netvirtina. Pristatymo, matavimo ir montavimo teritoriją reikia patikrinti tiesiogiai su kiekvienu kandidatu.`},{question:`Ar sąrašo vieta reiškia kokybės ar prieinamumo patvirtinimą?`,answer:`Ne. Įrašai nepatvirtinti, o jų eiliškumas nėra reitingas ar rekomendacija.`}]}function En(e,t){return[{question:`Kodėl ${e.title.toLocaleLowerCase(`lt-LT`)} kandidatai pateikti miesto ${t} puslapyje?`,answer:`Šių įrašų viešuose šaltiniuose užfiksuota kategorija „${e.title}“, o bazės miestas ar vietovė nurodyta ${t}. Abi žymos yra informacinės.`},{question:`Ar šie gamintojai aptarnauja visą miestą ${t}?`,answer:`Katalogas to netvirtina. Matavimo, pristatymo ir montavimo teritoriją reikia patikrinti tiesiogiai su kiekvienu kandidatu.`},{question:`Ar patekimas į šį sąrašą reiškia rekomendaciją?`,answer:`Ne. Įrašai yra nepatvirtinti, jų eiliškumas nėra reitingas, o kategorijos ir miesto sutapimas negarantuoja kokybės, prieinamumo ar tinkamumo konkrečiam projektui.`}]}function Dn(e){return`
  ...
  `}function On(e,t){let n=document.querySelector(`#landing-filter-controls`),r=document.querySelector(`#landing-manufacturer-list`);if(!n||!r)return;Gt([],[],[],!1),n.replaceChildren();let i=document.createElement(`form`);i.className=`filter-form landing-filter-form`,i.setAttribute(`role`,`search`),i.addEventListener(`submit`,e=>e.preventDefault());let a=document.createElement(`div`);a.className=`filter-field filter-field--search`;let o=document.createElement(`label`);o.htmlFor=`landing-search`,o.textContent=`Ieškoti šiame sąraše`;let s=document.createElement(`input`);s.id=`landing-search`,s.name=`paieska`,s.type=`search`,s.autocomplete=`off`,s.placeholder=`Pavadinimas, aprašymas ar kategorija`,s.value=W.query,a.append(o,s);let c=sn(`landing`),l=document.createElement(`div`);l.className=`filter-grid filter-grid--landing`,l.append(a,...Array.from(c.fields.children));let u=document.createElement(`div`);u.className=`filter-actions`;let d=document.createElement(`p`);d.className=`landing-filter-count`,d.setAttribute(`role`,`status`),d.setAttribute(`aria-live`,`polite`);let f=document.createElement(`button`);f.className=`text-button`,f.type=`button`,f.textContent=`Išvalyti šio sąrašo filtrus`,u.append(d,f),i.append(l,u),n.append(i);let p=()=>{Xt(),J(`push`),s.value=``,c.employeeBandSelect.value=``,c.foundedPeriodSelect.value=``,c.registryToggle.checked=!1,m(),s.focus()},m=()=>{t();let n=Jt(e,!1),i=!!(W.query||Yt());if(d.textContent=i?`Rodoma įrašų: ${n.length}. Iš viso šiame sąraše: ${e.length}.`:`Šiame sąraše – ${Zt(e.length)}.`,r.replaceChildren(),!n.length){let e=document.createElement(`div`);e.className=`message-state landing-empty-state`;let t=document.createElement(`h3`);t.textContent=`Pagal šiuos kriterijus įrašų nerasta`;let n=document.createElement(`p`);n.textContent=`Pasirinkite platesnį įmonės dydį ar įkūrimo laikotarpį, pakeiskite paiešką arba išvalykite filtrus.`;let i=document.createElement(`button`);i.className=`primary-button`,i.type=`button`,i.textContent=`Išvalyti šio sąrašo filtrus`,i.addEventListener(`click`,p),e.append(t,n,i),r.append(e);return}n.forEach(e=>r.append(ln(e)))};s.addEventListener(`input`,()=>{W.query=s.value.trimStart(),J(`replace`),m()}),c.employeeBandSelect.addEventListener(`change`,()=>{W.employeeBand=c.employeeBandSelect.value,J(`push`),m()}),c.foundedPeriodSelect.addEventListener(`change`,()=>{W.foundedPeriod=c.foundedPeriodSelect.value,J(`push`),m()}),c.registryToggle.addEventListener(`change`,()=>{W.registryCheckedOnly=c.registryToggle.checked,J(`push`),m()}),f.addEventListener(`click`,p),m()}function kn(e,t){if(!V)return;let n=Ce(U),r=Se(U),i=t?r.find(n=>n.category.slug===e&&n.citySlug===t):void 0,a=i?.category??(t?void 0:_e.find(t=>t.slug===e)),o=i?n.find(e=>e.city===i.city):t?void 0:n.find(t=>t.slug===e);if(!a&&!o||t&&!i){Mn(`Paieškos puslapis nerastas`,`Tokio kategorijos ar miesto puslapio nėra. Grįžkite į katalogą ir naudokite paiešką arba filtrus.`);return}let s=U.filter(e=>{let t=a?q(e.category_codes).includes(a.code):!0,n=o?e.city===o.city:!0;return t&&n}),c=!!(i&&a&&o),l=c?`${a?.title} – ${o?.city}`:a?a.title:`Baldų gamintojų kandidatai: ${o?.city}`,u=c?`Šiame puslapyje pateikiami ${s.length} nepatvirtinti kandidatų įrašai, kurių viešuose šaltiniuose nurodyta baldų rūšis „${a?.title}“ ir bazės miestas ar vietovė „${o?.city}“.`:a?a.intro:`Čia pateikiami ${s.length} nepatvirtinti baldų gamintojų kandidatai, kurių viešo šaltinio įraše kaip bazės miestas ar vietovė nurodytas ${o?.city}.`,d=c?`Kategorijos ir miesto sutapimas nepatvirtina darbų kokybės, dabartinio užimtumo, paslaugų teritorijos ar tinkamumo jūsų projektui. Matavimo, pristatymo ir montavimo sąlygas patikrinkite tiesiogiai.`:a?a.buyer_note:`Šis sąrašas nepatvirtina, kad kandidatai aptarnauja visą miestą ar aplinkinį regioną. Matavimo, pristatymo ir montavimo vietas patikrinkite tiesiogiai.`,f=c?En(a,o?.city??``):a?wn(a):Tn(o?.city??`šiame mieste`),p=i?.path??`/baldai-pagal-uzsakyma/${e}`,m=c?`${a?.title}, ${o?.city}: ${s.length} viešais šaltiniais paremti nepatvirtinti gamintojų kandidatai. Sąrašas nėra kokybės, prieinamumo ar paslaugų teritorijos garantija.`:a?`${a.title}: ${s.length} viešais šaltiniais paremti nepatvirtinti Lietuvos gamintojų kandidatai, miestai ir atrankos gairės.`:`${o?.city}: ${s.length} viešuose šaltiniuose šiame mieste registruoti baldų gamintojų kandidatai. Sąrašas nėra paslaugų teritorijos ar kokybės garantija.`,h=c?[{name:`Gamintojų katalogas`,path:`/`},{name:a?.title??``,path:`/baldai-pagal-uzsakyma/${a?.slug}`},{name:l,path:p}]:[{name:`Gamintojų katalogas`,path:`/`},{name:l,path:p}],g=()=>{P({title:`${l} | Gamintojų katalogas`,description:m,path:p,robots:window.location.search?`noindex, follow`:`index, follow`,structuredData:[N(h),Oe(f),ke(s)]})};g();let _=c?[{path:`/baldai-pagal-uzsakyma/${a?.slug}`,label:a?.title??``,count:U.filter(e=>q(e.category_codes).includes(a?.code??``)).length},{path:`/baldai-pagal-uzsakyma/${o?.slug}`,label:`Baldų gamintojų kandidatai: ${o?.city}`,count:o?.count??0}]:a?r.filter(e=>e.category.code===a.code).map(e=>({path:e.path,label:e.city,count:e.count})).sort((e,t)=>t.count-e.count||Ct.compare(e.label,t.label)):r.filter(e=>e.city===o?.city).map(e=>({path:e.path,label:e.category.title,count:e.count})),v=c?`Naršykite platesnius sąrašus`:a?`Šios baldų rūšies miestų puslapiai`:`Šio miesto baldų rūšių puslapiai`,y=c?`Grįžkite į visos baldų rūšies arba viso miesto kandidatų sąrašą.`:`Nuorodos rodomos tik toms baldų rūšies ir miesto sankirtoms, kuriose yra bent trys katalogo įrašai.`;V.innerHTML=`
    ${X(`directory`)}
    ...
    ${Z()}
  `,On(s,g)}function An(){V&&at({root:V,renderHeader:X,renderFooter:Z,manufacturers:U})}function jn(e){V&&(P({title:e.title,description:e.description,path:e.path,structuredData:[N([{name:`Gamintojų katalogas`,path:`/`},{name:e.heading,path:e.path}])]}),V.innerHTML=`
    ${X(`policy`)}
    ...
    ${Z()}
  `)}function Mn(e,t){V&&(P({title:`${e} | Baldai pagal užsakymą Lietuvoje`,description:t,path:window.location.pathname,robots:`noindex, follow`}),V.innerHTML=`
    ${X(`directory`)}
    <main class="profile-main">
  ...
  `;let t=document.querySelector(`#profile-main`);if(!t)return;let n=document.createElement(`a`);if(n.className=`back-link`,n.href=`/${window.location.search}`,n.dataset.internalLink=`true`,n.textContent=`← Grįžti į gamintojų katalogą`,!e){P({title:`Gamintojas nerastas | Baldai pagal užsakymą Lietuvoje`,description:`Gamintojo įrašas šiame viešų šaltinių kataloge nerastas.`,path:window.location.pathname,robots:`noindex, follow`});let e=document.createElement(`section`);e.className=`message-state profile-state not-found-state`,e.innerHTML=`
    ...
    `,t.append(n,e);return}let r=K(e.trading_name,`Gamintojo pavadinimas nenurodytas`),i=`/gamintojas/${e.slug}`;P({title:`${r} | Baldų gamintojo įrašas`,description:`${r}: viešais šaltiniais paremtas, nepatvirtintas gamintojo kandidato įrašas su vieta, kategorijomis ir šaltinių nuorodomis.`,path:i,robots:window.location.search?`noindex, follow`:`index, follow`,type:`profile`,structuredData:[N([{name:`Gamintojų katalogas`,path:`/`},{name:r,path:i}]),De(e)]});let a=document.createElement(`article`);a.className=`profile-sheet`;let o=document.createElement(`header`);o.className=`profile-hero`;let s=document.createElement(`div`);s.className=`profile-heading-group`;let c=document.createElement(`p`);c.className=`record-status`,c.textContent=`Nepatvirtintas viešų šaltinių įrašas`;let l=document.createElement(`h1`);l.textContent=r;let u=document.createElement(`p`);u.className=`profile-identity`,u.textContent=K(e.source_identity,`Šaltinyje pateikta tapatybė nenurodyta.`),s.append(c,l,u);let d=document.createElement(`div`);d.className=`profile-actions`;let f=document.createElement(`a`);f.className=`primary-button`,f.href=`/gauti-pasiulymus?gamintojas=${encodeURIComponent(e.slug)}`,f.dataset.internalLink=`true`,f.textContent=`Įtraukti į projekto užklausą`;let p=document.createElement(`a`);p.className=`secondary-button profile-claim-link`,p.href=`#pataisyti-profili`,p.textContent=`Atstovaujate įmonei? Papildyti profilį`;let m=document.createElement(`a`);m.className=`profile-guide-link`,m.href=`/gidas`,m.dataset.internalLink=`true`,m.textContent=`Prieš kreipdamiesi peržiūrėkite pirkėjo gidą →`;let h=en(e),g=document.createElement(`a`);g.className=`profile-guide-link`,g.href=`/gidas/${h.slug}`,g.dataset.internalLink=`true`,g.textContent=`${h.label} →`,d.append(f,p,m,g),o.append(s,d);let _=document.createElement(`div`);_.className=`profile-note`,_.innerHTML=`
  ...
  `;let b=document.createElement(`dl`);b.className=`profile-facts`;let x=q(e.category_labels);b.append(mn(`Viešas / prekinis pavadinimas`,r),$(`Juridinis pavadinimas`,e.legal_name,`Viešame šaltinyje juridinis pavadinimas nenurodytas.`),$(`Šaltinyje pateikta tapatybė`,e.source_identity),$(`Vietovė šaltinyje`,e.location),$(`Miestas ar vietovė`,e.city),$(`Šaltinio regiono grupė`,e.region_label),$(`Kategorijos`,x.join(`, `),`Kategorijos viešuose šaltiniuose nenurodytos.`),$(`Aprašymas`,e.description_lt,`Trumpas aprašymas šaltiniuose nepateiktas.`),$(`Šaltinyje aprašyta veiklos apimtis`,e.scope_evidence,`Papildomas veiklos apimties aprašymas šaltinyje nepateiktas.`),gn(`Svetainė`,e.website),gn(`Viešai nurodytas kontaktinis adresas`,e.public_contact_url)),v.append(y,b);let S=yn(e),C=vn(e),w=nn(e);a.append(o,bn(e)),S&&a.append(S),a.append(_,v),C&&a.append(C),w&&a.append(w),a.append(Sn(e),xn(e),Cn(e)),t.append(n,a)}function Pn(){if(!V)return;let e=[{question:`Ar katalogo įrašas yra gamintojo rekomendacija?`,answer:`Ne. Katalogas pateikia viešuose šaltiniuose rastus nepatvirtintus kandidatus ir palieka tapatybės, apimties bei pasiūlymo patikrą pirkėjui.`},{question:`Ar galima lyginti tik galutinę pasiūlymo kainą?`,answer:`Ne. Kainą reikia lyginti kartu su medžiagomis, furnitūra, matavimu, projektavimu, pristatymu, montavimu, terminais ir aiškiai nurodytomis išimtimis.`},{question:`Kaip patikrinti siūlomą gamybos terminą?`,answer:`Paprašykite grafiko etapais ir raštu patvirtinkite, nuo kokio įvykio terminas skaičiuojamas, kokios jo prielaidos ir kas nutinka pasikeitus apimčiai.`}];P({title:`Pirkėjo gidas | Baldai pagal užsakymą Lietuvoje`,description:`Lietuviški pirkėjo gidai apie baldų gamintojo pasirinkimą, realistiškas kainų nuorodas, projekto etapus, medžiagas, sutartį, avansą ir garantiją.`,path:`/gidas`,structuredData:[N([{name:`Gamintojų katalogas`,path:`/`},{name:`Pirkėjo gidas`,path:`/gidas`}]),Oe(e)]});let t=e=>`
  ...
  `;V.innerHTML=`
    ...
    ${Z()}
  `}function Fn(e,t){if(!V)return;let n=`/gidas/${e.slug}`;P({title:`${e.title} | Pirkėjo gidas`,description:e.summary,path:n,type:`article`,structuredData:[N([{name:`Gamintojų katalogas`,path:`/`},{name:`Pirkėjo gidas`,path:`/gidas`},{name:e.title,path:n}])]}),V.innerHTML=`
    ${X(`guide`)}
  ...
  `)}function zn(){V&&(P({title:`Gido straipsnis nerastas | Pirkėjo gidas`,description:`Prašomas pirkėjo gido straipsnis nerastas.`,path:window.location.pathname,robots:`noindex, follow`}),V.innerHTML=`
    ${X(`guide`)}
    <main class="article-main">
      <a class="back-link" href="/gidas" data-internal-link="true">← Grįžti į pirkėjo gidą</a>
    ...
    ${Z()}
  `)}function Bn(){let e=Y();if(e===`/gidas`){Pn();return}let t=decodeURIComponent(e.split(`/`).filter(Boolean)[1]??``);if(H.find(e=>e.slug===t)?.featured){if(V?.querySelector(`.guide-article`))return;window.location.replace(`${e}/`);return}if(t===`trumpasis-sarasas`){In();return}if(t===`uzklausa-ir-pasiulymas`){Ln();return}if(t===`terminai`){Rn();return}zn()}function Vn(){P({title:`Baldai pagal užsakymą Lietuvoje | Gamintojų katalogas`,description:`Viešais šaltiniais paremtas nepatvirtintų Lietuvos nestandartinių baldų gamintojų kandidatų katalogas su paieška pagal kategoriją ir vietą.`,path:`/`,robots:window.location.search?`noindex, follow`:`index, follow`})}function Hn(){let e=Bt();if(e){jn(e);return}if(zt()){if(!V)return;gt({root:V,renderHeader:X,renderFooter:Z});return}if(Pt()){if(!V)return;He({root:V,renderHeader:X,renderFooter:Z});return}if(Rt()){if(!V)return;Qe({root:V,renderHeader:X,renderFooter:Z});return}if(Lt()){An();return}if(Ft()){Bn();return}if(window.location.pathname.startsWith(`/gamintojas/`)){let e=decodeURIComponent(window.location.pathname.split(`/`).filter(Boolean)[1]??``);Nn(U.find(t=>t.slug===e));return}if(It()){let e=Y().split(`/`).filter(Boolean).map(e=>decodeURIComponent(e)),t=e[1]??``,n=e.length===4&&e[2]===`miestas`?e[3]:void 0;if(e.length!==2&&!n){Mn(`Paieškos puslapis nerastas`,`Tokio kategorijos ar miesto puslapio nėra. Grįžkite į katalogą ir naudokite paiešką arba filtrus.`);return}kn(t,n);return}if(window.location.pathname!==`/`){Mn(`Tokio puslapio nėra`,`Patikrinkite adresą arba grįžkite į gamintojų katalogą.`);return}Vn(),un(),pn()}async function Un(){if(!Tt){Tt=!0,window.location.pathname===`/`?V?.hasChildNodes()||(un(),dn()):window.location.pathname.startsWith(`/gamintojas/`)?V&&!V.hasChildNodes()&&(V.innerHTML=`
        ${X(`directory`)}
        <main class="profile-main">
          <div class="loading-state" role="status" aria-live="polite">
            <span class="loading-mark" aria-hidden="true"><span></span><span></span><span></span></span>
        ...
        ${Z()}
      `):Lt()&&V&&!V.hasChildNodes()&&(V.innerHTML=`
        ${X(`request`)}
        ...
        ${Z()}
      `);try{U=await Ht(),Et=!0,W=Nt(),Hn()}catch(e){console.error(`Nepavyko gauti gamintojų katalogo.`,e),fn()}finally{Tt=!1}}}function Wn(){if(W=Nt(),Ft()||Vt()||Rt()||zt()){Hn();return}if(Et){Hn();return}Un()}function Gn(e,t,n=!1){let r=e.querySelector(`.navigation-toggle`);if(!r)return;t?e.dataset.menuOpen=`true`:delete e.dataset.menuOpen,r.setAttribute(`aria-expanded`,String(t));let i=r.querySelector(`.navigation-current`)?.textContent?.trim()??``;r.setAttribute(`aria-label`,`${t?`Užverti`:`Atverti`} pagrindinį meniu. Dabartinis skyrius: ${i}`),n&&r.focus()}document.addEventListener(`click`,e=>{let t=e.target;if(!(t instanceof Element))return;let n=t.closest(`.navigation-toggle`);if(n){let e=n.closest(`.site-header`);if(!e)return;Gn(e,n.getAttribute(`aria-expanded`)!==`true`);return}let r=document.querySelector(`.site-header[data-menu-open="true"]`);r&&!r.contains(t)&&Gn(r,!1)}),document.addEventListener(`keydown`,e=>{if(e.key!==`Escape`)return;let t=document.querySelector(`.site-header[data-menu-open="true"]`);t&&(e.preventDefault(),Gn(t,!1,!0))}),document.addEventListener(`click`,e=>{let t=e.target;if(!(t instanceof Element))return;let n=t.closest(`a[data-internal-link="true"]`);if(!n||e.defaultPrevented||e.button!==0||e.metaKey||e.ctrlKey||e.shiftKey||e.altKey)return;let r=new URL(n.href,window.location.origin);r.origin===window.location.origin&&(e.preventDefault(),window.history.pushState({},``,`${r.pathname}${r.search}${r.hash}`),Wn(),window.scrollTo({top:0,behavior:`auto`}))}),window.addEventListener(`popstate`,Wn),Ft()||Vt()||Rt()||zt()?Hn():Un();