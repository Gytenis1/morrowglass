...
Šalių parašų vietos ir data: [įrašykite]`}];function Ne(){return Object.fromEntries(Me.map(e=>[e.id,e.value]))}function Pe(){let e=Ne();try{let t=localStorage.getItem(Oe);if(!t)return e;let n=JSON.parse(t);Me.forEach(t=>{typeof n[t.id]==`string`&&(e[t.id]=n[t.id])})}catch{return e}return e}function Fe({root:e,renderHeader:t,renderFooter:n}){I({title:`Baldų pirkimo sutarties šablonas | Pirkėjo įrankiai`,description:`Naršyklėje redaguojamas ir spausdinamas B2C baldų pirkimo sutarties struktūros šablonas su aiškiu teisinės konsultacijos ribojimu.`,path:Te,type:`article`,structuredData:[F([{name:`Gamintojų katalogas`,path:`/`},{name:`Pirkėjo gidas`,path:`/gidas`},{name:`Baldų pirkimo sutarties šablonas`,path:Te}])]});let r=Pe();e.innerHTML=`
    ${t(`guide`)}
  ...
  `,n.focus()}function Ge({root:e,renderHeader:t,renderFooter:n}){I({title:`Palyginti baldų pasiūlymus | Pirkėjo įrankiai`,description:`Deterministinis 2–5 baldų pasiūlymų palyginimas pagal pirkėjo svorius, aiškią formulę, įrodymus, trūkstamus ir nepalyginamus laukus.`,path:Ee,structuredData:[F([{name:`Gamintojų katalogas`,path:`/`},{name:`Pirkėjo įrankiai`,path:`/gidas`},{name:`Pasiūlymų palyginimas`,path:Ee}])]});let r=ze(),i=()=>{e.innerHTML=`
      ${t(`guide`)}
      <main class="buyer-tool-main comparison-tool-main">
        ...
        <section class="comparison-offers" aria-labelledby="offers-title">
          ...
          <div class="offer-editor-list">
            ${r.offers.map((e,t)=>Ve(e,t,r.offers.length>2)).join(``)}
          </div>
        ...
        <section class="request-form-section" aria-labelledby="request-form-title">
          ...
          <form class="buyer-request-form rfq-form" id="rfq-form" novalidate>
            ...
            <div class="form-field">
              ...
              <div class="select-wrap"><select id="category" name="category" required aria-describedby="error-category">${Je(Ke,`Pasirinkite kategoriją`)}</select></div>
              ${z(`category`)}
            </div>
            <div class="form-field">
            ...
            <div class="form-field form-field--wide">
              ...
              <div class="select-wrap"><select id="project-stage" name="project_stage" required aria-describedby="error-project-stage">${Je(qe,`Pasirinkite dabartinį etapą`)}</select></div>
              ${z(`project-stage`)}
            </div>
            <div class="form-field form-field--wide">
  ...
  `;let c=e.querySelector(`#rfq-form`),l=e.querySelector(`#manufacturer-search`),u=e.querySelector(`#manufacturer-choice-list`),d=e.querySelector(`#manufacturer-empty`),f=e.querySelector(`#manufacturer-selection-count`),p=e.querySelector(`#rfq-status`),m=c?.querySelector(`button[type="submit"]`);if(!c||!l||!u||!d||!f||!p||!m)return;let h=Array.from(u.querySelectorAll(`[name="preferred_shortlist"]`)),g=t=>{let n=h.filter(e=>e.checked);n.length>8&&t?(t.checked=!1,n=h.filter(e=>e.checked),B(e,`preferred_shortlist`,`Galima pasirinkti ne daugiau kaip 8 gamintojų kandidatus.`)):B(e,`preferred_shortlist`),f.textContent=n.length===0?`Kandidatų nepasirinkta`:n.length===1?`Pasirinktas 1 kandidatas iš 8`:`Pasirinkta kandidatų: ${n.length} iš 8`};h.forEach(e=>e.addEventListener(`change`,()=>g(e))),l.addEventListener(`input`,()=>{let e=l.value.trim().toLocaleLowerCase(`lt-LT`),t=0;u.querySelectorAll(`.manufacturer-choice`).forEach(n=>{let r=!e||(n.textContent??``).toLocaleLowerCase(`lt-LT`).includes(e);n.hidden=!r,r&&(t+=1)}),d.hidden=t>0}),c.querySelectorAll(`input, textarea, select`).forEach(t=>{t.name!==`preferred_shortlist`&&(t.setAttribute(`aria-invalid`,`false`),t.addEventListener(`input`,()=>B(e,t.name)),t.addEventListener(`blur`,()=>{t.validity.valid||B(e,t.name,t.validationMessage||`Patikrinkite šį lauką.`)}))}),c.addEventListener(`submit`,async t=>{if(t.preventDefault(),!Xe(e,c))return;let n=new FormData(c),r={full_name:String(n.get(`full_name`)??``).trim(),email:String(n.get(`email`)??``).trim(),phone:String(n.get(`phone`)??``).trim(),category:String(n.get(`category`)??``).trim(),municipality:String(n.get(`municipality`)??``).trim(),service_region:String(n.get(`service_region`)??``).trim(),project_stage:String(n.get(`project_stage`)??``).trim(),project_scope:String(n.get(`project_scope`)??``).trim(),dimensions_room_count:String(n.get(`dimensions_room_count`)??``).trim(),materials_requirements:String(n.get(`materials_requirements`)??``).trim(),budget_min:Number(n.get(`budget_min`)),budget_max:Number(n.get(`budget_max`)),desired_completion_date:String(n.get(`desired_completion_date`)??``),installation_access_constraints:String(n.get(`installation_access_constraints`)??``).trim(),preferred_shortlist:h.filter(e=>e.checked).map(e=>e.value)};m.disabled=!0,m.setAttribute(`aria-busy`,`true`),m.textContent=`Pateikiama…`,p.className=`form-status`,p.textContent=`RFQ kuriamas operatoriaus peržiūrai. Gamintojai nekontaktuojami.`;try{let t=await fetch(`${de.replace(/\/+$/,``)}/api/public/rfqs`,{method:`POST`,credentials:`omit`,headers:{"Content-Type":`application/json`},body:JSON.stringify(r)}),n=await t.json().catch(()=>null);if(!t.ok){if(Ze(e,n)){let t=e.querySelector(`#rfq-error-summary`);if(t){let n=Array.from(e.querySelectorAll(`.field-error`)).map(e=>e.textContent).filter(Boolean);t.hidden=!1,t.innerHTML=`<strong>Serveris paprašė pataisyti:</strong><ul>${n.map(e=>`<li>${L(e??``)}</li>`).join(``)}</ul>`,t.focus()}}throw Error(n?.message||`RFQ pateikti nepavyko.`)}if(!n?.reference)throw Error(`Serveris negrąžino RFQ nuorodos.`);let i=e.querySelector(`#rfq-success`),a=e.querySelector(`#rfq-reference`);a&&(a.textContent=n.reference),i&&(i.hidden=!1,i.focus()),p.className=`form-status form-status--success`,p.textContent=`RFQ ${n.reference} gautas operatoriaus peržiūrai. Gamintojai nebuvo kontaktuoti automatiškai.`,m.textContent=`RFQ pateiktas`,m.removeAttribute(`aria-busy`),c.querySelectorAll(`input, textarea, select`).forEach(e=>{e.disabled=!0});return}catch(e){console.error(`Nepavyko pateikti RFQ.`,e),p.className=`form-status form-status--error`,p.setAttribute(`role`,`alert`),p.textContent=`RFQ pateikti nepavyko. Įvesti duomenys liko formoje. Patikrinkite pažymėtus laukus ir interneto ryšį, tada bandykite dar kartą.`,p.focus()}finally{(!m.disabled||m.textContent!==`RFQ pateiktas`)&&(m.disabled=!1,m.removeAttribute(`aria-busy`),m.textContent=`Pateikti RFQ operatoriaus peržiūrai`)}})}var $e=50,et=[{value:`0`,label:`0 darbuotojų`},{value:`1-9`,label:`1–9 darbuotojai`},{value:`10-49`,label:`10–49 darbuotojai`},{value:`50-249`,label:`50–249 darbuotojai`},{value:`250+`,label:`250 ir daugiau darbuotojų`}],tt=[{value:`iki-1999`,label:`Iki 1999 m.`},{value:`2000-2009`,label:`2000–2009 m.`},{value:`2010-2019`,label:`2010–2019 m.`},{value:`nuo-2020`,label:`2020 m. ir vėliau`}],nt=40,rt=2e3,it=[`Virtuvės baldai`,`Spintos ar įmontuojami baldai`,`Miegamojo ar vonios baldai`,`Biuro ar komerciniai baldai`,`Minkšti baldai`,`Medžio masyvo ar kiti nestandartiniai baldai`,`Kitas projektas`],at=new Intl.Collator(`lt`,{sensitivity:`base`}),V=document.querySelector(`#app`),H=[{slug:`trumpasis-sarasas`,title:`Kaip sudaryti pagrįstą trumpąjį sąrašą`,summary:`Atrankos seka, patikrinami kriterijai ir klausimai prieš priimant pasiūlymą.`,readingLabel:`Atranka ir patikra`},{slug:`uzklausa-ir-pasiulymas`,title:`Kaip parengti užklausą ir palyginti pasiūlymus`,summary:`Ką aprašyti, kad gamintojai vertintų tą pačią apimtį, ir kas dažniausiai keičia kainą.`,readingLabel:`Užklausa ir apimtis`},{slug:`terminai`,title:`Kaip prašyti realistiško darbų grafiko`,summary:`Terminą lemiantys kintamieji, etapai ir klausimai, padedantys valdyti neapibrėžtumą.`,readingLabel:`Terminai ir eiga`},{slug:`kaip-pasirinkti-baldu-gamintoja`,title:`Kaip pasirinkti ir palyginti baldų gamintoją`,summary:`Ką paklausti, kokius įspėjamuosius ženklus pastebėti ir kaip atsargiai skaityti nepatvirtintus katalogo įrašus.`,readingLabel:`Atranka ir patikra`,featured:!0},{slug:`virtuves-baldu-kainos`,title:`Virtuvės baldų kainos: ribos ir kainą keičiantys sprendimai`,summary:`Dvi aiškiai atskirtos viešų šaltinių nuorodos, jų datos ir praktinis sąrašas, kas keičia individualaus projekto kainą.`,readingLabel:`Kaina ir apimtis`,featured:!0},{slug:`virtuves-ir-imontuojamu-baldu-projekto-eiga`,title:`Virtuvės ir įmontuojamų baldų projekto eiga`,summary:`Tipinė etapų seka nuo matavimo iki montavimo ir kontrolinis sąrašas sprendimams, kurie veikia grafiką.`,readingLabel:`Projekto eiga`,featured:!0},{slug:`medziagos-sutartis-avansas-garantija`,title:`Medžiagos, sutartis, avansas ir garantija: ką aptarti`,summary:`LMDP ir MDF, faneruotės, masyvo, stalviršių, furnitūros, briaunų, sutarties ir garantinio aptarnavimo klausimai.`,readingLabel:`Dokumentai ir atsakomybės`,featured:!0},{slug:`spintos-ir-drabuzines-kaina`,title:`Spintos ir drabužinės kaina: ką apibrėžti prieš lyginant pasiūlymus`,summary:`Apimtis, vidaus įranga ir montavimo sąlygos, kurios padeda palyginti pasiūlymus.`,readingLabel:`Spintos ir drabužinės`,featured:!0,buyerIntent:!0},{slug:`mdf-faneruote-masyvas-fasadai`,title:`MDF, faneruotė ar masyvas fasadams: klausimai prieš pasirenkant`,summary:`Fasadų specifikacijos, pavyzdžiai, priežiūra ir kompromisai.`,readingLabel:`Fasadai ir medžiagos`,featured:!0,buyerIntent:!0},{slug:`kvarcas-ar-akmuo-stalvirsiui`,title:`Kvarcas ar natūralus akmuo stalviršiui: apimtis, priežiūra ir klausimai`,summary:`Šablonavimo, išpjovų, sujungimų ir montavimo kontrolinis sąrašas.`,readingLabel:`Stalviršiai`,featured:!0,buyerIntent:!0},{slug:`matavimas-ir-montavimas-kontrole`,title:`Galutinis matavimas ir montavimo diena: kontrolinis sąrašas`,summary:`Objekto parengtis, dokumentai ir priėmimo patikra.`,readingLabel:`Matavimas ir montavimas`,featured:!0,buyerIntent:!0},{slug:`baldu-defektai-ir-garantinis-aptarnavimas`,title:`Baldų defektai ir garantinis aptarnavimas: kaip fiksuoti ir sekti`,summary:`Dokumentavimas, pranešimas ir sutartos korekcijos sekimas.`,readingLabel:`Defektai ir aptarnavimas`,featured:!0,buyerIntent:!0},{slug:`mazo-buto-irengimas-pagal-uzsakyma`,title:`Mažo buto įrengimas pagal užsakymą: prioritetai ir užklausos sąrašas`,summary:`Saugojimo, judėjimo, matavimo ir montavimo prioritetai.`,readingLabel:`Mažas butas`,featured:!0,buyerIntent:!0}],ot=[{path:`/privatumas`,title:`Privatumo pranešimas | Baldai pagal užsakymą Lietuvoje`,heading:`Privatumo pranešimas`,description:`Kaip GG Ventures UAB tvarko Baldininkai.org katalogo užklausų, atsiliepimų ir įrašų pataisymo ar atstovavimo formų asmens duomenis.`,summary:`Šiame pranešime paaiškiname, kokius asmens duomenis gauname per katalogo formas, kam juos naudojame, kiek laiko saugome ir kokias teises turite.`,content:`
      ...
      <section><h2>Duomenys ir kontaktas</h2><p>Prašymo informaciją naudojame tik katalogo peržiūrai, ryšiui ir galimiems teisiniams poreikiams pagal <a href="/privatumas" data-internal-link="true">privatumo pranešimą</a>. Tvarką administruoja <strong>GG Ventures UAB</strong>, įmonės kodas <strong>305442420</strong>; kontaktinis el. paštas <a href="mailto:info@baldininkai.org">info@baldininkai.org</a>.</p></section>`}],U=[],W=mt(),st=!1,ct=!1;function lt(e){return e.trim().toLocaleLowerCase(`lt-LT`)}function G(e){return e.replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`).replace(/"/g,`&quot;`).replace(/'/g,`&#039;`)}function K(e,t=`Viešuose šaltiniuose nenurodyta.`){return e?.trim()||t}function ut(e){return{0:`Viešame darbuotojų skaičiaus įraše – 0 darbuotojų`,"1-9":`Labai maža komanda – 1–9 darbuotojai`,"10-49":`Nedidelė įmonė – 10–49 darbuotojai`,"50-249":`Didesnė įmonė – 50–249 darbuotojai`,"250+":`Didelė įmonė – 250 ar daugiau darbuotojų`}[e]??`${e} darbuotojų (viešo šaltinio grupė)`}function dt(e){let t=e%100,n=e%10;return t>=11&&t<=19||n===0?`${e} patvirtintų atsiliepimų`:n===1?`${e} patvirtintas atsiliepimas`:`${e} patvirtinti atsiliepimai`}function ft(e){let t=e?new Date(e):null;return!t||Number.isNaN(t.getTime())?null:{iso:t.toISOString(),label:new Intl.DateTimeFormat(`lt-LT`,{year:`numeric`,month:`long`,day:`numeric`}).format(t)}}function pt(e){let t=e?.trim()??``,n=/^(\d{4})-(\d{2})-(\d{2})$/.exec(t);if(!n)return null;let r=Number(n[1]),i=Number(n[2]),a=Number(n[3]),o=new Date(Date.UTC(r,i-1,a));return o.getUTCFullYear()!==r||o.getUTCMonth()!==i-1||o.getUTCDate()!==a?null:{iso:t,label:new Intl.DateTimeFormat(`lt-LT`,{year:`numeric`,month:`long`,day:`numeric`,timeZone:`UTC`}).format(o)}}function q(e){return Array.isArray(e)?e.filter(e=>typeof e==`string`&&e.trim().length>0):[]}function mt(){let e=new URLSearchParams(window.location.search);return{query:e.get(`q`)?.trim()??``,category:e.get(`kategorija`)??``,city:e.get(`miestas`)??``,region:e.get(`regionas`)??``,employeeBand:e.get(`dydis`)??``,foundedPeriod:e.get(`ikurta`)??``,registryCheckedOnly:e.get(`registras`)===`patikrinta`}}function J(e){let t=new URLSearchParams;W.query&&t.set(`q`,W.query),W.category&&t.set(`kategorija`,W.category),W.city&&t.set(`miestas`,W.city),W.region&&t.set(`regionas`,W.region),W.employeeBand&&t.set(`dydis`,W.employeeBand),W.foundedPeriod&&t.set(`ikurta`,W.foundedPeriod),W.registryCheckedOnly&&t.set(`registras`,`patikrinta`);let n=t.toString(),r=`${window.location.pathname}${n?`?${n}`:``}`;r!==`${window.location.pathname}${window.location.search}`&&window.history[e===`push`?`pushState`:`replaceState`]({},``,r)}function Y(e=window.location.pathname){return e.replace(/\/index\.html$/,``).replace(/\/+$/,``)||`/`}function ht(e=window.location.pathname){return Y(e)===`/gidas/baldu-pirkimo-sutarties-sablonas`}function gt(e=window.location.pathname){let t=Y(e);return t===`/gidas`||t.startsWith(`/gidas/`)}function _t(e=window.location.pathname){return Y(e).startsWith(`/baldai-pagal-uzsakyma/`)}function vt(e=window.location.pathname){return Y(e)===`/gauti-pasiulymus`}function yt(e=window.location.pathname){return Y(e)===`/palyginti-pasiulymus`}function bt(e=window.location.pathname){let t=e.replace(/\/+$/,``)||`/`;return ot.find(e=>e.path===t)}function xt(e=window.location.pathname){return!!bt(e)}async function St(){let e=[],t=1,n=1;do{let r=await ue.collection(`manufacturers`).getList(t,$e,{sort:`trading_name`,requestKey:`manufacturers-page-${t}`});e.push(...r.items),n=r.totalPages,t+=1}while(t<=n);return e.sort((e,t)=>at.compare(e.trading_name,t.trading_name))}function Ct(e){let t=new Map;return e.forEach(e=>{q(e.category_codes).forEach((n,r)=>{let i=q(e.category_labels)[r];n&&i&&t.set(n,i)})}),Array.from(t,([e,t])=>({value:e,label:t})).sort((e,t)=>at.compare(e.label,t.label))}function wt(e,t,n){let r=new Map;return e.forEach(e=>{let i=e[t]?.trim(),a=e[n]?.trim();i&&a&&r.set(i,a)}),Array.from(r,([e,t])=>({value:e,label:t})).sort((e,t)=>at.compare(e.label,t.label))}function Tt(e=[],t=[],n=[],r=!0){let i=(e,t)=>e.some(e=>e.value===t),a=!1;r&&W.category&&!i(e,W.category)&&(W.category=``,a=!0),r&&W.city&&!i(t,W.city)&&(W.city=``,a=!0),r&&W.region&&!i(n,W.region)&&(W.region=``,a=!0),W.employeeBand&&!i(et,W.employeeBand)&&(W.employeeBand=``,a=!0),W.foundedPeriod&&!i(tt,W.foundedPeriod)&&(W.foundedPeriod=``,a=!0),a&&J(`replace`)}function Et(e,t){return t?Number.isInteger(e)?t===`iki-1999`?Number(e)<=1999:t===`2000-2009`?Number(e)>=2e3&&Number(e)<=2009:t===`2010-2019`?Number(e)>=2010&&Number(e)<=2019:t===`nuo-2020`?Number(e)>=2020:!1:!1:!0}function Dt(e){return lt(e.financial_verification_status??``)===`patikrinta`}function Ot(e,t=!0){let n=lt(W.query);return e.filter(e=>{let r=!n||[e.trading_name,e.legal_name??``,e.source_identity,e.description_lt,...q(e.category_labels),...q(e.category_codes)].some(e=>lt(e??``).includes(n)),i=!t||!W.category||q(e.category_codes).includes(W.category),a=!t||!W.city||e.city===W.city,o=!t||!W.region||e.region===W.region,s=!W.employeeBand||e.employee_count_band?.trim()===W.employeeBand,c=Et(e.founded_year,W.foundedPeriod),l=!W.registryCheckedOnly||Dt(e);return r&&i&&a&&o&&s&&c&&l})}function kt(){return!!(W.employeeBand||W.foundedPeriod||W.registryCheckedOnly)}function At(){W={query:``,category:``,city:``,region:``,employeeBand:``,foundedPeriod:``,registryCheckedOnly:!1}}function jt(e){let t=e%100,n=e%10;return t>=11&&t<=19?`${e} gamintojų kandidatų`:n===1?`${e} gamintojas kandidatas`:n>=2&&n<=9?`${e} gamintojai kandidatai`:`${e} gamintojų kandidatų`}function Mt(e){let t=e.legal_name?.trim()??``;if(!t)return``;let n=e=>lt(e).replace(/\b(uab|ab|mb|všį|iį|įi|kib|tūb)\b/g,``).replace(/[^\p{L}\p{N}]+/gu,` `).trim(),r=n(t),i=n(e.trading_name);return!r||r===i||r.includes(i)||i.includes(r)?``:t}function X(t){return`
  ...
  `}function Nt(e){let t=new Set(q(e.category_codes));return t.has(`W`)?{slug:`spintos-ir-drabuzines-kaina`,label:`Spintų ir drabužinių kainos bei apimties klausimai`}:t.has(`K`)?{slug:`kvarcas-ar-akmuo-stalvirsiui`,label:`Stalviršio medžiagos ir apimties klausimai`}:t.has(`OC`)||t.has(`HR`)?{slug:`matavimas-ir-montavimas-kontrole`,label:`Matavimo ir montavimo kontrolinis sąrašas`}:t.has(`SW`)?{slug:`mdf-faneruote-masyvas-fasadai`,label:`Medžiagų ir apdailos klausimai`}:{slug:`matavimas-ir-montavimas-kontrole`,label:`Matavimo ir montavimo kontrolinis sąrašas`}}function Pt(e){let t=[],n=e.city?.trim(),r=n?ye(U).find(e=>e.city===n):void 0;r&&t.push({kind:`Miestas`,slug:r.slug,label:`Baldų gamintojų kandidatai: ${r.city}`});let i=new Set(q(e.category_codes));return he.forEach(e=>{i.has(e.code)&&t.push({kind:`Kategorija`,slug:e.slug,label:e.title})}),t}function Ft(e){let t=Pt(e);if(!t.length)return null;let n=document.createElement(`section`);return n.className=`profile-landings`,n.setAttribute(`aria-labelledby`,`profile-landings-title`),n.innerHTML=`
    <div class="section-heading">
  ...
  `,n}function It(){let e=ye(U);return`
    <section class="landing-directory" aria-labelledby="landing-directory-title">
      ...
      <div class="landing-link-groups">
        <div>
          ...
          <ul>
            ${he.map(e=>`<li><a href="/baldai-pagal-uzsakyma/${e.slug}" data-internal-link="true">${G(e.title)}</a></li>`).join(``)}
          </ul>
        </div>
        ${e.length?`
          <div>
  ...
  `)}function Wt(){V&&(I({title:`Katalogas nepasiekiamas | Baldai pagal užsakymą Lietuvoje`,description:`Gamintojų katalogo duomenų šiuo metu nepavyko gauti.`,path:window.location.pathname,robots:`noindex, follow`}),V.innerHTML=`
    ${X(`directory`)}
    ...
    ${Z()}
  `,document.querySelector(`#retry-button`)?.addEventListener(`click`,()=>void yn()))}function Gt(){let e=document.querySelector(`#browse-content`);if(!e)return;let t=Ct(U),n=wt(U,`city`,`city`),r=wt(U,`region`,`region_label`);Tt(t,n,r),e.replaceChildren();let i=document.createElement(`div`);i.className=`browse-controls`;let a=document.createElement(`div`);a.className=`controls-heading`,a.innerHTML=`
    <div>
  ...
  `;let o=document.createElement(`form`);o.className=`filter-form`,o.setAttribute(`role`,`search`),o.addEventListener(`submit`,e=>e.preventDefault());let s=document.createElement(`div`);s.className=`filter-field filter-field--search`;let c=document.createElement(`label`);c.htmlFor=`directory-search`,c.textContent=`Ieškoti kataloge`;let l=document.createElement(`input`);l.id=`directory-search`,l.name=`paieska`,l.type=`search`,l.autocomplete=`off`,l.placeholder=`Pavadinimas, aprašymas ar kategorija`,l.value=W.query,s.append(c,l);let u=document.createElement(`div`);u.className=`filter-grid filter-grid--primary`,u.append(s,Lt(`category-filter`,`Baldų kategorija`,`Visos kategorijos`,t,W.category),Lt(`city-filter`,`Miestas`,`Visi miestai`,n,W.city),Lt(`region-filter`,`Šaltinio regiono grupė`,`Visos regiono grupės`,r,W.region));let d=zt(`directory`),f=document.createElement(`div`);f.className=`filter-actions`;let p=document.createElement(`p`);p.textContent=`Paieška atnaujinama iškart vedant tekstą.`;let m=document.createElement(`button`);m.className=`text-button`,m.id=`clear-filters`,m.type=`button`,m.textContent=`Išvalyti paiešką ir filtrus`,f.append(p,m),o.append(u,d.fields,f),i.append(a,o);let h=document.createElement(`div`);h.className=`results-area`,h.id=`results-area`,e.append(i,h),l.addEventListener(`input`,()=>{W.query=l.value.trimStart(),J(`replace`),Q()}),document.querySelector(`#category-filter`)?.addEventListener(`change`,e=>{W.category=e.currentTarget.value,J(`push`),Q()}),document.querySelector(`#city-filter`)?.addEventListener(`change`,e=>{W.city=e.currentTarget.value,J(`push`),Q()}),document.querySelector(`#region-filter`)?.addEventListener(`change`,e=>{W.region=e.currentTarget.value,J(`push`),Q()}),d.employeeBandSelect.addEventListener(`change`,()=>{W.employeeBand=d.employeeBandSelect.value,J(`push`),Q()}),d.foundedPeriodSelect.addEventListener(`change`,()=>{W.foundedPeriod=d.foundedPeriodSelect.value,J(`push`),Q()}),d.registryToggle.addEventListener(`change`,()=>{W.registryCheckedOnly=d.registryToggle.checked,J(`push`),Q()}),m.addEventListener(`click`,()=>{At(),J(`push`),Gt(),document.querySelector(`#directory-search`)?.focus()}),Q()}function Q(){let e=document.querySelector(`#results-area`);if(!e)return;window.location.pathname===`/`&&_n();let t=Ot(U),n=!!(W.query||W.category||W.city||W.region||kt());e.replaceChildren();let r=document.createElement(`div`);r.className=`result-header`;let i=document.createElement(`p`);if(i.className=`result-count`,i.setAttribute(`role`,`status`),i.setAttribute(`aria-live`,`polite`),i.textContent=n?`Rodoma įrašų: ${t.length}. Iš viso kataloge: ${U.length}.`:`Kataloge – ${jt(U.length)}.`,r.append(i),n){let e=document.createElement(`p`);e.className=`active-filters`,e.textContent=`Aktyvi atranka pagal pasirinktus kriterijus`,r.append(e)}if(e.append(r),t.length===0){let t=document.createElement(`div`);t.className=`message-state`;let n=document.createElement(`p`);n.className=`state-label`,n.textContent=`Rezultatų nėra`;let r=document.createElement(`h3`);r.textContent=`Pagal šiuos kriterijus įrašų nerasta`;let i=document.createElement(`p`);i.textContent=`Pakeiskite paieškos žodį, pasirinkite platesnę vietovę arba išvalykite filtrus.`;let a=document.createElement(`button`);a.className=`primary-button`,a.type=`button`,a.textContent=`Išvalyti visus kriterijus`,a.addEventListener(`click`,()=>{At(),J(`push`),Gt()}),t.append(n,r,i,a),e.append(t);return}let a=document.createElement(`div`);a.className=`manufacturer-list`,t.forEach(e=>a.append(Vt(e))),e.append(a)}function $(e,t){let n=document.createElement(`div`),r=document.createElement(`dt`);r.textContent=e;let i=document.createElement(`dd`);return i.textContent=t,n.append(r,i),n}function Kt(e,t){let n=document.createElement(`div`),r=document.createElement(`dt`);r.textContent=`Įmonės dydžio signalas`;let i=document.createElement(`dd`),a=document.createElement(`strong`);a.textContent=ut(t);let o=document.createElement(`p`);o.className=`fact-explanation`,o.textContent=`Tai viešame įmonės įraše nurodyta darbuotojų skaičiaus grupė. Ji neparodo darbų kokybės, dabartinio užimtumo ar galimybės priimti jūsų projektą.`,i.append(a,o);let s=q(e.public_details_source_urls).filter(e=>e.includes(`rekvizitai.vz.lt`));if(s.length){let e=document.createElement(`ul`);e.className=`fact-source-list`,s.forEach((t,n)=>{let r=document.createElement(`li`),i=document.createElement(`a`);i.href=t,i.target=`_blank`,i.rel=`noopener noreferrer`,i.textContent=s.length===1?`Atverti viešą darbuotojų skaičiaus šaltinį`:`Atverti viešą šaltinį ${n+1}`,r.append(i),e.append(r)}),i.append(e)}return n.append(r,i),n}function qt(e,t){let n=document.createElement(`div`),r=document.createElement(`dt`);r.textContent=e;let i=document.createElement(`dd`),a=t?.trim();if(a){let e=document.createElement(`a`);e.href=a,e.target=`_blank`,e.rel=`noopener noreferrer`,e.textContent=a,i.append(e)}else i.textContent=`Viešuose šaltiniuose nenurodyta.`,i.className=`unknown-value`;return n.append(r,i),n}function Jt(e,t){let n=document.createElement(`div`),r=document.createElement(`dt`);r.textContent=e;let i=document.createElement(`dd`),a=document.createElement(`a`);return a.href=`tel:${t.replace(/[^+\d]/g,``).replace(/(?!^)\+/g,``)}`,a.textContent=t,i.append(a),n.append(r,i),n}function Yt(e){let t=[],n=e.company_code?.trim(),r=e.public_phone?.trim(),i=e.street_address?.trim(),a=e.postcode?.trim(),o=e.employee_count_band?.trim();if(n&&t.push($(`Įmonės kodas`,n)),i&&t.push($(`Registracijos adresas`,i)),a&&t.push($(`Pašto kodas`,a)),Number.isInteger(e.founded_year)&&t.push($(`Įkurta`,String(e.founded_year))),o&&t.push(Kt(e,o)),r&&t.push(Jt(`Viešas telefono numeris`,r)),!t.length)return null;let s=document.createElement(`section`);s.className=`profile-details profile-public-details`,s.setAttribute(`aria-labelledby`,`profile-public-details-title`),s.innerHTML=`
    <div class="section-heading">
  ...
  `}function rn(e,t){let n=document.querySelector(`#landing-filter-controls`),r=document.querySelector(`#landing-manufacturer-list`);if(!n||!r)return;Tt([],[],[],!1),n.replaceChildren();let i=document.createElement(`form`);i.className=`filter-form landing-filter-form`,i.setAttribute(`role`,`search`),i.addEventListener(`submit`,e=>e.preventDefault());let a=document.createElement(`div`);a.className=`filter-field filter-field--search`;let o=document.createElement(`label`);o.htmlFor=`landing-search`,o.textContent=`Ieškoti šiame sąraše`;let s=document.createElement(`input`);s.id=`landing-search`,s.name=`paieska`,s.type=`search`,s.autocomplete=`off`,s.placeholder=`Pavadinimas, aprašymas ar kategorija`,s.value=W.query,a.append(o,s);let c=zt(`landing`),l=document.createElement(`div`);l.className=`filter-grid filter-grid--landing`,l.append(a,...Array.from(c.fields.children));let u=document.createElement(`div`);u.className=`filter-actions`;let d=document.createElement(`p`);d.className=`landing-filter-count`,d.setAttribute(`role`,`status`),d.setAttribute(`aria-live`,`polite`);let f=document.createElement(`button`);f.className=`text-button`,f.type=`button`,f.textContent=`Išvalyti šio sąrašo filtrus`,u.append(d,f),i.append(l,u),n.append(i);let p=()=>{At(),J(`push`),s.value=``,c.employeeBandSelect.value=``,c.foundedPeriodSelect.value=``,c.registryToggle.checked=!1,m(),s.focus()},m=()=>{t();let n=Ot(e,!1),i=!!(W.query||kt());if(d.textContent=i?`Rodoma įrašų: ${n.length}. Iš viso šiame sąraše: ${e.length}.`:`Šiame sąraše – ${jt(e.length)}.`,r.replaceChildren(),!n.length){let e=document.createElement(`div`);e.className=`message-state landing-empty-state`;let t=document.createElement(`h3`);t.textContent=`Pagal šiuos kriterijus įrašų nerasta`;let n=document.createElement(`p`);n.textContent=`Pasirinkite platesnį įmonės dydį ar įkūrimo laikotarpį, pakeiskite paiešką arba išvalykite filtrus.`;let i=document.createElement(`button`);i.className=`primary-button`,i.type=`button`,i.textContent=`Išvalyti šio sąrašo filtrus`,i.addEventListener(`click`,p),e.append(t,n,i),r.append(e);return}n.forEach(e=>r.append(Vt(e)))};s.addEventListener(`input`,()=>{W.query=s.value.trimStart(),J(`replace`),m()}),c.employeeBandSelect.addEventListener(`change`,()=>{W.employeeBand=c.employeeBandSelect.value,J(`push`),m()}),c.foundedPeriodSelect.addEventListener(`change`,()=>{W.foundedPeriod=c.foundedPeriodSelect.value,J(`push`),m()}),c.registryToggle.addEventListener(`change`,()=>{W.registryCheckedOnly=c.registryToggle.checked,J(`push`),m()}),f.addEventListener(`click`,p),m()}function an(e){if(!V)return;let t=he.find(t=>t.slug===e),n=ye(U),r=n.find(t=>t.slug===e);if(!t&&!r){cn(`Paieškos puslapis nerastas`,`Tokio kategorijos ar miesto puslapio nėra. Grįžkite į katalogą ir naudokite paiešką arba filtrus.`);return}let i=t?U.filter(e=>q(e.category_codes).includes(t.code)):U.filter(e=>e.city===r?.city),a=t?t.title:`Baldų gamintojų kandidatai: ${r?.city}`,o=t?t.intro:`Čia pateikiami ${i.length} nepatvirtinti baldų gamintojų kandidatai, kurių viešo šaltinio įraše kaip bazės miestas ar vietovė nurodytas ${r?.city}.`,s=t?t.buyer_note:`Šis sąrašas nepatvirtina, kad kandidatai aptarnauja visą miestą ar aplinkinį regioną. Matavimo, pristatymo ir montavimo vietas patikrinkite tiesiogiai.`,c=t?en(t):tn(r?.city??`šiame mieste`),l=`/baldai-pagal-uzsakyma/${e}`,u=t?`${t.title}: ${i.length} viešais šaltiniais paremti nepatvirtinti Lietuvos gamintojų kandidatai, miestai ir atrankos gairės.`:`${r?.city}: ${i.length} viešuose šaltiniuose šiame mieste registruoti baldų gamintojų kandidatai. Sąrašas nėra paslaugų teritorijos ar kokybės garantija.`,d=()=>{I({title:`${a} | Gamintojų katalogas`,description:u,path:l,robots:window.location.search?`noindex, follow`:`index, follow`,structuredData:[F([{name:`Gamintojų katalogas`,path:`/`},{name:a,path:l}]),Ce(c),we(i)]})};d();let f=t?n.map(e=>({...e,count:i.filter(t=>t.city===e.city).length})).filter(e=>e.count>0).sort((e,t)=>t.count-e.count||at.compare(e.city,t.city)):[],p=r?he.map(e=>({...e,count:i.filter(t=>q(t.category_codes).includes(e.code)).length})).filter(e=>e.count>0):[];V.innerHTML=`
    ${X(`directory`)}
    ...
    ${Z()}
  `,rn(i,d)}function on(){V&&Qe({root:V,renderHeader:X,renderFooter:Z,manufacturers:U})}function sn(e){V&&(I({title:e.title,description:e.description,path:e.path,structuredData:[F([{name:`Gamintojų katalogas`,path:`/`},{name:e.heading,path:e.path}])]}),V.innerHTML=`
    ${X(`policy`)}
    ...
    ${Z()}
  `)}function cn(e,t){V&&(I({title:`${e} | Baldai pagal užsakymą Lietuvoje`,description:t,path:window.location.pathname,robots:`noindex, follow`}),V.innerHTML=`
    ${X(`directory`)}
    <main class="profile-main">
    ...
    ${Z()}
  `)}function ln(e){if(!V)return;V.innerHTML=`
    ${X(`directory`)}
  ...
  `;V.innerHTML=`
    ...
    ${Z()}
  `}function dn(e,t){if(!V)return;let n=`/gidas/${e.slug}`;I({title:`${e.title} | Pirkėjo gidas`,description:e.summary,path:n,type:`article`,structuredData:[F([{name:`Gamintojų katalogas`,path:`/`},{name:`Pirkėjo gidas`,path:`/gidas`},{name:e.title,path:n}])]}),V.innerHTML=`
    ${X(`guide`)}
  ...
  `)}function hn(){V&&(I({title:`Gido straipsnis nerastas | Pirkėjo gidas`,description:`Prašomas pirkėjo gido straipsnis nerastas.`,path:window.location.pathname,robots:`noindex, follow`}),V.innerHTML=`
    ${X(`guide`)}
    <main class="article-main">
      <a class="back-link" href="/gidas" data-internal-link="true">← Grįžti į pirkėjo gidą</a>
    ...
    ${Z()}
  `)}function gn(){let e=Y();if(e===`/gidas`){un();return}let t=decodeURIComponent(e.split(`/`).filter(Boolean)[1]??``);if(H.find(e=>e.slug===t)?.featured){if(V?.querySelector(`.guide-article`))return;window.location.replace(`${e}/`);return}if(t===`trumpasis-sarasas`){fn();return}if(t===`uzklausa-ir-pasiulymas`){pn();return}if(t===`terminai`){mn();return}hn()}function _n(){I({title:`Baldai pagal užsakymą Lietuvoje | Gamintojų katalogas`,description:`Viešais šaltiniais paremtas nepatvirtintų Lietuvos nestandartinių baldų gamintojų kandidatų katalogas su paieška pagal kategoriją ir vietą.`,path:`/`,robots:window.location.search?`noindex, follow`:`index, follow`})}function vn(){let e=bt();if(e){sn(e);return}if(ht()){if(!V)return;Fe({root:V,renderHeader:X,renderFooter:Z});return}if(yt()){if(!V)return;Ge({root:V,renderHeader:X,renderFooter:Z});return}if(vt()){on();return}if(gt()){gn();return}if(window.location.pathname.startsWith(`/gamintojas/`)){let e=decodeURIComponent(window.location.pathname.split(`/`).filter(Boolean)[1]??``);ln(U.find(t=>t.slug===e));return}if(_t()){an(decodeURIComponent(window.location.pathname.split(`/`).filter(Boolean)[1]??``));return}if(window.location.pathname!==`/`){cn(`Tokio puslapio nėra`,`Patikrinkite adresą arba grįžkite į gamintojų katalogą.`);return}_n(),Ht(),Gt()}async function yn(){if(!st){st=!0,window.location.pathname===`/`?V?.hasChildNodes()||(Ht(),Ut()):window.location.pathname.startsWith(`/gamintojas/`)?V&&!V.hasChildNodes()&&(V.innerHTML=`
        ${X(`directory`)}
        <main class="profile-main">
          <div class="loading-state" role="status" aria-live="polite">
            <span class="loading-mark" aria-hidden="true"><span></span><span></span><span></span></span>
        ...
        ${Z()}
      `):vt()&&V&&!V.hasChildNodes()&&(V.innerHTML=`
        ${X(`request`)}
        ...
        ${Z()}
      `);try{U=await St(),ct=!0,W=mt(),vn()}catch(e){console.error(`Nepavyko gauti gamintojų katalogo.`,e),Wt()}finally{st=!1}}}function bn(){if(W=mt(),gt()||xt()||yt()){vn();return}if(ct){vn();return}yn()}document.addEventListener(`click`,e=>{let t=e.target;if(!(t instanceof Element))return;let n=t.closest(`a[data-internal-link="true"]`);if(!n||e.defaultPrevented||e.button!==0||e.metaKey||e.ctrlKey||e.shiftKey||e.altKey)return;let r=new URL(n.href,window.location.origin);r.origin===window.location.origin&&(e.preventDefault(),window.history.pushState({},``,`${r.pathname}${r.search}${r.hash}`),bn(),window.scrollTo({top:0,behavior:`auto`}))}),window.addEventListener(`popstate`,bn),gt()||xt()||yt()?vn():yn();