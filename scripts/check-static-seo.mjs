...
const FINANCIAL_DATASET_CSV_URL = `${SITE_URL}/baldininkai-org-gamintoju-finansai.csv`;
...
const FILED_FINANCE_PATH = '/baldu-sektoriaus-finansai';
...
const ENGLISH_QUOTE_REQUEST_URL = `${SITE_URL}${ENGLISH_QUOTE_REQUEST_PATH}/`;
...
const employeeBands = new Set(ENGLISH_EMPLOYEE_BAND_LABELS.keys());
...
function expectedFiledFinancialYearGroups(records, minimumYear = 2019) {
...
function expectedFiledFinancialHistory(record) {
...
function expectedDatasetFinancialFields(record) {
  ...
  const financialHistory = expectedFiledFinancialHistory(record);
  return {
    ...(turnover && {
    ...
    ...(employeeBands.has(record.employee_count_band) && { employee_count_band: record.employee_count_band }),
...
function expectedDatasetRecord(record) {
  return {
    ...
    ...expectedDatasetFinancialFields(record),
  };
}

function expectedFinancialDatasetRows() {
  return manufacturers.flatMap((record) => expectedFiledFinancialHistory(record).map((period) => ({
    slug: record.slug,
    profile_url: `${SITE_URL}/gamintojas/${record.slug}/`,
    trading_name: record.trading_name?.trim() || null,
    legal_name: record.legal_name?.trim() || null,
...
async function checkGeneratedAssets() {
  ...
  if (!generatedDatasetMetadata || typeof generatedDatasetMetadata !== 'object' || Array.isArray(generatedDatasetMetadata)) {
    ...
    const expectedMetadataKeys = ['attribution', 'generated_date', 'license', 'official_sources', 'open_data_url', 'record_count', 'source', 'source_url', 'version'];
    if (JSON.stringify(Object.keys(generatedDatasetMetadata).sort()) !== JSON.stringify(expectedMetadataKeys)) addError('/baldininkai-org-gamintojai.json', 'metadata keys do not match the documented licence/source/count structure');
    ...
    if (generatedDatasetMetadata.record_count !== manufacturers.length) addError('/baldininkai-org-gamintojai.json', `metadata record_count must equal ${manufacturers.length}`);
    ...
    const expectedOfficialSources = [{ name: OFFICIAL_FINANCIAL_SOURCE_NAME, url: OFFICIAL_FINANCIAL_SOURCE_URL, api_model_path: OFFICIAL_FINANCIAL_API_MODEL_PATH }];
    if (JSON.stringify(generatedDatasetMetadata.official_sources) !== JSON.stringify(expectedOfficialSources)) addError('/baldininkai-org-gamintojai.json', 'metadata official_sources must name the Lithuanian register/open-data financial source and API model');
  ...
  const expectedFinancialRows = expectedFinancialDatasetRows();
  try {
    const csv = await readFile(join(publicDir, 'baldininkai-org-gamintoju-finansai.csv'), 'utf8');
    const lines = csv.split(/\r?\n/);
    const metadataLines = [];
    while (lines[0]?.startsWith('#')) metadataLines.push(lines.shift());
    const expectedMetadataLines = [
      ...
      `# Records: ${manufacturers.length}`,
      `# Official source: ${OFFICIAL_FINANCIAL_SOURCE_NAME} - ${OFFICIAL_FINANCIAL_SOURCE_URL} - API model: ${OFFICIAL_FINANCIAL_API_MODEL_PATH}`,
      `# Financial periods: ${expectedFinancialRows.length}`,
    ];
    if (JSON.stringify(metadataLines) !== JSON.stringify(expectedMetadataLines)) addError('/baldininkai-org-gamintoju-finansai.csv', 'comment metadata must contain the exact licence, attribution, date/version, source and financial-period lines before the header');
    const rows = parseCsv(lines.join('\n'));
    const [headers, ...dataRows] = rows;
    if (JSON.stringify(headers) !== JSON.stringify(financialDatasetHeaders)) addError('/baldininkai-org-gamintoju-finansai.csv', 'headers do not match the normalized financial-period header order');
    if (dataRows.length !== expectedFinancialRows.length) addError('/baldininkai-org-gamintoju-finansai.csv', `expected ${expectedFinancialRows.length} data rows, found ${dataRows.length}`);
    dataRows.forEach((row, index) => {
      const fileLine = metadataLines.length + index + 2;
      if (row.length !== financialDatasetHeaders.length) addError('/baldininkai-org-gamintoju-finansai.csv', `row ${fileLine} has ${row.length} fields; expected ${financialDatasetHeaders.length}`);
      const expected = expectedFinancialRows[index];
      if (!expected) return;
      const expectedRow = financialDatasetHeaders.map((header) => Array.isArray(expected[header]) ? expected[header].join(' | ') : expected[header] == null ? '' : String(expected[header]));
      if (JSON.stringify(row) !== JSON.stringify(expectedRow)) addError('/baldininkai-org-gamintoju-finansai.csv', `row ${fileLine} does not match source-derived period ${expected.slug} ${expected.fiscal_period_end}`);
      for (const cell of row) if (/^[{[]/.test(cell.trim())) addError('/baldininkai-org-gamintoju-finansai.csv', `row ${fileLine} contains a JSON blob instead of normalized cells`);
    });
  } catch (error) {
    addError('/baldininkai-org-gamintoju-finansai.csv', `missing or invalid CSV (${error.message})`);
  }
...
const expectedFinancialHistoryProfiles = manufacturers.filter((record) => filedFinancialHistory(record).length).length;
const expectedFinancialHistoryPeriods = manufacturers.reduce((total, record) => total + filedFinancialHistory(record).length, 0);
const expectedFiledFinanceYears = expectedFiledFinancialYearGroups(manufacturers).map(([year, rows]) => {
  const revenueRows = rows.filter(({ entry }) => entry.revenue);
  const revenues = revenueRows.map(({ entry }) => entry.revenue.amount).sort((a, b) => a - b);
  const pbtValues = rows.filter(({ entry }) => entry.profitBeforeTax).map(({ entry }) => entry.profitBeforeTax.amount);
  return {
    year,
    rows,
    revenueRows,
    ...
    revenueQ1: median(revenues.slice(0, Math.floor(revenues.length / 2))),
    revenueQ3: median(revenues.slice(Math.ceil(revenues.length / 2))),
    revenueBands: new Map([
      ['lt-100k', revenues.filter((amount) => amount < 100_000).length],
      ['100k-500k', revenues.filter((amount) => amount >= 100_000 && amount < 500_000).length],
      ['500k-2m', revenues.filter((amount) => amount >= 500_000 && amount <= 2_000_000).length],
      ['gt-2m', revenues.filter((amount) => amount > 2_000_000).length],
    ]),
...
const expectedFiledFinanceComparisons = expectedFiledFinanceYears.flatMap((current, index) => {
  const previous = expectedFiledFinanceYears[index - 1];
  ...
  const cohort = current.revenueRows
    ...
    .map(({ record, entry }) => ({ previous: previousRevenueBySlug.get(record.slug), current: entry.revenue.amount }));
...
const knownEnglishRegionLabels = new Set(ENGLISH_REGION_LABELS.keys());
const expectedEnglishRegionGroups = [...expectedRegions]
...
const expectedOfficialLocationRecords = manufacturers.filter((record) => !knownEnglishRegionLabels.has(record.region_label?.trim()));
if (expectedOfficialLocationRecords.length) {
  expectedEnglishRegionGroups.push({
    sourceLabel: 'Oficialaus šaltinio vietovės be katalogo regiono grupės',
    count: expectedOfficialLocationRecords.length,
    ...
    records: expectedOfficialLocationRecords,
  });
}
...
for (const file of files.sort()) {
  ...
  if (route === ENGLISH_HUB_PATH) {
    ...
    if (!html.includes('data-en-quote-request-cta') || !text.includes('Ready to describe a real project?') || !text.includes('submission is not an introduction guarantee') || !text.includes('makers are not contacted automatically')) {
      addError(route, 'English hub must contain the visible, accurately scoped quote-request callout');
    }
  ...
  if (route === ENGLISH_QUOTE_REQUEST_PATH) {
    counts.englishQuoteRequests += 1;
    const text = visibleText(html);
    for (const required of [
      'Prepare one comparable furniture quote request',
      'unverified public-source candidates',
      'submission does not guarantee an introduction',
      'makers are not contacted automatically',
      'Submitting the interactive form creates a private quote-request review record',
      'JavaScript is preparing the quote-request form',
      'enable JavaScript and reload the page',
    ]) {
      if (!text.includes(required)) addError(route, `missing English quote-request evidence: ${required}`);
    }
    if (!html.includes('data-en-quote-request-fallback')) addError(route, 'missing functional-page loading/fallback marker');
    for (const href of ['/en/', '/en/quote-request/', '/en/sourcing-guide/', '/en/lithuanian-furniture-makers-data/', '/atviri-duomenys/']) {
      if (!html.includes(`href="${href}"`)) addError(route, `English quote-request page is missing suite link ${href}`);
    }
    if ((html.match(/href="\/en\/quote-request\/"/g) ?? []).length < 3) addError(route, 'quote-request route must appear in the page navigation, tool navigation and footer');
    const webPages = schemasOfType(schemas, 'WebPage');
    if (webPages.length !== 1) addError(route, `expected exactly one WebPage schema, found ${webPages.length}`);
    else {
      const page = webPages[0];
      const expectedKeys = ['@context', '@type', 'description', 'inLanguage', 'isPartOf', 'name', 'url'];
      if (JSON.stringify(Object.keys(page).sort()) !== JSON.stringify(expectedKeys)) addError(route, 'quote-request WebPage schema contains missing or undocumented properties');
      if (page['@context'] !== 'https://schema.org' || page['@type'] !== 'WebPage' || page.name !== 'Request furniture quotes in Lithuania' || page.description !== descriptions[0] || page.url !== ENGLISH_QUOTE_REQUEST_URL || page.inLanguage !== 'en') addError(route, 'quote-request WebPage schema does not match the visible canonical page');
      if (JSON.stringify(page.isPartOf) !== JSON.stringify({ '@id': `${SITE_URL}/#website` })) addError(route, 'quote-request WebPage schema must reference the site WebSite identity');
      if (hasEmptySchemaValue(page)) addError(route, 'quote-request WebPage schema must not contain empty properties');
    }
  }
  ...
  if (expectedEnglishCategory) {
    ...
    for (const href of ['/en/', '/en/quote-request/', lithuanianPath, '/atviri-duomenys/', DATASET_LICENSE_URL]) {
      if (!html.includes(`href="${href}"`)) addError(route, `missing required English category link ${href}`);
    }
    if (!html.includes('data-en-quote-request-cta') || !text.includes('Turn this category search into a project brief') || !text.includes('does not contact the maker automatically')) {
      addError(route, 'English category page must contain the visible quote-request callout with the automatic-contact boundary');
    }
  ...
  if (isProfile) {
    ...
    if (!profileEntity) addError(route, 'manufacturer profile is missing its Organization or LocalBusiness schema');
      ...
      if (!source) addError(route, 'profile route has no matching source record');
        ...
        const history = filedFinancialHistory(source);
        const historyContainers = [...html.matchAll(/<div\b[^>]*\bdata-filed-financial-history\b[^>]*>/g)].map((match) => match[0]);
        const historyRows = [...html.matchAll(/<tr\b([^>]*)\bdata-filed-financial-period\b([^>]*)>([\s\S]*?)<\/tr>/g)];
        const historySourceLines = [...html.matchAll(/<p\b[^>]*\bdata-financial-history-source\b[^>]*>([\s\S]*?)<\/p>/g)];
        const turnoverRows = [...html.matchAll(/<dt>Apyvarta \((\d{4}) m\.\)<\/dt><dd>([\s\S]*?)<\/dd><\/div>/g)];
        if (history.length) {
          counts.financialHistories += 1;
          counts.financialHistoryPeriods += history.length;
          if (historyContainers.length !== 1) addError(route, `profile must render exactly one filed-financial-history block, found ${historyContainers.length}`);
          else if (attribute(historyContainers[0], 'data-period-count') !== String(history.length)) addError(route, `financial history period count must be ${history.length}`);
          if (historyRows.length !== history.length) addError(route, `profile must render ${history.length} valid latest financial periods, found ${historyRows.length}`);
          if (turnoverRows.length) addError(route, 'profile with valid filed financial history must not also render the legacy single-turnover row');
          historyRows.forEach(([, beforeMarker, afterMarker, row], index) => {
            const expected = history[index];
            if (!expected) return;
            const rowTag = `<tr ${beforeMarker} data-filed-financial-period ${afterMarker}>`;
            const expectedAttributes = {
              'data-period-start': expected.periodStart,
              'data-period-end': expected.periodEnd,
              'data-filing-registration-date': expected.filingDate,
              'data-source-url': expected.sourceUrl,
              'data-api-model-path': expected.apiModelPath,
              'data-jar-entity-id': expected.jarEntityId,
            };
            for (const [name, value] of Object.entries(expectedAttributes)) {
              if (attribute(rowTag, name) !== value) addError(route, `financial history row ${index + 1} has incorrect ${name}`);
            }
            if (!row.includes(`<time datetime="${expected.periodStart}">${expected.periodStart}</time> – <time datetime="${expected.periodEnd}">${expected.periodEnd}</time>`)) addError(route, `financial history row ${index + 1} must show its full fiscal period`);
            if (!row.includes(`<time datetime="${expected.filingDate}">${expected.filingDate}</time>`)) addError(route, `financial history row ${index + 1} must show its filing registration date`);

            const revenueCell = /<td\b([^>]*)\bdata-financial-revenue\b([^>]*)>([\s\S]*?)<\/td>/.exec(row);
            if (!revenueCell) addError(route, `financial history row ${index + 1} is missing its revenue cell`);
            else {
              const revenueTag = `<td ${revenueCell[1]} data-financial-revenue ${revenueCell[2]}>`;
              if (expected.revenue) {
                if (attribute(revenueTag, 'data-api-record-id') !== expected.revenue.apiRecordId || attribute(revenueTag, 'data-amount') !== String(expected.revenue.amount) || attribute(revenueTag, 'data-status')) addError(route, `financial history row ${index + 1} revenue evidence does not match the source entry`);
                if (!revenueCell[3].includes(formatEuro(expected.revenue.amount))) addError(route, `financial history row ${index + 1} must show revenue ${formatEuro(expected.revenue.amount)}`);
              } else if (attribute(revenueTag, 'data-status') !== 'not-filed' || !revenueCell[3].includes('Šiame laikotarpyje nepateikta')) addError(route, `financial history row ${index + 1} must clearly mark revenue as not filed`);
            }

            const profitCell = /<td\b([^>]*)\bdata-financial-profit-before-tax\b([^>]*)>([\s\S]*?)<\/td>/.exec(row);
            if (!profitCell) addError(route, `financial history row ${index + 1} is missing its profit-before-tax cell`);
            else {
              const profitTag = `<td ${profitCell[1]} data-financial-profit-before-tax ${profitCell[2]}>`;
              if (expected.profitBeforeTax) {
                if (attribute(profitTag, 'data-api-record-id') !== expected.profitBeforeTax.apiRecordId || attribute(profitTag, 'data-amount') !== String(expected.profitBeforeTax.amount) || attribute(profitTag, 'data-status')) addError(route, `financial history row ${index + 1} profit-before-tax evidence does not match the source entry`);
                if (!profitCell[3].includes(formatEuro(expected.profitBeforeTax.amount))) addError(route, `financial history row ${index + 1} must show profit before tax ${formatEuro(expected.profitBeforeTax.amount)}`);
              } else if (attribute(profitTag, 'data-status') !== 'not-filed' || !profitCell[3].includes('Šiame laikotarpyje nepateikta')) addError(route, `financial history row ${index + 1} must clearly mark profit before tax as not filed`);
            }
          });
          if (historySourceLines.length !== 1 || !historySourceLines[0][1].includes(`href="${OFFICIAL_FINANCIAL_SOURCE_URL}"`) || !visibleText(historySourceLines[0][1]).includes(`Šaltinis: ${OFFICIAL_FINANCIAL_SOURCE_NAME}`)) addError(route, 'financial history must have one clear source line naming and linking the official Lithuanian register/open-data portal');
        } else {
          if (historyContainers.length || historyRows.length || historySourceLines.length) addError(route, 'profile must not render filed financial history without at least one fully evidenced official entry');
          ...
          if (!turnover) {
          ...
          } else if (turnoverRows.length !== 1 || Number(turnoverRows[0]?.[1]) !== turnover.year) {
            addError(route, `profile without valid history must preserve exactly one Apyvarta (${turnover.year} m.) row for published turnover`);
          } else {
            ...
            if (!turnoverHtml.includes(`href="${turnover.sourceUrl}"`) || !/Atverti apyvartos šaltinį/.test(turnoverHtml)) addError(route, 'profile turnover row must link its public financial source with the source label');
          }
        }
      }
    }
  ...
  if (route === '/atviri-duomenys') {
    const text = visibleText(html);
    for (const required of ['Atviri Baldininkai.org duomenys', 'JSON duomenų rinkinys', 'Katalogo CSV duomenų rinkinys', 'Pateiktų finansinių laikotarpių CSV', 'viena eilutė vienam galiojančiam įmonės fiskaliniam laikotarpiui', 'Registrui pateikti finansiniai rodikliai', 'konkreti pateikta eilutė turi API įrodymą', `visus ${manufacturers.length}`, `kiekvienam iš ${manufacturers.length}`, OFFICIAL_FINANCIAL_SOURCE_NAME, 'not_published', 'no_public_contact_route', 'Ribotumai', 'Licencija ir priskyrimas', DATASET_LICENSE_NAME, 'CC BY 4.0', 'versija 4.0', 'This dataset is licensed under Creative Commons Attribution 4.0 International (CC BY 4.0).', generatedDatasetMetadata?.attribution]) {
      if (!required || !text.includes(required)) addError(route, `missing open-data content: ${String(required)}`);
    }
    if (!html.includes('href="/baldininkai-org-gamintojai.json"') || !html.includes('href="/baldininkai-org-gamintojai.csv"') || !html.includes('href="/baldininkai-org-gamintoju-finansai.csv"')) addError(route, 'missing dataset download links');
    ...
    if (!html.includes(`href="${OFFICIAL_FINANCIAL_SOURCE_URL}"`)) addError(route, 'missing direct Lithuanian official register/open-data source link');
    ...
    if (datasetSchemas.length !== 1) addError(route, `expected exactly one Dataset schema, found ${datasetSchemas.length}`);
      ...
      if (typeof datasetSchema.description !== 'string' || !datasetSchema.description.includes('finansinių rodiklių') || !datasetSchema.description.includes('pateiktų finansinių laikotarpių')) addError(route, 'Dataset schema description must cover source-backed latest and filed financial data');
      ...
      if (!Array.isArray(datasetSchema.keywords) || datasetSchema.keywords.length < 5 || !datasetSchema.keywords.includes('pateiktos finansinės ataskaitos') || datasetSchema.keywords.some((keyword) => typeof keyword !== 'string' || !keyword.trim())) addError(route, 'Dataset schema keywords must include filed-financial coverage and suitable terms');
      ...
      if (!Array.isArray(datasetSchema.distribution) || datasetSchema.distribution.length !== 3) addError(route, 'Dataset schema must contain exactly three DataDownload distributions');
      else {
        const expectedDistributions = new Map([
          [DATASET_JSON_URL, 'application/json'],
          [DATASET_CSV_URL, 'text/csv'],
          [FINANCIAL_DATASET_CSV_URL, 'text/csv'],
        ]);
  ...
  if (route === MARKET_OVERVIEW_PATH) {
    ...
    for (const href of ['/atviri-duomenys/', '/baldu-sektoriaus-finansai/', '/baldininkai-org-gamintojai.json', '/baldininkai-org-gamintojai.csv']) {
  ...
  if (route === FILED_FINANCE_PATH) {
    counts.filedFinancePages += 1;
    const text = visibleText(html);
    for (const required of [
      'Baldų sektoriaus finansai pagal katalogo įmonių ataskaitas',
      'Registrui pateiktos pajamos pagal metus',
      'Pajamų intervalai',
      'Tų pačių įmonių pokytis tarp gretimų metų',
      'Pelno prieš apmokestinimą aprėptis',
      'Metodika ir ribotumai',
      'medianos padalytų pusių metodu',
      'Skirtingų įmonių metinės bendros sumos nėra lyginamos kaip augimas.',
      '„Nepakito“ reiškia tiksliai tokią pačią sumą eurais abiem metais.',
      'Trūkstama PBT reikšmė nėra laikoma nuliu ir nėra numanoma.',
      'Tai katalogo aprėpties suvestinė, o ne Lietuvos nacionalinė ar baldų rinkos statistika.',
      'mišrias finansinių ataskaitų formas ir fiskalinius laikotarpius',
      'nėra įmonių reitingas, rekomendacija ar darbų kokybės, pajėgumo, užimtumo bei tinkamumo konkrečiam projektui vertinimas',
      'tik kai konkreti Juridinių asmenų registrui pateikta eilutė Lietuvos atvirų duomenų portale turi API įrodymą',
    ]) {
    ...
    if (annualRows.length !== expectedFiledFinanceYears.length) addError(route, `expected ${expectedFiledFinanceYears.length} filed-finance annual rows, found ${annualRows.length}`);
    for (const summary of expectedFiledFinanceYears) {
      ...
      if (!html.includes(attributes)) addError(route, `annual revenue attributes for ${summary.year} do not match independently calculated values`);
      const row = new RegExp(`<tr ${attributes}>([\\s\\S]*?)<\\/tr>`).exec(html)?.[1] ?? '';
      for (const value of [summary.revenueCount, formatEuro(summary.revenueTotal), formatEuro(summary.revenueMedian), formatEuro(summary.revenueQ1), formatEuro(summary.revenueQ3)]) {
        if (!row.includes(String(value))) addError(route, `annual revenue row ${summary.year} is missing ${value}`);
      }

      const bandRow = new RegExp(`<tr data-filed-band-year="${summary.year}">([\\s\\S]*?)<\\/tr>`).exec(html)?.[1] ?? '';
      if (!bandRow) addError(route, `missing revenue-band row for ${summary.year}`);
      for (const [band, count] of summary.revenueBands) {
        if (!bandRow.includes(`data-filed-revenue-band="${band}" data-count="${count}"`)) addError(route, `revenue band ${band} for ${summary.year} must equal ${count}`);
        if (!bandRow.includes(`${count} (${formatPercent(count, summary.revenueCount)})`)) addError(route, `revenue band ${band} for ${summary.year} is missing its count/share`);
      }
      ...
      if (!html.includes(pbtAttributes)) addError(route, `PBT attributes for ${summary.year} do not match independently calculated values`);
      const pbtRow = new RegExp(`<tr ${pbtAttributes}>([\\s\\S]*?)<\\/tr>`).exec(html)?.[1] ?? '';
      for (const value of [
        `${summary.pbtCount} iš ${summary.rows.length} (${formatPercent(summary.pbtCount, summary.rows.length)})`,
        `${summary.pbtPositive} (${formatPercent(summary.pbtPositive, summary.pbtCount)})`,
        `${summary.pbtZero} (${formatPercent(summary.pbtZero, summary.pbtCount)})`,
        `${summary.pbtNegative} (${formatPercent(summary.pbtNegative, summary.pbtCount)})`,
      ]) {
        if (!pbtRow.includes(value)) addError(route, `PBT row ${summary.year} is missing ${value}`);
      }
    }

    const cohortRows = [...html.matchAll(/<tr data-filed-cohort-from="(\d{4})"[^>]*>/g)];
    if (cohortRows.length !== expectedFiledFinanceComparisons.length) addError(route, `expected ${expectedFiledFinanceComparisons.length} same-company cohort rows, found ${cohortRows.length}`);
    for (const comparison of expectedFiledFinanceComparisons) {
      ...
      if (!html.includes(attributes)) addError(route, `same-company cohort ${comparison.fromYear}–${comparison.toYear} does not match independently calculated values`);
      const row = new RegExp(`<tr ${attributes}>([\\s\\S]*?)<\\/tr>`).exec(html)?.[1] ?? '';
      for (const value of [
        `${formatEuro(comparison.previousTotal)} → ${formatEuro(comparison.currentTotal)}`,
        `${formatSignedEuro(comparison.change)}${comparison.changeRatio == null ? '' : ` (${formatSignedPercent(comparison.changeRatio)})`}`,
        `${comparison.grew} (${formatPercent(comparison.grew, comparison.count)})`,
        `${comparison.held} (${formatPercent(comparison.held, comparison.count)})`,
        `${comparison.declined} (${formatPercent(comparison.declined, comparison.count)})`,
      ]) {
        if (!row.includes(value)) addError(route, `same-company cohort ${comparison.fromYear}–${comparison.toYear} is missing ${value}`);
      }
    }

    const articles = schemasOfType(schemas, 'Article');
    const filedFinanceArticle = articles.find((article) => article.url === FILED_FINANCE_URL);
    if (!filedFinanceArticle) addError(route, 'missing Article schema for the filed-finance canonical URL');
    else {
      const expectedKeys = ['@context', '@type', 'author', 'dateModified', 'datePublished', 'description', 'headline', 'inLanguage', 'mainEntityOfPage', 'publisher', 'url'];
      if (JSON.stringify(Object.keys(filedFinanceArticle).sort()) !== JSON.stringify(expectedKeys)) addError(route, 'filed-finance Article schema contains missing or undocumented properties');
      if (filedFinanceArticle['@context'] !== 'https://schema.org' || filedFinanceArticle['@type'] !== 'Article') addError(route, 'filed-finance structured data must be a schema.org Article');
      if (filedFinanceArticle.headline !== 'Baldų sektoriaus finansai: registrui pateiktų duomenų metinė suvestinė' || filedFinanceArticle.description !== descriptions[0]) addError(route, 'filed-finance Article headline or description is inaccurate');
      if (filedFinanceArticle.url !== FILED_FINANCE_URL || filedFinanceArticle.mainEntityOfPage !== FILED_FINANCE_URL || filedFinanceArticle.inLanguage !== 'lt-LT') addError(route, 'filed-finance Article canonical page or language is inaccurate');
      if (!validIsoDate(filedFinanceArticle.datePublished) || !validIsoDate(filedFinanceArticle.dateModified) || filedFinanceArticle.datePublished !== generatedDatasetMetadata?.generated_date || filedFinanceArticle.dateModified !== generatedDatasetMetadata?.generated_date) addError(route, 'filed-finance Article dates must match the generated build date');
      for (const role of ['author', 'publisher']) {
        if (JSON.stringify(filedFinanceArticle[role]) !== JSON.stringify({ '@id': `${SITE_URL}/#organization` })) addError(route, `filed-finance Article ${role} must reference the site organization`);
      }
    }
  }
  ...
  if (route === ENGLISH_MARKET_OVERVIEW_PATH) {
    ...
    for (const [region, count] of expectedRegions) {
      ...
      const englishRegion = expectedEnglishRegionGroups.find((group) => group.sourceLabel === region)
        ?? expectedEnglishRegionGroups.find((group) => group.path === '/en/regions/official-source-locations-unassigned-region');
  ...
  if (!/href="\/baldu-sektoriaus-finansai\/?"/.test(html)) addError(route, 'market-related footer navigation is missing the filed-finance link');
